...
| `duo inspect [--on\|--off]` | **ENH-156b** — toggle element-inspect mode in the active browser pane. No arg toggles; `--on` / `--off` force a state. While active, hover any element to outline it in Duo orange; click to ship its tag + selector_path + heading trail + capped innerText + key attrs (id/role/aria-label/href/etc.) to the active terminal as a structured paste. ESC inside the page exits without picking. Pairs with `duo dom <selector>` — the inspect snapshot tells you WHERE on the page; `duo dom` reads the structure once you know. Chord parity: ⌘⇧C inside the WCV is the keystroke equivalent. | JSON: `{active}` |
