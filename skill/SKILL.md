...
> **CRITICAL — same rule for `.html` files open in Duo's canvas.** Mutate the live canvas through the `duo html` verbs (`set` / `replace` / `append` / `remove` / `attr` for surgical edits against `data-duo-id` anchors; `new` to seed a fresh file). Filesystem `Write`/`Edit` against an open canvas has the same silent-overwrite class as the markdown case (FOLLOWUP-019's watcher catches the read direction but cannot un-clobber a save race). Using the `duo html` verbs also paints the highlighted-edit visual that lets the user see exactly which parts you changed — a plain `Write` paints nothing. If you're regenerating an entire HTML document and the `duo html` verbs are awkward, close the tab first (`duo tabs` to find the id, then `duo close <n>`), `Write` the new file, then `duo open --canvas <path>` to reopen — that sidesteps the race entirely.
