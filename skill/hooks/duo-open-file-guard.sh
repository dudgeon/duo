#!/bin/sh
# duo-open-file-guard — ENH-195 PreToolUse nudge (warn-only).
#
...
# autosave (BUG-085) and skip the change-highlight a `duo` verb paints.
#
...
#    `duo status` is an ENH-195 verb; if this install predates it the call
#    just errors and we no-op — exactly the intended fail-open behaviour.
STATUS=$(node -e '
...
case "$TARGET" in
  *.md|*.markdown)
    echo "Duo: '$TARGET' is open in the markdown editor. Prefer a duo doc verb so your change lands in the live buffer and paints the change-highlight: 'duo doc edit <path> --find \"…\" --replace \"…\"' for a surgical swap, or 'duo doc write --replace-all' (stdin) to rewrite the body. A direct Write can be clobbered by the editor's autosave (BUG-085) and skips the highlight. Proceeding anyway." >&2
    ;;
  *.html|*.htm)
    echo "Duo: '$TARGET' is open in the HTML canvas. Prefer the duo html verbs so element IDs + comment threads survive: 'duo html set --id <duo-id> --content \"…\"' / 'duo html replace --id <duo-id> --html \"…\"' / 'duo html append --parent <duo-id> --html \"…\"' / 'duo html attr --id <duo-id> --set k=v'. A direct Write can be clobbered by autosave (BUG-085), drops data-duo-id anchors, and paints no change-highlight. Proceeding anyway." >&2
    ;;
  *.json|*.yaml|*.yml|*.jsonl)
    echo "Duo: '$TARGET' is open in the JSON/YAML view. Prefer a duo json verb so the edit goes through the live surface: 'duo json set <path> <dotpath> <value>' for a single key, or 'duo json merge <path> <file.json>' to deep-merge. A direct Write can be clobbered by the view's autosave (BUG-085) and skips the change-highlight. Proceeding anyway." >&2
    ;;
esac
