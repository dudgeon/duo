...
>    live **`.base`** file rendered by `duo base render`; in **OKF mode**
>    there are **no `.base` files** — listings are static `index.md`/`log.md`
>    regenerated by `duo vault publish`. Authoring a `.base` in an OKF vault
>    is a no-op. Same intent, different verb.
