...
>    at-rest listings are static `index.md`/`log.md` regenerated by
>    `duo vault publish` (the root `index.md` can carry a `listing:` spec for a
>    grouped, engine-driven body — ENH-230). OKF doesn't *auto-render* `.base`
>    files the way Obsidian does — but you still **author a `.base` as a query
>    and render it on demand** with `duo rollup render <base> --md|--html` (both
>    modes; the way to make a shareable HTML rollup from an OKF vault — see
>    [rollup.md](rollup.md)).
