...
| `duo cron list` | List scheduled ("cron") Claude jobs — each a `CronJobView` (the job + computed `nextFireAt` + human `scheduleLabel` + last-run status). Runs are **interactive only** and fire **only while Duo is open** (ENH-221). | JSON: `CronJobView[]` |
| `duo cron add --name <n> --cwd <path> --say "<instruction>" (--every hourly\|daily\|weekdays\|weekly [--at HH:MM] [--on <weekday>] \| --cron "<expr>") [--session fresh\|same] [--catch-up]` | Create a job. On fire, Duo opens an interactive Claude tab seeded with the instruction (`claude --session-id <uuid> "<instruction>"` fresh, or `claude --resume <uuid> "<instruction>"` for `--session same`). `--catch-up` runs once on next launch if an occurrence was missed while Duo was closed. Headless `-p` is rejected (gated by `FEATURE_HEADLESS_CRON`, default off). | JSON: `CronJobView` |
| `duo cron show <id>` | Inspect one job's full recipe + next-fire + last-run pointer. | JSON: `CronJobView` |
| `duo cron run <id>` | Fire a job now (manual), same path as a scheduled fire. | JSON: `CronJobView` |
| `duo cron pause <id>` / `duo cron resume <id>` | Disable / re-enable a job without deleting it (paused jobs persist but never fire). | JSON: `CronJobView` |
| `duo cron rm <id>` | Delete a job. | JSON: `{ ok, removed }` |
...
| `duo window new [--cwd <path>]` | ENH-191 P5a — open a SECOND app window (blank: its own workspace, browser pane, navigator — not a clone of window 1). Same action as File → New Window (⌥⌘N). `--cwd` roots the new window's navigator at a path (e.g. a git worktree — ENH-210 D1-part2). Requires "Allow Multiple Windows" (Settings menu, default on); exits non-zero with a disabled-error when off (never a silent no-op). | JSON: `{ ok, error? }` |
...
### Vault (ENH-208 + ENH-216) — work-notes conventions

A **vault** is a folder of work-notes (markdown + YAML frontmatter + folders + a link graph). **ENH-216 — two at-rest formats, one graph model:** **OKF** (standard markdown relative links `[Display](./<note>.md)`, root `okf_version` `index.md` marker) and **Obsidian** (`[[wikilinks]]` + `.obsidian/` marker + `.base` rollups). `okf_version` wins if both markers are present (D4). These verbs read the filesystem **directly** — no running app, no socket — so they also work headless. The vault is resolved by walking up from the cwd to the nearest marker; pass `--vault <path>` to target another.

| Command | What it does | Output |
| --- | --- | --- |
| `duo vault init <path> --format=okf\|obsidian [--name "…"] [--force]` | Scaffold a new vault. **`--format` is REQUIRED** (ENH-216 D2 — deliberate asymmetry; the New Vault dialog defaults to OKF). `okf`: root `okf_version` `index.md` marker + static listings + starter `templates/` (D19 filing rules) + `inbox/` + registry/`notes/` folders, no `.obsidian/`. `obsidian`: `.obsidian/` + `bases/processing.base` + README (legacy). The fresh vault becomes the default. Refuses to clobber unless `--force`. Warns if the target is under `~/Documents` (iCloud-eviction trap). | JSON: `{ root, created[], warnings[], mode }` |
...
| `duo vault mv <from> <to> [--vault <path>]` | **ENH-216 D5 (clean path)** — move a note (vault-relative POSIX paths) and rewrite every inbound markdown link to its new home, re-basing the moved note's own outbound links. Throws on a dest collision (never clobbers). In OKF mode a move changes link paths, so prefer this over `duo file rename` / shell `mv` for vault notes. | JSON: `{ fromRel, toRel, inboundRewritten:[{fromRel, count}], outboundRebased }` |
| `duo vault relink [--dry-run] [--vault <path>]` | **ENH-216 D5 (out-of-band repair)** — re-resolve markdown links broken by a move outside Duo (Finder/git) by stable frontmatter `id:` first (the primary key, minted on every OKF note at create time, D10), then slug/basename fallback. Rewrites the unambiguous ones; **reports** ambiguous (>1) + broken (0) — never guesses (warn-don't-block). Auto-runs on vault open; this verb is the headless/inspection twin. `--dry-run` reports without writing. | JSON: `{ repaired:[{fromRel, oldHref, newHref, via:'id'\|'slug', targetRel}], ambiguous[], broken[] }` |
| `duo vault publish [--index-only\|--log-only] [--dir] [--open] [--vault <path>]` | **ENH-216 D8** — (re)generate the OKF static listings from the corpus: root `index.md` (section-6: a heading per type/group, then `* [Title](rel) - description` bullets; the root frontmatter is byte-preserved, only the body after the `<!-- duo:listing -->` fence is regenerated) + `log.md` (section-7: `## YYYY-MM-DD` groups newest-first from file mtimes). `--dir` adds per-folder `index.md`; `--index-only`/`--log-only` narrow; `--open` surfaces `index.md` as a tab. OKF-mode-gated (throws in Obsidian mode — Obsidian stays byte-identical). | JSON: `{ mode, written[] }` |
| `duo vault promote <note> --heading "<h>" --type <t> [--vault <path>]` | **ENH-216 D9** — split a `## heading` section of a note into its own typed entity (D19-filed), remove the section from the source note, and leave a markdown LINK behind (a `[[wikilink]]` in Obsidian) — **NEVER an embed-transclusion**. Heading matched case-insensitively. | JSON: `{ entityRel, leftLink, created }` |
| `duo graph backlinks <note> [--vault <path>]` | Every note that links to `<note>` — both wikilinks (matched by basename, so they survive moves) AND markdown relative links (ENH-216) are edges; scans frontmatter relationships as well as body links. | JSON: `[{ path, absPath, line, excerpt }]` |
