...
- TCP fallback validated via `DUO_TCP_ONLY=1` simulation; first real-sandbox confirmation comes from the owner's next Acme Claude Code session post-install. The `duo doctor` output names the failure mode if anything's off.
