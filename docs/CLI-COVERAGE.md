...
| `duo goto <home\|vault\|rollups>` | **ENH-257** — activate the synthesized Home/Vault/Rollups top-level tab (same as a sidebar/tab-strip click); `view`/`edit` don't recognize these tabs' `duo://` sentinel paths, and there was previously no CLI path to them at all. Errors (not silent) when `vault`/`rollups` is requested with no default vault set |
...
| `duo doc delete <file> --text "X" [--occurrence N]` | **BUG-138 Phase 3; ENH-260 D5** — wrap "X" as a CM deletion (`{--X--}`). Composes with pending `{++…++}` insertion tokens instead of refusing: a sub-range inside one shrinks/drops the token (net-zero), a plain sub-range gets `{--…--}`, and a range spanning both decomposes. `changed=false` only if the range overlaps an existing deletion/substitution/highlight/comment token (split the op). |
| `duo doc substitute <file> --text "X" --with "Y" [--occurrence N]` | **BUG-138 Phase 3; ENH-260 D5** — wrap "X→Y" as a CM substitution (`{~~X~>Y~~}`). Empty `--with` = effective delete. A range fully inside one pending `{++…++}` insertion edits that suggestion's text in place instead; a range spanning insertion + plain text decomposes (per delete) then appends `{++Y++}`. Same non-insertion overlap refusal as delete. |
...
| `duo vault relink --frontmatter [--dry-run] [--vault p]` | **ENH-266** — the frontmatter-link migration (OKF-mode only; throws otherwise). **Runs AUTOMATICALLY on the auto-relink-on-vault-open pass** (owner decision 2026-07-13, superseding the original opt-in-only lock — same write/report + D5 foreign-bundle gating as `relink`, so a legacy OKF vault just works in Obsidian with zero user action); this verb runs the SAME migration explicitly, for a `--dry-run` preview or headless/CI use. Four categories, id-then-slug resolved (never guessed — ambiguous/broken are reported and left untouched): **(a)** frontmatter wikilink values (`owner: "[[Alice Park]]"`, scalar/list, quoted/unquoted) → the quoted markdown-link form (a title-based frontmatter wikilink is an unresolved phantom node in Obsidian, since OKF filenames are slugs, not titles); **(b)** frontmatter BARE unbracketed rel-path values (`owner: "./people/alice-park.md"` — invisible to every link parser) → the quoted markdown-link form, only when the path resolves to a real file; **(c)** leftover prose-BODY wikilinks (someone edited the vault in Obsidian with factory-default wikilink settings) → markdown links, reusing the SAME id-then-slug resolution; **(d)** alias backfill — an entity whose `title:` differs from its slug filename gets that title appended to `aliases:` (skipped if already present). The generated-listing marker files (`_index.md`/`index.md`/`_log.md`/`log.md`) are never touched (frontmatter byte-preservation). `--dry-run` resolves + reports every category but writes nothing | JSON `{frontmatterWikilinks:{repaired[],ambiguous[],broken[]}, frontmatterBarePaths:{…}, bodyWikilinks:{…}, aliasBackfills:[{fromRel,title}]}` |
...
| `duo rollup new --type <t[,t2]> [--title "t"] [--group a,b] [--bucket v[=Label]]... [--filter k=v\|k!=v\|k~=v\|k^=v\|@=v\|k?\|k!?]... [--columns a,b] [--vault p]` | **ENH-243** — scaffold a builder-canonical `type: rollup` note (the Rollups tab's CLI twin); ordered `--group` = multi-depth grouping (level 1 in the base block, full list in `group_by:` frontmatter). **ENH-255:** `k~=v` = membership on a multi-valued field (list-of-links matched by note IDENTITY, not display text); **ENH-259:** `k^=v` = transitive "is under" (any_parent) — `parent^=California` matches every row whose `parent` chain reaches California at any level (`ancestors("parent").contains(…)`, cycle-safe); **ENH-262:** `@=v` = "links to" (any-property entity edge, `file.hasLink`); **ENH-261:** `--group`/`--columns` accept `ancestor:<prop>:<type>` (group by the goal a row rolls up to). Slug operands canonical; display names fold (BUG-260); `--bucket 'value[=Label]'` declares level-1 buckets that always render — even empty — in flag order (needs `--group`; splits at the first unescaped `=`, `\=` escapes a literal `=` in the value) | JSON `{root, note, absPath, model}` |
