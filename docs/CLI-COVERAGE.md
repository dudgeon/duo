...
| `duo rollup new --type <t[,t2]> [--title "t"] [--group a,b] [--bucket v[=Label]]... [--filter k=v\|k!=v\|k~=v\|k^=v\|k?\|k!?]... [--columns a,b] [--vault p]` | **ENH-243** — scaffold a builder-canonical `type: rollup` note (the Rollups tab's CLI twin); ordered `--group` = multi-depth grouping (level 1 in the base block, full list in `group_by:` frontmatter). **ENH-255:** `k~=v` = membership on a multi-valued field (list-of-links matched by note IDENTITY, not display text); **ENH-259:** `k^=v` = transitive "is under" (any_parent) — `parent^=California` matches every row whose `parent` chain reaches California at any level (`ancestors("parent").contains(…)`, cycle-safe); `--bucket 'value[=Label]'` declares level-1 buckets that always render — even empty — in flag order (needs `--group`; splits at the first unescaped `=`, `\=` escapes a literal `=` in the value) | JSON `{root, note, absPath, model}` |
