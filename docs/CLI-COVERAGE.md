...
| `duo worktree [list] [<path>]` | **ENH-210** — list the git worktrees of the repo at `<path>` (defaults to the cwd). Reads git directly (client-side in the CLI bundle, like the vault verbs — no socket / running app needed, sandbox-tolerant). Returns JSON `[{ path, branch, head, isMain, isCurrent, detached, prunable, colorIndex }]` (main checkout first, the cwd's worktree flagged `isCurrent`; `colorIndex` is the hash-stable project hue per `shared/projects.ts`). Powers the navigator Worktrees section + lets an agent tell whether it is in a linked worktree vs main. |
...
### Scheduling (cron) — ENH-221 Tier 1 (engine + CLI)
...
| Verb | UI parallel | Shape |
|---|---|---|
| ✅ `duo cron list` | Home "Scheduled" rows (Tier 2) | **Shipped (ENH-221 Tier 1).** `CronJobView[]` — each job + computed `nextFireAt` + human `scheduleLabel` + last-run status. |
| ✅ `duo cron add --name <n> --cwd <path> --say "<instruction>" (--every hourly\|daily\|weekdays\|weekly [--at HH:MM] [--on <weekday>] \| --cron "<expr>") [--session fresh\|same] [--catch-up]` | "+ Schedule" / File ▸ New Scheduled Job (Tier 2) | **Shipped (ENH-221 Tier 1).** Creates a job. Fresh runs pre-allocate the session id (`claude --session-id <uuid> "<instruction>"`, D3); `--session same` resumes the prior run's session (`claude --resume <uuid> …`), falling back to fresh if it's gone. `--catch-up` = run once on next launch if an occurrence was missed while closed (D5). Returns `CronJobView`. |
| ✅ `duo cron edit <id> [--name <n>] [--cwd <path>] [--say "<instruction>"] (--every <preset> [--at HH:MM] [--on <weekday>] \| --cron "<expr>") [--session fresh\|same] [--catch-up \| --no-catch-up]` | Home "Edit" row action (Tier 2) | **Shipped (ENH-221 Tier 2).** Edits a job — only the flags you pass change; any schedule flag replaces the whole schedule; `--no-catch-up` turns catch-up off. Returns `CronJobView`. |
| ✅ `duo cron show <id>` | Job row detail (Tier 2) | **Shipped (ENH-221 Tier 1).** One job's `CronJobView`. |
| ✅ `duo cron run <id>` | "Run now" row action (Tier 2) | **Shipped (ENH-221 Tier 1).** Fires a job now (manual), same path as a scheduled fire. Returns `CronJobView`. |
| ✅ `duo cron pause <id>` / `duo cron resume <id>` | Pause/resume toggle (Tier 2) | **Shipped (ENH-221 Tier 1).** Disable / re-enable without deleting (paused jobs persist but never fire). Returns `CronJobView`. |
| ✅ `duo cron rm <id>` | Delete row action (Tier 2) | **Shipped (ENH-221 Tier 1).** Deletes a job. Returns `{ ok, removed }`. |

**Tier 2 (Home surface) + ENH-223 (the "waiting on you" tab badge) are tracked
separately** — see `docs/prd/enh-221-scheduled-sessions.md`.
