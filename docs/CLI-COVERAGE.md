...
| `duo split-view <op> [args]` | ENH-041 / Sprint 3 — Split View aux pane (canvas's right-side companion slot). Sub-verbs: `open <path>`, `close`, `promote`, `resize <pct>`, `state` (or no sub-verb). v1 single-slot; multi-tab aux queued for v2. State is renderer-authoritative; main caches snapshot for the no-arg query. Locked spec: `docs/prd/canvas-split-view-research.html`. |
