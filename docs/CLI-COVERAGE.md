...
| `duo window new` | **ENH-191 P5a** — open a SECOND app window (blank: own workspace/browser/navigator, NOT a clone of window 1). Same action as File → New Window (⌥⌘N). Gated on the "Allow Multiple Windows" setting (default on); exits non-zero when off. CLI parity for the menu item. |
