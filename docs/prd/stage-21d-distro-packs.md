...
A distro pack lives in a single folder. Duo walks the folder
structure to discover content; the manifest carries only identity
+ FTUX/integration toggles.

```
<distro-pack-root>/
├── DISTRO.json                       # identity + integration toggles
├── skills/                           # → ~/.claude/skills/<distro>/
│   ├── make-page-aip.md
│   └── corp-onboarding.md
├── agents/                           # → ~/.claude/agents/<distro>-*.md
│   └── compliance-reviewer.md
├── packs/                            # → ~/.claude/duo/packs/<distro>-<pack>/
│   └── aip-quickstart/               #   (Stage 28 lesson-pack format)
│       ├── PACK.json
│       ├── canvases/
│       └── lesson-skill/
├── canvas-templates/                 # → ~/.claude/duo/distros/<distro>/canvas-templates/
│   └── corp-poster.html
├── playgrounds/                      # → ~/.claude/duo/distros/<distro>/playgrounds/
│   └── dashboard.html
├── canvases/                         # → ~/.claude/duo/distros/<distro>/canvases/
│   └── welcome.html
...
**Internal namespacing rule:** within a single distro pack, every
skill / agent / template / playground / canvas filename must be
unique. The pack-builder skill validates this at authoring time.

**Cross-distro collision:** every distro lands under its own
`<distro-name>/` namespace (in `~/.claude/skills/<distro>/`,
`~/.claude/duo/distros/<distro>/`, etc.). Collisions across distros
are impossible by construction — different `<distro-name>` ⇒
different paths.
...
- `name` — kebab-case unique identifier; namespaces every install
  destination. Forms the `<!-- distro:<name>-managed-vX.Y.Z -->`
  marker in user CLAUDE.md.
...
- `openOnFirstLaunch` — relative paths that auto-open as canvas /
  playground tabs on FIRST detection of this pack. Re-fires if a
  new pack is detected post-original-FTUX.
...
- `claudeMdSnippet` / `primingAdditions` / `externalDomainsAdditions`
  — booleans that gate whether the corresponding files at the
  pack root get installed/merged. Owner can opt out of any subset
...
1. **Read & validate DISTRO.json.** Schema check; fail with clear
   message if invalid.
...
4. **Merge integration files:**
   - `claude-md-snippet.md` (if `claudeMdSnippet: true`) → upserts
     a `<!-- distro:<name>-managed-vX.Y.Z -->` block in
     `~/.claude/CLAUDE.md`. Coexists with Duo's own managed block
     and any other distros' blocks. Same merge logic as ENH-088.
   - `priming-additions.md` (if `primingAdditions: true`) →
     appended to `~/.claude/duo/priming.md` between
     `<!-- distro:<name>-priming-start -->` /
     `<!-- distro:<name>-priming-end -->` markers.
   - `external-domains.json` (if `externalDomainsAdditions: true`)
     → entries union-merged into the user's list.
5. **FTUX:**
   - If this is the FIRST EVER install of this pack, fire FTUX:
     open every `openOnFirstLaunch[]` entry as a tab on next
     visible launch; add `pinnedFiles[]` to pins.
   - Re-fires once per pack-name (tracked via
     `~/.claude/duo/installed.json § distroFtuxFired[<name>]: true`).
6. **Notify:** banner / toast: "AIP Corporate pack 1.0 installed —
   N new canvases, M new skills."
...
2. Delete every file in the manifest.
...
User-customized files (e.g. user edited `make-page-aip.md` after
install) are detected via content-hash comparison; the user's
edited version is preserved at
`~/.claude/skills/<distro>/.user-modified/make-page-aip.md` so
the work isn't lost. Mirror of Stage 21e-iii's provenance pattern.
...
1. **Scaffold a new pack.** `Build me an AIP corporate distro pack`
   ...
   stub `DISTRO.json`, empty content directories, README explaining
   conventions.
2. **Validate an existing pack.**
   - Schema-check `DISTRO.json` against the v1 schema.
   - Walk content directories; confirm namespacing (no duplicate
     filenames within the pack).
...
4. **Update version.**
   - Bumps `DISTRO.json § version` (semver-aware: --patch / --minor
     / --major).
   - Updates the marker version in the pack's `claude-md-snippet.md`
     (so the post-install marker matches the new version).
...
- `requiresDuoVersion` enforcement.
...
- New skill at `skill/pack-builder.md` covering scaffold / validate
  / build (zip + pkg + bundled-fork) / version-bump.
- Synced via `npm run sync:claude`.
- DISTRO v1 schema lives at `skill/references/distro-v1-schema.json`
  (canonical reference for distros).
