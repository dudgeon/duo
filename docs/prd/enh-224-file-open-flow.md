...
| **Phase 3 — already-local detection** (`core/git/local-clone-match.ts` + socket `open` branch + `OPEN_MATCH_LOCAL_CLONE` IPC + OpenBar third choice + `duo open --checkout`) | D6 | ✅ BUILT + **live-verified 2026-06-21** | typecheck clean · 6 pure tests (`remoteMatchesRepo`) · currency 76/76 · prod bundle compiles. Before a checkout, matches the repo against navigator git-root projects (by `origin` remote + file-exists); opens from the user's clone (modality 1) — UI offers it, CLI prefers-and-reports (`--checkout` overrides). Never edits the user's tree; footer gated to managed checkouts. **Live:** `duo open dudgeon/duo/blob/main/CLAUDE.md` → `via:local-clone` opened the LOCAL CLAUDE.md (on its real branch, not `main`); `--checkout` forced the managed checkout; the `matchLocalClone` IPC returns the match. |
| **Phase 4 — format breadth + escape hatch** (`ProposeBar` in `JsonView` + `PageTab`; `core/git/share-back.exportCheckoutFile` + `duo pr export`) | D8, D4 | ✅ BUILT + **live-verified 2026-06-21** | typecheck clean · 1750 tests · currency 76/76 · prod bundle compiles. The "Propose changes" footer now mounts in the JSON/YAML viewer + the HTML canvas (format-agnostic round-trip — D8); `duo pr export <path> <dest>` is the D4 "save a local copy" escape hatch (refuses to clobber an existing dest); binary stays view-only (footer is text-editor-only). **Live:** the footer rendered + morphed in a JSON checkout doc AND an HTML-canvas checkout doc; `duo pr export` copied to `/tmp` then refused the existing dest. UI "Save a copy…" affordance deferred (surface = owner UX choice). |
...
- **2026-06-21 — Phases 3 + 4 LIVE-VERIFIED → ENH-224 fully done.** Walked on the
  dev build: Phase 3 — `duo open dudgeon/duo/blob/main/CLAUDE.md` (dudgeon/duo is
  a navigator project) returned `via:local-clone` + opened the LOCAL CLAUDE.md on
  its real branch (`claude/enh-221-file-history`, NOT the URL's `main` — D6 opens
  the user's version), `--checkout` forced the managed checkout, and the
  `matchLocalClone` IPC (the Open bar's 3rd-choice source) returned the match.
  Phase 4 — the "Propose changes" footer rendered + morphed in a JSON checkout
  doc AND an HTML-canvas checkout doc (format breadth, D8); `duo pr export` copied
  the doc to `/tmp` then refused an existing dest (the overwrite guard). An
  adversarial review (3 lenses, 22 confirmed — mostly PASS/nit: D6 read-only +
  D4 guard + inert-guarantee all held) yielded 3 minor polish fixes (`2074c8d`):
  the export overwrite guard, a clarified `matchLocalClone` docstring (it opens
  the user's current-branch version by design), and a cosmetic socket re-indent.
