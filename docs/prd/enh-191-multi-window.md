...
**Last updated:** 2026-06-07 (**P2 seams complete + live-smoke PASSED on dev** — registry adoption, `mainWindow` global deleted, gate baseline 0; P3 next; Cut 1 merge + run-on-`main` pending). Update this table at each seam/cut boundary; lessons accrue in Appendix E.
...
**STATUS (2026-06-07):** Phase H + P0 + P1 + **P2 SHIPPED** on `claude/enh-191-multiwindow` (rebased onto `main` @ `13c874d`, v0.9.2). P2 = the `let mainWindow` global **DELETED**; all 41 sends resolve through the `WindowRegistry` (`window-resolve.ts`); **961 tests** green, lint 0, **grep-gate baseline 0**; **adversarial-verify SHIP (0 issues)**; **live-smoke PASSED** (backgrounded-CLI + all 4 visibility exec sites + browser verbs). **You are resuming at P3** — the `event.sender` sweep: un-discard `_event` in the **13** PUSH/STATE handlers, key the **12** caches into `Map<windowId, …>`, validate `event.sender` on the **10** reqId families, add `Session.ownerWindowId` (full scope: §4 P3). The S9 block comment at `setupIPC()` + the live `FilesService.startWatch` handler are the canonical `event.sender` template; `resolveBySender` ships in `window-resolve.ts`. **Before scoping, read Appendix A → "Post-P2 re-baseline"** and RE-GREP every P3 anchor against HEAD — §3/§4 line numbers are spec-authoring-time and have drifted hard (P2 rewrote `main.ts` + the rebase shifted again). Cut 2 = P2+P3, so the version cut waits for P3.
