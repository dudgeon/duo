...
**STATUS (2026-06-07):** Phase H + P0 + P1 **SHIPPED**; **P2 seams complete on dev** (12 commits S1–S11 + S-del — the `let mainWindow` global is DELETED, all 41 sends resolve through the `WindowRegistry` via `window-resolve.ts`; **960 tests** green, **grep-gate baseline 0**). **You are resuming at P3** (the `event.sender` sweep — un-discard `_event` in the 13 PUSH/STATE handlers, key the 12 caches into `Map<windowId, …>`, validate `event.sender` on the 10 reqId families, add `Session.ownerWindowId`; see §4 P3). The S9 block comment at `setupIPC()` is the canonical `event.sender` template for P3. Still owed on P2 before the Cut-2 boundary: the **P2 live-smoke leg** (backgrounded-CLI incl. the visibility cluster + browser-verb parity) and clearing the **adversarial-verify** findings (Appendix E / §4 carry-forward). Re-grep all anchors against HEAD — line numbers in §3/§4 are spec-authoring-time and have drifted again (P2 added the registry/resolver seams).
