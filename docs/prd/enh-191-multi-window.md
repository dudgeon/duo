...
**STATUS (2026-06-07):** Phase H + P0 + P1 have **SHIPPED** on this branch (`claude/enh-191-multiwindow`) — Cut 0 + the Cut 1 seams are complete and P1's **live smoke PASSED**. **You are resuming at P2** (registry adoption + identity-resolve the send sites). Before touching code, read the **Build status table** (top of this PRD) and **Appendix A → "Post-P1 re-baseline"** for the CURRENT branch-HEAD anchors + counts — the line numbers in §3/§4 below are spec-authoring-time and have drifted; re-grep per the rule below.
