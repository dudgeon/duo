...
| Cut 2 | **P2 + P3** — registry adoption (41 sends) + cache/event sweep | 🟡 **P2 seams complete on dev** — all 41 main→renderer sends + the `let mainWindow` global → `WindowRegistry` (registry-of-one). 12 commits (S1–S11 + S-del); **960 tests** green, typecheck + lint clean, **grep-gate baseline 0** (`npm run check:routing`). New files: `window-resolve.ts` (taxonomy resolvers), `once-guard.ts`, `scripts/check-window-routing.sh`. Class-(ii) broadcasts → `registry.all()`; cardinal rule intact. **P2 live-smoke PASSED (2026-06-07, dev build v0.9.1):** backgrounded-CLI (Duo not frontmost) — every read/send/exec resolves the single window, NO focus substitution; all 4 visibility-cluster exec sites ✓ via `resolveDefault` (`dom`/`layout`/`status` + `edit --reveal`); browser verbs (`eval`/`open`/`navigate`/`edit`) ✓ via `liveBrowser`/`liveCdp`. **Flagged, non-P2:** `duo screenshot` (`Page.captureScreenshot`) times out on the restored browser state — shares the working `getCdp` thunk with `eval`, and `main.ts` doesn't touch screenshot, so it's a pre-existing browser-pane CDP-capture issue, NOT a P2 routing regression (needs a clean-session repro). **P3 SEAMS COMPLETE on dev (2026-06-07)** — the `event.sender` sweep: 12 caches → `WindowKeyedCache`, 13 push-handlers un-discard `_event` + key by `event.sender`, 12 reqId families → `PendingRegistry` (sender-validated), PtyManager `ownerWindowId` + owner-routed sends (cold-start fallback) + `disposeForWindow` + owner-filtered `listIdsByCwd`, per-window `ClaudePresenceProbe` (+ idle-park) + `tabsThatHostedClaude`, per-window active-workspace pointer (title/badge, NFR-6.1), SocketServer addressed-cue routing, projects fan-out. 24 commits (M1–M4 + S1–S12); **1045 tests** green (+84 over P2), typecheck + lint clean (0 errors / 59 pre-existing warnings), **grep-gate baseline 0** + self-test fires. 4 new pure modules (`cache-key`/`reqid-validate`/`eventsink-route`/`pty-owner`) import zero Electron. Adversarial-verify: **SHIP, 0 blockers**. Branch **rebased on current `main`** (#74 ENH-203) + force-pushed (origin `189d663`, 0 behind / 58 ahead, 1057 tests, `check:skill-currency` PASS). The Cut-2 smoke-walk + the cut + merges are the SEPARATE merge session's job — not driven from this dev session. |
| Cut 3 | **P4** — `{windows: WindowState[]}` session envelope | 🟡 **In progress** — seams 1–2 done (`session-envelope.ts` v1→v2 migration · `perTabPrune.ts` per-window prune; both pure + pushed). Resume at seam 3 (localStorage triage → `SessionStateService` v2 adoption → tests → active-workspace persistence). Plan + **EXECUTE-CONTINUOUSLY directive**: `docs/dev/enh-191-p4-seam-plan.md`. |
| Cut 4a / 4b | **P5a / P5b** — open window 2 + terminal-origin + `--window` | ✅ **DONE on branch (2026-06-07)** — P5a (window-opening + interaction crashers + app-menu focus-pointer + workspace windowId-threading + N-window restore w/ id-reconciliation + NFR-6.2 blank-pin + Tier-4) **AND** the P5b CLI addressing (`DUO_WINDOW` stamp consumed + global `--window N` override + `duo windows` enumeration + `duo doctor` Windows:N) all landed. The S4 resolver core is `registry.primary()` (lowest-id, non-throwing) retiring the `only()` placeholder; `getFocusedWindow` stays 0 (focus tracked via the `browser-window-focus` event). **10 commits `ebf8d68`..`910293c`; 1093 tests, typecheck clean, routing baseline 0, check:skill-currency 67 verbs.** Live-verified via `duo` probes + a 2-window `/smoke-walk` v0.9.3 (**8/8 PASS**, owner-walked). **PR submitted** (branch → main). |

**Last updated:** 2026-06-07 (**P5a Tier 2-4 + S4 + P5b addressing DONE on branch + 8/8 smoke-walk PASS → PR submitted**; window 2 is fully functional + CLI-addressable. NEXT: merge + a `cut-version` flow — roadmap/CHANGELOG/RELEASES/faq + DMG are cut-time, owned by a separate session). Update this table at each seam/cut boundary; lessons accrue in Appendix E.
...
7. **Update the P0 teardown harness** to assert `teardownWindow` fires `cdp.detach` + `browser.dispose` exactly once per window (idempotent on re-close), and `teardownApp` fires `socket.stop` + `external.dispose` exactly once **at `before-quit` ONLY** — with an explicit assertion that a window close does NOT stop app services (dock-reopen safe), plus negative controls for double-stop and missed-dispose.

**Exit criteria.** App boots one window with `SocketServer`/`ExternalDomainsService` constructed at app scope; closing the only window does NOT stop the socket (per-window teardown only — dispose that window's `browserManager`/`cdpBridge`); the socket + external-domains are torn down ONLY at `before-quit`, so the CLI bridge stays UP for a dock-reopen (Option A); Cmd+Q crash-free; `duo doctor` shows socket DOWN after quit; full single-window smoke-checklist passes **unchanged**; teardown-once harness green. Then **Cut 1 (P0+P1):** `/smoke-walk` via Skill tool (Leg B walked + full single-window checklist; wait for pasted results) → `/cut-version`.
...
6. **Identity-resolve the visibility cluster** (**4** `executeJavaScript` sites on this branch — re-grep; ~`:3245`/`:3328`/`:3345`/`:3360`; the count rose 3→4 since v2, see the Appendix A re-baseline grep-gate row. `openDevToolsForTarget` is resolve-by-identity but is NOT one of the 4 exec sites). These are live `executeJavaScript` round-trips — **structurally untestable by the harness** — so they MUST resolve by `registry.only()` (NOT focus) and be enumerated in the P2 smoke-parity leg. *(CLAUDE.md tells agents to reach for `duo dom`/`duo eval`/`duo layout`/`duo devtools` FIRST when debugging blind, so a wrong-window answer actively misleads — highest-consequence focus-substitution member.)*
...
**P2 → P3/P5a carry-forward (discovered during the P2 registry adoption, 2026-06-07).** All resolve `registry.only()` at N=1 (byte-identical); each is a *known deferral surfaced by the adversarial taxonomy pass*, NOT a P2 bug:
- **§3.3 storage-bus listener gap (theme / author / claude-key-prefs).** These global prefs persist to localStorage on receive but register NO `window` `'storage'` listener (only `useWorkspacePillMenuFlag` / `useSendPillFlags` / `autosavePreference` do). So their cross-window propagation is **not actually wired** — at N>1 a `duo theme` / `author` / `claude-return` flip won't reach other windows until reload. Classification stays (i) (no main fan-out); **P5a owes a `'storage'` listener** on `useTheme` / `useAuthor` / `useClaudeKeyPrefs` (or promotes those SETs to fan-out).
- **`PINS_TOGGLE` emits no broadcast.** `pins.json` (Stage 24 pinned WorkingPane tabs) is a shared identity file, but its toggle handler returns via the invoke reply only — no `PINS_CHANGED` channel exists. At N>1 a pin toggled in window A leaves window B's pinned rail stale until reload. Fixing needs a NEW `PINS_CHANGED` channel + invoke change (full CLI-plumbing), out of the 41-send P2 scope → **P5a** (peer to the `registry.all()` adoption).
- **`EXTERNAL_REDIRECTED` is class-(ii) by fiat, not derivation.** Its trigger is an action+read (`shell.openExternal` + read-only `externalDomainsService.match`), NOT a shared-file write, and the renderer consumer is an ephemeral toast (no rail repaint). At N>1 it's a genuine design choice — broadcast the receipt to all windows (current `broadcastAll`) vs address only the firing window. Two sibling `EXTERNAL_REDIRECTED` sends in `browser-manager.ts` (~`:892`/`:918`) must adopt the SAME N>1 resolution. **P5a decision.**
- **`WORKSPACE_FILE_ACTIVE_CHANGED` is a per-window titlebar push over a shared file.** `active-workspace.json` IS a shared on-disk pointer (the tripwire that forces scrutiny), but the send is a titlebar-badge push decoupled from `activeWorkspaceService.set()`, and active-workspace is NOT one of the four locked rail-identity files. Stays (i) at N=1. **P5a must consciously decide:** each window owns its active workspace (stays per-window / `event.sender`) vs one global active workspace (a NEW titlebar broadcast — distinct from the locked (ii) rail set; do not silently bucket as plain (i)).
- **`registry.only()` is fail-loud at N>1 by design.** The ~30 BROWSER_* IPC handlers + the 12 reqId dispatch sends + the {ok} CLI helpers resolve `registry.only()` (which THROWS at N>1). At N=1 that's the sole window; at P5a these become `event.sender`-addressed (P3 sweep for caches/reqId; the BROWSER_* handlers in P3/P5b). The throw is the **intended signal** that a site still needs per-window routing — never a silent wrong-window send.
...
**Flag: `multiWindow.enabled` — ⚠️ OWNER DECISION 2026-06-07: default `TRUE`** (overrides the original "default `false` until Cut 4a ships green"). The owner wants multi-window ON by default; users can disable it via a **Settings menu** toggle ("I don't expect users to need to access it"). **Implication:** default-on means the capability is LIVE the moment P5a merges, so the two-window `/smoke-walk` gate (owned by the separate merge/cut session) is the only thing between it and users — it MUST pass before Cut 4a ships. The runtime kill-switch (toggle off → verified registry-of-one) is the safety net. The behavior below (what the flag gates) is unchanged; only the default flips.
...
**Flag storage (avoid the sidecar anti-pattern, CLAUDE.md §12).** ENH-191 P5a S1 implements this as a main-side `SettingsService` over `~/.claude/duo/settings.json` (`{ multiWindow: boolean }`, default `true`) — Duo's per-feature-file convention, atomic write, read once at boot. NOT a shared-origin localStorage key on the storage-event bus (that would make the flag itself a per-window-divergence hazard — the C13 class). The Settings-menu toggle calls `SettingsService.set`; the renderer learns the value via one-shot IPC, not the storage broadcast. **New Window shortcut (owner decision 2026-06-07): `⌥⌘N`** (`⌘N` = New File, `⌘⇧N` = New Folder are already bound).
...
> ⛔ **RESUME → P4, and EXECUTE CONTINUOUSLY.** Read `docs/dev/enh-191-p4-seam-plan.md` FIRST — it has the resume directive + the 6 P4 seams (1–2 done, resume at seam 3). The owner's standing direction is "keep pushing multi-window forward (P4/P5)": build seam-by-seam, gate, commit + push EACH, and KEEP GOING. **Do NOT stop to report milestones, ask "shall I continue?", offer sequencing options, or re-spin smoke-walks / version-cuts / merge proposals** — the owner has repeatedly called mid-plan checkpoints out as ignoring a clear direction. Merges + cuts are a SEPARATE session's job. Build P4 → P5.

**STATUS (2026-06-07):** Phase H + P0 + P1 + **P2 SHIPPED** + **P3 SEAMS COMPLETE** on `claude/enh-191-multiwindow` (rebased onto `main` @ `13c874d`, v0.9.2). P3 = the `event.sender` sweep — the registry-of-one READ-model, byte-identical at N=1: **12 caches** → `WindowKeyedCache` (`cache-key.ts`); **13 PUSH/STATE handlers** un-discard `_event` + key by `BrowserWindow.fromWebContents(event.sender)?.id`; **12 reqId families** (NOT 10 — the PRD-era list omitted `docEditPlainPending` + `jsonOpPending`, both verified) → `PendingRegistry` (`reqid-validate.ts`, sender-validated replies); `Session.ownerWindowId` + dormant `DUO_WINDOW` + owner-routed PTY sends (`pty-owner.ts`, **cold-start fallback**) + `disposeForWindow` (scopes the workspace-swap dispose) + owner-filtered `listIdsByCwd` (C9); per-window `ClaudePresenceProbe` (+ NFR-1.3 idle-park) + per-window `tabsThatHostedClaude`; per-window active-workspace pointer drives `applyWindowTitle(ctx)` + the badge (NFR-6.1); SocketServer addressed-cue routing (`eventsink-route.ts`); projects fan-out (item 12). 24 commits (M1–M4 + S1–S12); **1045 tests** green (+84), typecheck + lint 0-errors (59 pre-existing warnings), **grep-gate baseline 0** + self-test fires; the 4 new pure modules import zero Electron. P3 adversarial-verify: **SHIP, 0 blockers**. **Branch REBASED onto current `main`** (incl. #74 ENH-203) + force-pushed — origin HEAD `189d663`, 0 behind / 58 ahead, **1057 tests**, `check:skill-currency` PASS. **NOW IN P4** — the persistence hard gate (C8+C13 data-corruption): seams **1–2 DONE** (`session-envelope.ts` v1→v2 migration · `perTabPrune.ts` per-window prune, both pure + pushed); **resume at seam 3** (localStorage triage → `SessionStateService` v2 adoption → tests → active-workspace persistence). Full P4 plan + anchors + the EXECUTE-CONTINUOUSLY resume directive: `docs/dev/enh-191-p4-seam-plan.md`. **Known P5 carry-forward:** WindowKeyedCache-slot teardown on window close (non-N=1-blocking). The Cut-2 smoke-walk + any version cut + merges are NOT this session's job (a separate merge session owns them); do not drive them.

You are converting Duo from single-window to N-independent-windows. The whole effort is **one de-risking sequence of zero-user-visible-change releases (P0→P4) culminating in a single user-visible release (P5)** — **not** a product ladder. The scary core (replacing `let mainWindow` (`electron/main.ts:265` on this branch; a ~143-line `mainWindow` surface — a LINE count, not edits) with a window registry) lands *behind exactly one window* and is **provably byte-identical** because `registry.only()` returns the same sole window `mainWindow` is today. The proof obligation is the node-env routing-assertion harness (§5.1, modeled on `electron/safe-send.test.ts`) plus the unchanged single-window smoke-checklist with two added leak-hunting steps. **The single most important rule: resolve every default send target by IDENTITY (`registry.only()` for app-wide; `BrowserWindow.fromWebContents(event.sender)` for push/reply), NEVER by `getFocusedWindow()`** — a focus-based "fix" silently drops backgrounded sends and is invisible to interactive testing because the one window is usually focused.
...
- [ ] the export-helper reads in the **49-line** guard cluster (re-grep — was ~30 in v1, 46 at v2) + push/reply dispatch resolve by identity (`registry.only()` / `event.sender`).
...
| guard cluster `if (!mainWindow \|\| mainWindow.isDestroyed())` | 46 (v2) | **49** | +3 since v2 (P1 churn); the dominant read cluster P2 item 4 refactors as one unit |
| visibility `executeJavaScript` sites (P2 item 6) | 3 (v2) | **4** | ~:3245/:3328/:3345/:3360 — one drifted in since v2 |
...
### Post-P2 re-baseline (2026-06-07 — `claude/enh-191-multiwindow` after P2, rebased onto `main` @ `13c874d`, v0.9.2)

**P2 shipped + the `let mainWindow` global is DELETED.** The entire Post-P1 anchor table above is now **historical** — there is no `mainWindow` global; `createWindow` builds the sole window as a local `const` and registers a `WindowContext`. Every main→renderer path resolves through the registry, so **the grep-gate baseline is now 0** (`grep -cE 'mainWindow\??\.webContents\.(send|executeJavaScript)' electron/main.ts` = **0**).

| Post-P2 fact | Value |
|---|---|
| `let mainWindow` global | **deleted** (was :265) — now a `createWindow`-local `const mainWindow` (~:598) |
| grep-gate baseline (was 41) | **0** — all 41 sends routed via `safeSend` / `resolveDefault` / `broadcastAll` |
| resolution accessors (main.ts) | `resolveDefault(registry)` (class-i sends) · `broadcastAll(registry, …)` (the 3 class-ii broadcasts) · `liveMainWindow()` (dialog/title/focus/devtools/once/reload/reqId — BrowserWindow cast) · `liveBrowser()` / `liveCdp()` (per-window managers) |
| `getFocusedWebContents` / `getFocusedWindow` | **1** (allowlisted Copy-as-Plain-Text) / **0** — cardinal rule holds |
| new files | `electron/window-resolve.ts` (+test) · `electron/once-guard.ts` (+test) · `scripts/check-window-routing.sh` (+`__fixtures__/routing-violation.ts.txt`) |
| tests | **961** green (incl. main's ENH-195/202 post-rebase) |

**For P3 (the `event.sender` sweep):** the §4 P3 anchors (the **13** PUSH/STATE handlers, **12** caches, **10** reqId families, the EventSink/`Session` lines) are spec-authoring-time (v0.8.x) and have drifted HARD — P2 rewrote `main.ts` (~+250 lines of registry/resolver/guard churn) AND the rebase onto `main` shifted again. **Re-grep every P3 anchor against HEAD before scoping** (iterate the named channel/cache literal lists in §4 P3, never the line numbers). The canonical `event.sender` template is the S9 block comment at `setupIPC()` + the live `FilesService.startWatch` (`FILES_WATCH_START`) handler; the pure `resolveBySender(registry, BrowserWindow.fromWebContents(event.sender)?.id)` helper already ships in `window-resolve.ts`.

### Post-P3 re-baseline (2026-06-07 — `claude/enh-191-multiwindow` after P3, the `event.sender` sweep)

**P3 SEAMS COMPLETE.** The whole main-process read-model is now registry-of-one per-window-routable, byte-identical at N=1. Anchors for P4/P5 (re-grep before depending on a line number):

| Post-P3 fact | Value |
|---|---|
| read-model caches | 12 → `WindowKeyedCache<T>` (`electron/cache-key.ts`); writers `cache.set(BrowserWindow.fromWebContents(event.sender)?.id, …)`; readers `cache.getDefault(registry)` (= `getOrDefault(only().id)`) |
| reqId families | **12** (not the PRD-era 10 — `+docEditPlainPending` `+jsonOpPending`) → `PendingRegistry<R>` (`electron/reqid-validate.ts`); dispatch `set(reqId, win.id, cb)`, reply `deliver(reqId, fromWebContents(event.sender)?.id, res)` |
| PTY | `Session.ownerWindowId` + dormant `DUO_WINDOW` stamp; `setOwnerRouting({resolveOwner, resolveDefault})` → owner-routed sends w/ cold-start fallback (logic in `core/pty-owner.ts`); `disposeForWindow(id)`; `listIdsByCwd(cwd, ownerWindowId?)` |
| per-window `WindowContext` fields | `activeWorkspace?` (title/badge, NFR-6.1) · `presence?` (ClaudePresenceProbe) · `tabsThatHostedClaude?` (S3 gate) — all on `electron/window-registry.ts` |
| new pure modules (zero Electron) | `cache-key.ts` · `reqid-validate.ts` · `eventsink-route.ts` · `core/pty-owner.ts` (+ each `.test.ts`) |
| SocketServer cues | `setEventSink(send, getAddressedWindowId?)`; both ambient emits pass the 3rd addressed-window-id arg → `routeAmbientCue` |
| tests / gate | **1045** green (+84); `check:routing` baseline 0 + self-test; lint 0-errors |
| module globals DELETED | `claudePresence`, `tabsThatHostedClaude` (now per-window `WindowContext` fields); the 12 cache `let`s + 12 reqId `Map`s (now keyed) |

**P5 carry-forwards surfaced by P3:** (1) `WindowKeyedCache`-slot teardown on window close (non-N=1-blocking — a closed window's slots linger unread since `only()` is undefined post-close; wire `delete(id)` for all caches + clear the `WindowContext` fields into `registry.unregister`/closed at P5); (2) the `getAddressedWindowId` / enrich-hook owner / `applyNewSessionState` dispose / workspace `set`+`clear` sites resolve via `registry.only()` (the fail-loud N>1 placeholder) — P5 threads the real per-command/per-window id; (3) the `duo send` read sites + `workspaceCurrent` stay `only()`-resolved (CLI default-resolution is P5a's `DUO_WINDOW` consumer).

**Adversarial-verify (2026-06-07, 7 skeptics, HEAD `f390e78`): SHIP-WITH-FOLLOWUPS, 0 blockers.** Independently re-ran the gates (1045 tests / typecheck / `check:routing`) + mutation-tested the negative-control battery RED-on-injection; the N=1 byte-identity contract holds; the only "majors" are the DELIBERATE N>1 fail-loud (`only()` throws — covered by carry-forward (2)). **Minor/nit followups (non-blocking, P5 or optional polish):** (a) the `describe.each` PUSH_FAMILIES(13)/REQID_FAMILIES(12) arrays are hand-maintained + length-asserted (verified complete today; a future omitted family wouldn't auto-RED — consider deriving from a shared registry); (b) `applyWindowTitle` sends the badge via raw `win.webContents.send`+try/catch rather than `ctx.safeSend` (byte-identical; `ctx.safeSend` is unpopulated in P2, so the raw send is the correct choice today); (c) `disposeForWindow` re-resolves `liveMainWindow()` rather than reusing the swap path's `win` (byte-identical nit); (d) `TERMINAL_ACTIVE_PUSH`→`ctx.presence` + `applyWindowTitle(ctx)` lack dedicated unit tests (main.ts is Electron-bound — P5 integration coverage closes it).
...
**Process / workflow**
- **The Bash tool's cwd is the PRIMARY checkout, NOT the worktree — and a `cd` does not persist across calls.** During P2, gate commands (`npm run typecheck` / `test:run`) silently ran against `main` (whose `electron/main.ts` had diverged ~395 lines) until vitest's printed `RUN … <root>` line exposed it. `Write`/`Read`/`Edit` use absolute worktree paths (correct), but bare `npm`/`npx`/`grep` resolve from the primary checkout. **Fix:** `cd <worktree> && …` for npm/npx; absolute paths for grep; and confirm a tool's printed root points at the worktree before trusting a green gate.
...
**Technical**
- **The `if (!mainWindow || mainWindow.isDestroyed())` guard is shared across {ok} send helpers, non-send dialog functions, AND reqId dispatchers — a blind guard-`replace_all` is wrong.** P2 split the conversion: the send seam (S6) converted only the SENDS (leaving guards on the still-present global), then the global-deletion seam (S-del) converted the guards + the BrowserWindow-method reads (dialog/setTitle/focus/once/reload) via a `liveMainWindow()` cast accessor, typecheck-driven. Separating "convert sends" from "delete the global" kept each commit byte-identical + reviewable. The grep-gate regex widened to `mainWindow[?!]?\.` — the `!.` form is the `mainWindow!.webContents.send` reqId dispatch (12 sites) that the v2 spec's `\??` regex would have missed.
