...
**2026-07-09 (ENH-266d) — D15 extended: templates-folder exclusion for type-filtered queries.** Live-tested in real Obsidian 1.12.7: a `type == "X"` filter with no `templates/` exclusion also matches `templates/X.md` (the schema template carries the same default `type:`), rendering a phantom row when the `.base` is opened **natively in Obsidian** — Duo's own `duo rollup render`/`duo base render` never show this (the corpus reader excludes `templates/` unconditionally per P1/§3). `duo base lint`'s D15 vocabulary (§ "Lint (L1)" above) now also flags this case: for each view's EFFECTIVE filter (top-level `filters:` AND the view's own `filters:`, matching the actual render combination), a `type == "X"` filter with no `file.inFolder("templates")` exclusion anywhere in that tree gets a `warn` finding — advisory only, per D15's warn-and-render contract, unchanged. `core/vault/scaffold.ts`'s Obsidian-mode initiative template (the `initiative == this` milestone rollup, § 3/P3) now carries the exclusion explicitly rather than relying incidentally on `initiative == this` alone.

**2026-07-09 (ENH-266e) — alias auto-seed on OKF entity stubs.** D6's slug/title split (§ "OKF vs Obsidian" glossary) means vanilla Obsidian's file explorer/quick-switcher/search/link-autocomplete only ever show the slug filename, never `title:`. `createEntityStub` (§ P4's silent-stub path) now auto-seeds `aliases:` with the human title whenever it differs from the final on-disk stem, so those surfaces show the readable name too. Obsidian-mode stubs are unaffected (the filename already IS the title). Full write-up: `skill/references/vault-guide.html` ch. 12 ("OKF in real Obsidian").
