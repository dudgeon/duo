...
> **Status (2026-05-17 evening, post-compaction round 4):** All four 🟡 v0.7.0 decision gates closed (BUG-125 v2, FOLLOWUP-025 v2, ENH-159 v2, GH-cluster v2+prototype+occlusion-fix). All five walk-revs (rev1 → rev5) walked. **Rev5 walk-in-progress** — owner picked the modified-B chip-occlusion fix mid-walk; round-2 shipped same session; round-3 (post-compaction) moved popover into a `createPortal` + `position: fixed`; **round-4** repositioned chip below the row (left-aligned with row content) per owner pick — visually tied to its folder, extends rightward into the working pane as needed. Final re-walk owed before cut.

---

## 🔥 Post-compaction me: read this first

1. **Walk state:** rev5 is the active walk. Owner walked it through item 1 (GH-CLUSTER-PHASE-2), discovered the chip-occlusion issue, picked modified-Option-B as the fix, and we shipped round-2. Round-3 portaled the popover to escape overflow clipping. Round-4 (current) repositioned the chip **below the row, left-aligned with row content** per owner pick — visually tied to its folder, no horizontal clamping needed. Hover the small `⎇` icon — chip appears just under the row and extends rightward into the working pane as needed. Next session: confirm round-4 PASS, then either close rev5 or generate a rev6 walk that includes just the round-4 icon/popover verification.
