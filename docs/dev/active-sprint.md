...
> **Status (2026-05-17 evening, post-compaction round 5 + BUG-132 rev2):** All four 🟡 v0.7.0 decision gates closed. **Rev5 walk PASSED** (owner: "mark it a pass") after BUG-132 rev1+rev2 fix — peer-repo "Open on GitHub" no-op was rooted in two bugs stacked: (a) `files.openExternal` was actually `shell.openPath` (silently fails on URLs), fixed rev1 with new `FILES_OPEN_EXTERNAL_URL` IPC; (b) `gitSnap` climbed to outer non-GitHub repo when navigator at parent-of-peer-repos, fixed rev2 by preferring `childRepoMap`'s peer-repo snapshot. All 5 walk-revs walked + closed. **Cut ready.**

---

## 🔥 Post-compaction me: read this first

1. **Walk state:** **rev5 PASSED 2026-05-17 evening.** GH-CLUSTER-PHASE-2 item PASS after BUG-132 rev1+rev2 fix (peer-repo right-click "Open on GitHub" was silently failing through two stacked bugs — wrong IPC + wrong gitSnap). Two SKIPs intentional (POST-CUT + enterprise-machine). **Cut is unblocked.**
