...
**Status:** 🟢 Locked (2026-06-05, owner — audit-confirmed). ✅ Shipped through P5b in v0.10.0 (2026-06-08, PRs #73 + #78) — a real second window. Per-phase detail in the PRD.
...
**Decision.** Convert Duo from single-window to **N independent windows**, each its own workspace (focus, navigator cwd, `.duo-workspace`, pins, terminals, browser pane, geometry — all restored across relaunches in ascending-id order), under these locked invariants:

- **Registry-of-one spine.** Replace the single `let mainWindow` global with a `WindowRegistry` (`Map<windowId, WindowContext>`). At N=1 `only()` returns the same sole window, so the refactor landed **behind one window, byte-identical**, across the zero-user-visible cuts (P0–P4) before the second window opened (P5). Duo stays internally byte-identical at N=1 vs the prior single-window behavior.
- **Identity, not focus.** Every default-target send resolves by IDENTITY — `registry.only()` (app-wide) / `event.sender` (per-window) / `registry.all()` (shared-state broadcast) — **never** `getFocusedWindow()`, which silently drops backgrounded sends. The default app-level resolution is the deterministic lowest-id "primary" window; app-*menu* commands resolve the FOCUSED window, and renderer IPC resolves by `event.sender` (so window 2's actions hit window 2). This is **mechanically enforced**: the `check:routing` grep-gate (`scripts/check-window-routing.sh`, `npm run check:routing`) drives `mainWindow.webContents.send`/`executeJavaScript` to ZERO in `electron/main.ts` — every send routes through `safeSend` (`electron/safe-send.ts`) / `resolveDefault` / `broadcastAll` (`electron/window-resolve.ts`) — and bans `getFocusedWindow` outright for default resolution (the lone `getFocusedWebContents`, the "Copy as Plain Text" menu item, is allowlisted). A negative-control fixture + `--selftest` prove the gate fires.
- **Single Unix socket** (locked). One socket; the CLI addresses any window via a `DUO_WINDOW` terminal-origin PTY stamp + a global `--window N` / `--window=N` flag (parsed + stripped before verb dispatch) + a `duo windows` verb (JSON `[{id, primary, focused, activeWorkspace}]`). `duo doctor` reports the live window count. An unknown/stale `--window` / `DUO_WINDOW` id falls back to the PRIMARY (lowest-id) window, validated against the live registry.
- **Session is a versioned envelope, lossy-downgrade accepted.** The on-disk session moved from a flat v1 document to `{ version: 2, windows: [...] }`. Every persisted window is seeded into the in-memory model **before the first save** (so a single-window save can't clobber an N-window file), and a one-time `session-state.json.v1.bak` is written before the first v2 write. Forward migration (old → new) is lossless. **Accepted downgrade cost:** an older Duo reading a v2 file boots an EMPTY session GRACEFULLY — no crash, no corruption, no pref or `.duo-workspace` loss; a v0.10.0 session simply isn't restored by an older build. This is the inherent, owner-accepted price of the format bump.
- **"Allow Multiple Windows" defaults ON.** A `multiWindow` setting (`SettingsService`, `DEFAULT_SETTINGS.multiWindow: true` — owner decision 2026-06-07) gates ONLY window-opening. When OFF: the New Window menu item is disabled and `duo window new` exits non-zero with a clean disabled error; on relaunch only the first window restores (the rest stay dormant in the session file — re-enabling brings them back, never lost). An invalid/missing setting value falls back to the ON default.
- **A new window opens BLANK (NFR-6.2).** A second window does NOT clone window 1's pinned file tabs — it starts with an empty workspace. Open via `File → New Window` (⌥⌘N) or `duo window new`.
...
- **Sequence.** Six cuts — Cut 0 = Phase H (write-serialization, shipped v0.9.1), Cut 1 = P0+P1, Cut 2 = P2+P3, Cut 3 = P4 (session envelope), Cut 4a/4b = P5a/P5b (the only user-visible cuts, **shipped v0.10.0**). **No shippable focused-window-CLI midpoint.**
