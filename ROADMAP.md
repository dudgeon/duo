...
| **21** | **Distribution polish** (was 14b; code sign + notarize + auto-update + session restore + browser history + fork-friendly architecture; cross-refs issues #24, #27) | 🟡 **21a ✅ shipped 2026-04-27 (signed + notarized DMG).** v0.4.1 GH release re-uploaded with signed DMGs (Apple `Developer ID Application: Geoffrey Dudgeon (R39EF29X3Y)`; hardened runtime; Notarization Ticket stapled; `spctl` accepts as `Notarized Developer ID`). Toolchain in `scripts/dist-signed.sh` + `cut-version` skill Step 4.5 + `cert-procurement.md` Sequoia compatibility appendix. Resolution artifact: `docs/dev/intent-conversations/2026-04-27-stage-21-signing.md`. Toolchain commits on main: `4ffde29` + `955f959`. **The actual blocker** turned out to be iCloud File Provider tagging directories under `~/Documents/`, not `com.apple.provenance` as the wip exploration suggested — fix is `electron-builder -c.directories.output=$HOME/.cache/duo-build` so the build happens outside iCloud-touched filesystem. **21b/c/d/e still ⬜:** **21b** — custom app icon + DMG background, light/dark theme refinements; **21c** — `electron-updater` integration + GH-Releases auto-update channel, session restore on relaunch (issue #24), browser history persistence for autocomplete (issue #27); **21d** — socket auth token for Trailblazers cohort, agent-driven-navigation notifications, Trailblazers README + install guide; **21e** — fork-friendly architecture (`fork.config.json` driving electron-builder + bundle ID + update endpoints + bootstrap defaults, runtime config injection, provenance-aware install with conflict detection, fork docs). 21e enables enterprise downstream forks; layered fork modes documented in [`docs/HOW-TO-FORK.md`](docs/HOW-TO-FORK.md). | **L4** |
