...
| **21** | **Distribution polish** (was 14b; code sign + notarize + auto-update + session restore + browser history + fork-friendly architecture; cross-refs issues #24, #27) | 🟡 **21a + 21c Phase 1+2 shipped.** **21a ✅ (v0.4.1)** signed + notarized DMG; toolchain in `scripts/dist-signed.sh`, Sequoia compatibility appendix in `cert-procurement.md`. **21c Phase 1 ✅ (v0.4.2)** electron-updater integration — background download + native "Restart Duo to install update?" dialog + auto-install-on-quit. **21c Phase 2 ✅ (v0.4.2)** session restore on relaunch — terminals, file tabs, browser tabs persist via `~/.claude/duo/session-state.json`; closes issue #24. **21b/c-Phase3/d/e still ⬜:** **21b** — custom app icon + DMG background; **21c Phase 3** — browser history persistence (issue #27); **21d** — socket auth token, agent-driven-navigation notifications, Trailblazers README; **21e** — fork-friendly architecture (`fork.config.json`, runtime config injection, provenance-aware install with conflict detection); v0.5.0 target with 21e implementation in flight on `stage-21e-fork-friendly` branch. Layered fork modes documented in [`docs/HOW-TO-FORK.md`](docs/HOW-TO-FORK.md). | **L4** |
