...
Layer 2 — New surfaces (built against Layer 1)
   ...
   17b–e HTML canvas — IDs/sidecar, agent overlay, comments, polish   ← next
...
| **17** | **HTML canvas** (was Stage 19; new WorkingPane tab type for `.html`) | 🟡 **17a shipped 2026-04-26** — render + edit primitive: `html-canvas` tab type, iframe-srcdoc + contentEditable + MutationObserver autosave, Stage 11-style toolbar (B/I/U/S/code + ⌘K), own selection-aware mark applicator (no `execCommand`), `duo html new` + `duo edit/view <path.html>` routes. ⌘N's H6.1 question dissolved into extension-based dispatch (one shortcut, one interstitial). 17b–e queued. **17a.5 design exploration filed 2026-04-26 night** — five directions (starter gallery / slash-menu blocks / AI-drafted-on-prompt / smart-blank onboarding / user-template registry as cross-cut). **Owner committed direction F (markdown shortcuts on typing) same night** — `# ` → H1, `**bold**` → bold, etc. — keydown listener + targeted DOM rewrites at the caret (no ProseMirror per PRD H3). A–E remain on the books as additive. Card: [`docs/roadmap.html#s17a5-design`](docs/roadmap.html). **17a polish & parity card** filed same night during owner walk: (1) markdown shortcuts on typing [committed], (2) `+` tab-strip button → file interstitial (currently opens a dead browser tab — see `WorkingTabStrip.tsx:46` + `WorkingPane.handleNew`), (3) **literal toolbar component reuse** [committed] — refactor `EditorToolbar.tsx` into a presentational component taking an `EditorActions` interface; both surfaces inject implementations (MarkdownEditor wires TipTap; CanvasTab wires DOM-mutation primitives). PRD H28's divergent slash-menu approach kept as additive (17e), not as a replacement, (4) canvas-side `EditorActions` impl in new `blockOps.ts` sibling to `inlineMarks.ts`, (5) **native execCommand undo/redo** wired (no custom snapshot stack — native covers contentEditable + execCommand-backed mutations for free; PRD §8 was scoped to marks, not blocks), (6) contextual table strip + table operations (canvas side), (7) placeholder text on fresh + empty pages. **All 7 shipped 2026-04-26 night** — typecheck clean; HMR clean; CLI smoke confirmed MD editor read + selection cache + heading-trail intact post-refactor. Owner partial verification: "+" works as expected; toolbar looks good (not fully tested). 10-item in-depth verification punch list (V1–V10) filed in the polish & parity card for a future smoke session — V2 (MD toolbar regression) is the highest-risk because the refactor went through every button. Card: [`docs/roadmap.html#s17a-polish`](docs/roadmap.html). PRD: [docs/prd/stage-17-html-canvas.md](docs/prd/stage-17-html-canvas.md). | **L2** |
