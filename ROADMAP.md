...
## Owner pre-work (cert procurement) — ✅ complete 2026-04-25/26 · Stage 21 ✅ shipped 2026-04-27

All five required artifacts for Stage 21 (signed + notarized DMG
distribution) are in place, **and Stage 21 itself shipped 2026-04-27**
— the v0.4.1 GH release ships signed + notarized DMGs; Gatekeeper
accepts as `Notarized Developer ID`. Tracker: `docs/dev/cert-procurement.md`
(now with Sequoia compatibility appendix).
...
**Stage 21 (the 21a "mechanical core") shipped 2026-04-27.** The yml
stays env-agnostic — `mac.identity` and `mac.notarize` remained
commented; `dmg.sign: false` stayed as-is. electron-builder
auto-discovers the cert from `CSC_NAME` and the notarization flow from
the `APPLE_API_*` env vars without yml flips. The signed cut runs via
`bash scripts/dist-signed.sh` (sources `.env`, strips the `Developer
ID Application: ` prefix off `CSC_NAME`, builds to
`$HOME/.cache/duo-build` to dodge iCloud File Provider xattrs on
`~/Documents/`, copies signed DMGs back to `dist/`, runs
`scripts/validate-signed-dmg.sh`). End-to-end ~5–8 min on M1.
**21b/c/d still ⬜** (app icon, electron-updater, session restore,
socket auth token, Trailblazers README) — see Stage 21 stage card.
...
Layer 2 — New surfaces (built against Layer 1)
   17a   HTML canvas — render + edit primitive                        ✓ shipped
   17a.5 Smart-blank overlay (direction D)                            ✓ shipped (visual smoke owed)
   17a polish & parity — items 1-7 (markdown shortcuts + shared toolbar)  ✓ shipped (V1-V10 verification owed)
...
Layer 4 — Distribution finalization (cert pre-work ✓; L3 to stabilize)
   ...
   25    Post-redirect chrome banner                                   ✓ shipped v0.4.0
...
| **12** | **Visual redesign — Atelier** (system-wide token swap, light-as-hero, layout depth, tab-strip rhyme, files-pane width 208 + collapse-to-rail) | ✅ **Shipped v0.5.3 (2026-04-30).** Phases 1–3 (Atelier tokens + tab-strip rhyme + cozy mode) shipped 2026-04-26. Whisper-level agent presence (titlebar dot + selection glow) shipped 2026-04-30: `ClaudePresenceDot` next to ThemeToggle pulses on Claude-live; `data-claude-reading="1"` flashes the working-pane wrapper for 1.2s when `duo selection` fires (selection-anchored CHR-driven glow filed as a future polish item). | **L0** |
...
| **15** | **Send → Duo** (floating pill + `duo send` + `duo selection-format` CLI) | ✅ **Shipped v0.5.3 (2026-04-30).** 15.1 + 15.2 shipped 2026-04-26 (editor pill + CLI verbs; browser-pane CDP observer). 15.3 polish trio shipped 2026-04-30: ⌘D global chord routes via CustomEvent to whichever surface has a cached selection; 5000-char length cap with `… [truncated; N total]` marker so the agent knows to call `duo selection` for the full text; canvas image-flatten replaces `<img>` with `[image: alt-or-filename]` placeholders so embedded images don't disappear silently. **2026-05-01 walk fail traced to ENH-013 (pill suppresses when no Claude session is live), not a 15.3 regression** — user's terminal showed `[process exited]` which forces `onSendToDuo = null` per design. Stage 15 stays ✅. PRD: [docs/prd/stage-15-send-to-duo.md](docs/prd/stage-15-send-to-duo.md). | **L1** |
...
| **17** | **HTML canvas** (was Stage 19; new WorkingPane tab type for `.html`) | 🟡 **17a + 17a polish + 17b + 17a.5 D + 17c + 17d-A shipped 2026-04-26.** **17a** (render + edit primitive): `html-canvas` tab type, iframe-srcdoc + contentEditable + MutationObserver autosave, own selection-aware mark applicator, `duo html new` + `duo edit/view <path.html>` routes; ⌘N H6.1 dissolved into extension-based dispatch. **17a polish & parity items 1-7**: markdown shortcuts on typing (direction F), `+` tab-strip → file interstitial, literal toolbar component reuse (`EditorActions` interface; both surfaces wire their own impls), canvas blockOps + tableOps + placeholder. **17a.5 design exploration**: five directions filed; F (markdown shortcuts) committed; D (smart-blank overlay) shipped (visual smoke owed). **17b** (Phases A–D): ULID injection + first-open prompt with per-directory choice persistence (PRD H12–H14), `<file>.duo.json` sidecar with `version: 1` + `recentEdits` capped at 50 (H22), all seven `duo html *` agent verbs end-to-end via single `html-op` socket cmd + new `IPC.CANVAS_HTML_OP[_RESULT]` channels (H37, H38), pretty-printed serializer with stable attr order + runtime-chrome strip (H34). **17c** (agent overlay + selection): canvas-side just-added wash binding via `installJustAddedStyles(doc)` injecting the keyframe + class into the iframe stylesheet (PRD H20), serializer scrubs `duo-just-added` from `class=""` so the wash never persists; recent-edits repaint at canvas open within 6s freshness window (re-uses sidecar's `recentEdits` log); `duo selection --pane canvas` returns the H25 union shape via new `CANVAS_SELECTION_PUSH` IPC + `getCanvasSelection` NavBridge entry (extends `--pane` choices: `auto|editor|browser|canvas`); auto pane fallthrough chain is browser → canvas → editor; persistent blurred selection via CSS Custom Highlight Registry (Chromium 105+) — no DOM mutation, no false-dirty (PRD H26); Send → Duo pill on canvas with iframe-content rect → viewport rect translation + `formatCanvasSendPayload` for all three formats; warn-before-overwrite banner gates dirty-buffer html-op writes via in-renderer `pendingHtmlOp` queue + `<WriteWarningBanner>` reuse (PRD H36, mirrors Stage 13b). 22-item verification punch list (V1–V22) filed for a future smoke session — V2 / V14 / V20 / V22 are the highest-risk. Cards: [`docs/roadmap.html#s17`](docs/roadmap.html), [`#s17a-polish`](docs/roadmap.html), [`#s17a5-design`](docs/roadmap.html). PRD: [docs/prd/stage-17-html-canvas.md](docs/prd/stage-17-html-canvas.md). **17d-A** (comment rail + canvas binding): `<CommentRail>` editor-agnostic primitive in `renderer/components/editor/primitives/CommentRail.tsx` — visual layer only, props are surface-shaped (CommentThread record); the same primitive will serve the markdown editor's Stage 11d binding when CriticMarkup ships. Canvas binding: thread grouping by `data-duo-id` with document-order sort, anchor badges painted in body via `commentAnchors.ts` (runtime DOM with `data-duo-canvas-runtime` sentinel, never persisted), full reply / resolve / reopen UX. New-comment flow: "💬 Comment" button pairs with the Send → Duo pill on selection; clicking opens a composer popover; submitting persists + dismisses. CLI verbs: `duo html comment --id|--selector|--text --body "…"` (anchor resolution → nearest `data-duo-id` ancestor) + `duo html comments [--filter all|open|resolved]`. New IPC channels: `CANVAS_HTML_COMMENT[_RESULT]`, `CANVAS_HTML_COMMENTS_LIST[_RESULT]`. `SidecarV1` extended with additive `resolvedThreads?: Record<anchorId, {ts, by}>` (thread state at the sidecar root, not per-entry). 27-item verification punch list (V1–V27) filed for a future smoke session — V20 (CSS Highlight Registry), V22 (warn-before-overwrite), V27 (no comment chrome leaks to disk) are the highest-risk. Cards: [`docs/roadmap.html#s17`](docs/roadmap.html), [`#s17a-polish`](docs/roadmap.html). PRD: [docs/prd/stage-17-html-canvas.md](docs/prd/stage-17-html-canvas.md). **Next: 17d-B** (lock convention) / **17d-C** (skill snippet bundle) / **Stage 14** (markdown editor's CommentRail binding). | **L2** |
...
| **19** | **Duo detection & default-to-claude tabs** (was Stage 18; env signals + passive priming + split-button TabBar + `duo new-tab` CLI) | ✅ **19a + 19b + 19c all shipped 2026-04-26 (v0.2.0 + v0.3.0).** 19a (env signals: `DUO_SESSION`, `DUO_SOCKET`, `DUO_VERSION`, `TERM_PROGRAM=Duo`) in commit `640ec0e`. 19c (split-`+` button on terminal strip — `+` opens claude, `>` opens shell; ⌘T from terminal focus opens claude; ⌘⇧T always vanilla shell; `TabSession.kind` + `claude · <basename>` title; install banner when claude missing; `duo new-tab [--shell|--claude] [--cwd] [--cmd]` CLI verb) shipped on worktree branch `worktree-stage-19c-default-claude-tabs` and merged to main 2026-04-26 (merge commit `cbadc5f`). **19b (passive priming) shipped v0.3.0:** load-bearing PATH shim at `~/.claude/duo/bin/claude` wraps the real binary with `--append-system-prompt "$(cat ~/.claude/duo/priming.md)"` when `$DUO_SESSION` is set; redundant `SessionStart` hook in `~/.claude/settings.json` backs it up; bundled `priming.md` ships at install time and is user-editable. Owner reframed PRD's "primary hook + secondary shim" to "shim load-bearing because we cannot rely on hooks." 19d (mid-tab launch-claude banner) deferred to a later cut. | **L3** |
| **20** | **Interaction polish + `duo doctor` + TCP fallback + pane-aware shortcuts** (was Stage 13; cross-refs issues #12, #22, #23) | 🟡 **Sandbox-resilience cluster shipped v0.4.1 (commit `0293049`).** TCP fallback transport (dual-listener + per-launch random auth token at `~/Library/Application Support/duo/duo.port`); CLI fall-through on `EPERM` / `ECONNREFUSED` / `ENOENT` / connect-timeout. `duo doctor` diagnostic — probes both transports, names "Claude Code sandbox detected" when that's the cause. Sandbox-writable install path (`~/.claude/bin/duo` preferred over `/usr/local/bin/duo`). `duo wait --timeout` socket-cap race fix (`max(explicit + 5s, default)`). **`duo reload` verb shipped v0.5.3** (closed 1 of 6 remaining items). **Remaining ⬜:** tab numbers in unified strip, terminal selection / clipboard refinements, pane-aware `⌘+/-` zoom (issues #22 / #23), PTY-side sandbox audit (issue #12), `⌘[` parent-folder navigation (note: ⌘[ now bound to editor list-outdent in v0.5.3, would need a re-spec). Unblocked by 19's `DUO_SESSION` for `duo doctor`. | **L3 parallel** |
| **21** | **Distribution polish** (was 14b; code sign + notarize + auto-update + session restore + browser history + fork-friendly architecture; cross-refs issues #24, #27) | 🟡 **21a ✅ (v0.4.1)** signed + notarized DMG; toolchain in `scripts/dist-signed.sh`, Sequoia compatibility appendix in `cert-procurement.md`. **21c Phase 1+2 ✅ (v0.4.2)** electron-updater integration + session restore (terminals, file tabs, browser tabs persist via `~/.claude/duo/session-state.json`; closes issue #24). **21c Phase 3 ✅ (v0.5.1)** browser history persistence + native `<datalist>` URL autocomplete; closes [issue #27](https://github.com/dudgeon/duo/issues/27). **21b app icon ✅ (v0.5.1)** `build/icon.icns` + source `build/icon.png` committed. **21e ✅ (v0.5.0)** fork-friendly architecture — `fork.config.json` + Vite runtime injection + provenance-aware install (SHA-256 tracking) preserving user customizations on upgrade. Layered fork modes documented in [`docs/HOW-TO-FORK.md`](docs/HOW-TO-FORK.md). **Still ⬜:** **21b DMG background image** (deferred from v0.5.1); **21d** — socket auth token, agent-driven-navigation notifications, Trailblazers README. | **L4** |
| **22** | **Navigator dual-pane overhaul (context pedagogy)** | ✅ **Shipped v0.4.0.** Top pane "Your Claude settings" with curated three (`~/.claude/CLAUDE.md`, `skills/`, `agents/`) + show-all toggle + collapsible header. Bottom pane gains "Project Claude context" group above the regular file tree (surfaces `./CLAUDE.md` / `./.claude/` / `./tasks.md` / `./AGENTS.md` when they exist; auto-hides when none). Separate state machines (`useUserClaudeNavigator` for top, existing `useNavigator` for bottom) sharing the `<TreeNodes>` rendering primitive (now exported from `FileTree.tsx`). Plain-English label (no literal `~/.claude/`). Pin toggle still on the bottom pane only. File-ops symmetry (rename / delete / reveal-in-Finder) deferred to the Navigator polish backlog bundle. | **L2** |
| **23** | **Canvas actions — Claude ↔ HTML loop** | ✅ **Shipped v0.3.0.** v1 vocabulary live: `claude:spawn` (new tab; optional `data-cwd` + `data-cmd`), `terminal:send` (writes to active PTY; optional `data-enter="true"`), `browser:open` (`data-url` routes through duo-open/external — external-domains.json applies). Renderer-side dispatch via a delegated capture-phase listener on the iframe doc — no `allow-scripts` needed. Trust gate: path-restricted to `~/.claude/duo/` (covers help pages and Stage 18b skill packs); user-marked-trusted folders deferred. `duo send --enter` flag (Stage 23b) pairs with `data-enter="true"`. Worked example at `~/.claude/duo/help/canvas-actions-demo.html`; skill reference at `~/.claude/skills/duo/examples/canvas-actions.md`. Cap One AIP distro live demo lives in Stage 18b's pack. | **L2** |
| **24** | **Pin WorkingPane tabs** | ✅ **Shipped 2026-04-26 (commit `c1c4ea9`).** Right-click on any working-pane tab → Pin/Unpin; pinned tabs render with a pin glyph (replaces the type icon), sort to leftmost in the strip, and gate ⌘W behind a confirm modal. Pin identity is stable across sessions (browser tabs by URL, file tabs by absolute path). Storage at `~/.claude/duo/pins.json` (atomic tmp-rename writes; schema v1; corrupt or missing returns empty list). Pins state owned at App.tsx so the keyboard ⌘W handler can gate before calling closeFileTab/closeBrowserTab; strip-side close-button uses its own local confirm state; both render `<PinnedCloseConfirm />`. Distro pre-pins via Stage 18b's `PACK.json § pins` (deferred). Pinned tabs become highest-priority entries in Stage 21c session restore (deferred). Pin scope: WorkingPane only — terminal pinning deferred. **Verification owed:** main-process change needs Electron full restart. Card: [`docs/roadmap.html#s24`](docs/roadmap.html). | **L3 (FTUX trio)** |
| **25** | **Post-redirect chrome banner** | ✅ **Shipped v0.4.0.** After `shell.openExternal` succeeds, main pushes `IPC.EXTERNAL_REDIRECTED { host, reason? }`; renderer mounts `<ExternalRedirectedBanner>` above the WorkingPane with auto-dismiss after 6s. Most-recent-wins (back-to-back redirects replace, not stack). Schema extension: `external-domains.json` entries can now be `string` OR `{host, reason?}` — backward-compatible. v0.4.0 also seeds `*.capitalone.com` into the bootstrap list so Cap One Trailblazers' internal sites auto-route on first install. Contact-link mechanism (mailto, slack) deferred. | **L4** |
| **26** | **Navigator polish & ergonomics** (was the `backlog-nav-polish` item — promoted to a stage 2026-04-27) | ✅ **PRs 1 + 2 shipped v0.5.0.** PR 1 = row-interaction cluster (single/double-click, chevron split BUG-025, right-click Rename/Trash + CLI, hover-Claude). PR 2 = Pinned section (ENH-010) + default-collapsed user-claude pane (ENH-012). PR 3 (ambient signals + Go-to path — items 2/3/4/5/8) deferred to v0.5.1+. **v0.5.0 follow-ups closed in v0.5.1:** BUG-007 (deleted-files-linger watcher hardening), BUG-028 (Escape dismisses inline rename), BUG-029 (right-click context menu flips up at viewport bottom), BUG-030 (CLI pin/unpin pushes to renderer live). | **L2** |
| **27** | **Canvas authoring vocabulary + skill** (Sprint A of the FTUX-tutorial initiative) | ⬜ Not started. Filed 2026-05-01 after the v0.5.5 walk results spawned a richer vision: ship the *primitives* for interactive canvas content (action verbs + agent events + form-input bindings + templates + skill), THEN use them to author tutorial packs (Stage 28). **What ships:** (a) **six new canvas-action verbs** — extends Stage 23's three (`claude:spawn` / `terminal:send` / `browser:open`) with `editor:open`, `nav:reveal`, `selection:set`, `theme:set`, `terminal:focus`, `duo:event`; (b) **`duo events --follow`** streaming CLI verb — pulls in [issue #19](https://github.com/dudgeon/duo/issues/19) from the 15-family backlog so `duo:event` clicks reach the running Claude agent as a JSON-line stream; (c) **`data-payload-from`** form-input binding — `<button data-action="duo:event" data-event="step-done" data-payload-from="#name-input">` reads the input value into the event payload; (d) **`<meta name="duo-default-editable">`** routing convention + per-tab edit-mode toggle (folds in ENH-034); (e) **canvas authoring skill** at `skill/canvas-authoring.md` — full reference covering action vocabulary, `data-duo-pane` stable selectors, when-to-canvas-vs-browser-vs-markdown, anti-patterns; (f) **five reference templates** at `skill/examples/canvas-templates/` — `button-card.html`, `paint-target.html`, `form-input.html`, `lesson-scaffold.html`, `dashboard.html`. Subsumes Stage 17a.5 (template gallery — Direction A). **No tutorial content** — Sprint C / Stage 28 builds that on top. | **L2** |
| **28** | **First-launch lesson packs** (Sprint C — content built on Stage 27's primitives) | ⬜ Not started. Filed 2026-05-01. Two skill packs that ship as part of the v0.6.0 install bundle: **(a) `intro-to-duo` pack** — single-canvas FTUX tutorial for CLI-noob users new to Claude Code. "Start lesson" button uses `claude:spawn` (Stage 23) with `data-cmd` to open a fresh Claude tab in a chosen CWD with a lesson skill auto-invoked. The lesson skill (installed at `~/.claude/skills/intro-to-duo-lesson/SKILL.md` via the pack) knows the lesson structure, runs `duo events --follow` to track user clicks, paints content into `data-duo-pane` regions on demand. **(b) `claude-code-basics` pack** — multi-canvas pack derived from `untitled-folder/claude-code-basics-temp.md` (Capital One PM curriculum). One canvas per "Family" or per 30-min spine module. Validates the multi-canvas pack-with-shared-skill case. Both packs are FTUX defaults (`PACK.json § defaults`) — auto-open as tabs on first launch; user dismisses → tab closes via existing close machinery → tab does NOT reopen on subsequent launches (existing session-restore semantics). **Validation goal:** by building these packs we confirm Sprint A's primitives are sufficient — anywhere we have to escape the convention is a primitive gap. **Cross-ref:** Stage 18b (distro pack format — Sprint B; if not yet shipped, packs install via temporary `cp -r` + manual sync until 18b lands). | **L2 → L3** |
...
| Templates folder + new-file template loader (`.md` and `.html`) | First-class templates surface that **both** new-file flows (`⌘N` + `duo <type> new`) pull from and the **duo agent** can read / list / write / install. v1 location: `~/.claude/duo/templates/<ext>/<name>.<ext>` (next to existing agent-shared `external-domains.json`); project-local `./.duo/templates/` is a v2 layer. `App.tsx § onCommitNewFile` grows a "Blank · Template ↓" picker on the filename interstitial when templates exist for the typed extension; `↵ for blank` muscle memory preserved. Agent-side CLI (CLAUDE.md §4 parity): `duo template list [--ext <ext>] [--json]`, `duo template get <name>`, `duo template new <path.ext> --from <template-name>`, `duo template install <src> [--name <n>]` — full new-CLI-verb plumbing checklist (shared/types, preload, main, socket-server, cli/duo, SKILL.md, **`agents/duo.md`** cheat-sheet). v1 schema = raw files in subdirs by ext; v2 = optional `<name>.duo-template.json` sidecar (`{ description, author, tags }`) for picker tooltip + `--json` listing. **Cross-refs:** Stage 17a.5 (HTML template-registry direction overlaps — if 17a.5's 3-template seed lands first, this backlog promotes the location and adds the markdown branch + agent CLI; if this lands first, 17a.5's seeds become the directory's first inhabitants), Stage 11 D33a (host of the picker), Stage 18 (installer should bootstrap the dir alongside `~/.claude/duo/`), `shared/html-boilerplate.ts` (retires or becomes the `default.html` template). **Open question:** per-extension `default` template auto-seeds without picker UI? Probably yes for muscle memory; picker is for "give me something different." Card: [`docs/roadmap.html#backlog-templates`](docs/roadmap.html). |
| Help / FAQ surface — agent-readable, in-app browsable, with live keyword search | Single help / FAQ source the **duo agent** can read (so it can answer "how do I…" without the user leaving the terminal) **and** the human can browse from inside the app. v1 can be as simple as a static, uneditable HTML file with a text input at the top driving JS-based live-type keyword filter — no backend, no search index, just `document.querySelectorAll` + an `input` event. **The hard part is routing:** this file must NOT open in the HTML canvas (Stage 17a's iframe excludes `allow-scripts` per PRD H4/H8 — live-search JS wouldn't run) and must NOT be editable. **Architecture wedge — declare a routing property.** Two candidates: (a) **`open_in: browser`** — file declares it should mount as a browser tab (Stage 2's `WebContentsView`) rather than a canvas tab; cleaner because scripts run, the file is naturally read-only, and the property is *declarative* about intent (**recommend** for v1); (b) **`canvas: locked`** — file mounts in canvas but `contentEditable` off + toolbar hidden; doesn't solve script-execution, so insufficient for the live-search use case alone (still useful follow-up for read-only HTML that doesn't need scripts — frozen reports, rendered specs). **Where the property lives:** HTML `<meta name="duo-open-in" content="browser">` in the file itself (no sidecar, copy-safe — recommend for the FAQ case); or `<file>.duo.json` sidecar (extends the schema already on the books for Stage 17b — better for runtime metadata that grows); or path convention (`~/.claude/duo/help/` always opens in browser — defer). `renderer/components/fileClassifier.ts` reads the property before deciding tab type. **FAQ location:** `~/.claude/duo/help/faq.html`, shipped in the app bundle and copied by the Stage 18 installer alongside the skill / agent. Agent reads via `cat` or a new `duo help [topic]` verb (defer until felt). **Human entry point:** menu item ("Help → Duo FAQ") opens it as a browser tab — discoverable, no chrome change; alternatives: "?" button, `⌘?` shortcut. **The HTML itself:** ~30 LOC of inline JS — `<input id="q">` at top; FAQ entries as `<article data-q="..." data-tags="...">`; filter on `input` by matching query against `data-q` + `data-tags` + article text. **Cross-cuts:** `open_in: browser` is generally useful — any HTML the agent generates that should run scripts (chart.js, interactive demos, mini-tools) will want it. **Sequencing:** can ship before Stage 17b — the routing property + a single FAQ file are independent of the agent-write surface for the canvas. Most likely shape: small dedicated stage or sub-item that rides Stage 18's installer work (the FAQ has to land somewhere on disk and the installer is already touching `~/.claude/duo/`). Card: [`docs/roadmap.html#backlog-help-faq`](docs/roadmap.html). |
| Navigator polish & ergonomics pass | Bundle (filed 2026-04-26): (1) double-click to open / single-click to select — unlocks selection state for context-menu actions; (2) highlight active terminal's CWD in the tree as ambient signal; (3) distinguish open-but-inactive from active in the file row highlight (refines existing Stage 10 follow-up "highlight files open in WorkingPane tabs"); (4) verification spike — does "new terminal" default CWD to current navigator view? (Stage 10 PRD silent on the default); (5) **BUG-007** — deleted files linger until reload (filed in `tasks.md`); (6) right-click delete (`shell.trashItem` for macOS-native trash) / inline rename + CLI parity `duo file rename` / `duo file trash`; (7) hover-action on folder rows: tiny clawd-icon button → opens new terminal tab with CWD = folder AND auto-launches `claude` in fresh PTY (CLI parity overlaps with backlog 15-family `duo tab --cmd` — could reuse as `duo new-tab --cwd <path> --cmd "claude"`); (8) **"Go to" path input** — clicking the breadcrumb (or hitting `⌘⇧G` Finder-parity, alt `⌘L`) flips it to an editable input pre-filled with the current path + selected; ↵ commits, ⎋ cancels; `~/` expanded; relative paths resolve against navigator cwd; if the resolved path is a *file*, navigate to its parent and select the file. CLI parity already lives in `duo reveal <path>` (Stage 10 § D16) — item 8 is the user-facing UI counterpart so the human can do what the agent already can. Items 1+6+7 share the row-interaction surface and ship well together; item 8 layers onto the breadcrumb chrome. Card lives at [`docs/roadmap.html#backlog-nav-polish`](docs/roadmap.html). |
...
- [x] **`duo reload`** ✅ shipped v0.5.3 — pair for `duo navigate`
      that doesn't require a URL; reloads the active browser tab
      in place. Closes the Stage 8 iteration flow.
...
### Phase 17a — Render + edit primitive (~3–4 PRs) **— shipped 2026-04-26 night**
- [x] WorkingPane registers `html-canvas` tab type; FileTree click on
      `.html` opens it via the file classifier (`.html`/`.htm` → `html-canvas`).
- [x] `duo html new <path.html> [--title "…"]` + `duo edit <path.html>`
      alias (via classifyFile) + `duo view <path.html>`.
- [x] `⌘N` extension-based dispatch (PRD H6.1 audible) — one
      interstitial, typed extension picks the surface.
- [x] Iframe-srcdoc host with contentEditable on body; MutationObserver
      drives autosave + dirty.
- [x] Top toolbar mirroring Stage 11's EditorToolbar visual: B/I/U/S/code
      + link picker (⌘K).
- [x] Selection-aware mark applicator without `document.execCommand`
      (PRD §8 non-negotiable).
- [x] Save: autosave (~800ms) + `⌘S` + dirty dot (H33).
- [x] Skill stub (H16) at `skill/examples/html-canvas-authoring.md` —
      README only, no snippets yet.
- **Exit:** PM opens an `.html`, edits prose, saves; the file is
  clean HTML another tool can read. ✓

**Explicit 17a deferrals** (consumed in 17b/c/e — see PRD § 7):
- Pretty-printed save (H34) — `outerHTML` as-is in 17a; serializer
  in 17b/e.
- Scripts (H8) — preserved on disk but inert (sandbox without
  `allow-scripts`); per-file opt-in dialog in 17e.
- `data-duo-id` injection (H12–H15) + `duo html query/get/set/replace/append/attr` — 17b.
- Send → Duo pill on the canvas (H27) — 17c (gated on canvas-side
  selection observer).
- External-write reconciliation (H35) — 17e.
...
- [ ] **ENH-027** — canvas-default routing for local HTML via
      ...
      **Held until 17e** because the same scripts opt-in machinery
      ...
      expose Reveal/Trash menu — ✅ shipped v0.5.3) covers the
