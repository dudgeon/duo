...
| **27** | **Canvas authoring vocabulary + skill** (Sprint A of the FTUX-tutorial initiative) | 🔄 **In progress — code shipped in v0.5.6 as INTERNAL PREVIEW (not yet validated). Walk-2 of v0.6.0 (2026-05-02) surfaced 4 regressions in this surface (BUG-052..055); full release in v0.6.0 once those fix.** Sprint A code committed: (a) **six new canvas-action verbs** — extends Stage 23's three with `editor:open`, `nav:reveal`, `selection:set`, `theme:set`, `terminal:focus`, `duo:event`; (b) **`duo events --follow`** streaming CLI verb (closes [issue #19](https://github.com/dudgeon/duo/issues/19)); (c) **`data-payload-from`** form-input binding; (d) **`<meta name="duo-default-editable">`** routing + per-tab edit-mode toggle (folds in ENH-034); (e) **canvas authoring skill** split into `canvas-authoring.md` + `canvas-interaction.md` (Anthropic single-responsibility); (f) **five reference templates** at `skill/examples/canvas-templates/`. **Walk-2 regressions still open:** BUG-052 (`editor:open data-mode='canvas'` toolbar/strip missing), BUG-053 (`nav:reveal` doesn't highlight file), BUG-054 (`terminal:focus` cursor not active), BUG-055 (canvas click should focus working pane — BUG-037 sibling/regression). | **L2** |
| **28** | **First-launch lesson packs** (Sprint C — content built on Stage 27's primitives) | 🔄 **In progress — code shipped in v0.5.6 as INTERNAL PREVIEW (not yet validated). Renders cleanly on walk-2; "Start lesson" spawn behavior in `intro-to-duo` needs gating (ENH-049). Sanitized of all employer-specific references. Full release in v0.6.0.** Both packs committed: **(a) `intro-to-duo` pack** — single-canvas FTUX tutorial; "Start lesson" uses `claude:spawn` to open a fresh Claude tab with a lesson skill auto-invoked. **(b) `claude-code-basics` pack** — multi-canvas pack derived from a generic Claude Code curriculum. Validates multi-canvas-with-shared-skill. Both packs are FTUX defaults (`PACK.json § defaults`) — auto-open on first launch; user dismisses → tab does NOT reopen on subsequent launches. Both rendered cleanly in walk-2; gating on the spawn (ENH-049) and full validation queued for v0.6.0. **Cross-ref:** Stage 18b (distro pack format — Sprint B, code shipped in v0.5.6, CLI walk passed). | **L2 → L3** |
