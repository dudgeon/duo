...
| **27** | **Canvas authoring vocabulary + skill** (Sprint A of the FTUX-tutorial initiative) | ✅ **Shipped v0.6.0 (2026-05-02).** All walk-2 + walk-3 regressions fixed. Sprint A surface: (a) **six new canvas-action verbs** — extends Stage 23's three with `editor:open`, `nav:reveal`, `selection:set`, `theme:set`, `terminal:focus`, `duo:event`; (b) **`duo events --follow`** streaming CLI verb (closes [issue #19](https://github.com/dudgeon/duo/issues/19)); (c) **`data-payload-from`** form-input binding (cross-realm safe); (d) **`<meta name="duo-default-editable">`** routing + per-tab edit-mode toggle (folds in ENH-034); (e) **canvas authoring skill** split into `canvas-authoring.md` + `canvas-interaction.md`; (f) **five reference templates** at `skill/examples/canvas-templates/`. Walk-2 regressions BUG-052..055 + walk-3 BUG-053-v2 (route nav:reveal to user-claude pane for ~/.claude paths) all shipped. v0.6.0 known issues: ENH-050 (smoother WCV mute), BUG-062 (dev-build update banner version mismatch), BUG-063 (smoke-walk mid-sentence Copy-block extraction); none release-blocking for canvas-authoring users. | **L2** |
| **28** | **First-launch lesson packs** (Sprint C — content built on Stage 27's primitives) | ✅ **Shipped v0.6.0 (2026-05-02).** Both packs ship as FTUX defaults: **(a) `intro-to-duo` pack** — single-canvas FTUX tutorial; "Start lesson" uses `claude:spawn` to open a fresh Claude tab with a lesson skill auto-invoked. **(b) `claude-code-basics` pack** — 7-canvas curriculum (orientation + 7 family canvases) derived from a generic Claude Code curriculum, sanitized of all employer-specific references. Validates the multi-canvas-with-shared-skill pattern. Both auto-open on first launch via `PACK.json § defaults[].openOnFirstLaunch`; user dismisses → tab does NOT reopen on subsequent launches. ENH-049 ("Start lesson" gating) queued for a v0.6.x follow-up — currently the spawn fires unconditionally; users without an active terminal session see the spawn but might miss the destination. **Cross-ref:** Stage 18b (distro pack format — Sprint B, ✅ shipped v0.6.0). | **L2 → L3** |
