...
A macOS desktop app ("Orbit") that pairs multiple Claude Code terminal sessions
with an embedded Chrome browser, connected by a local CLI bridge (`orbit`) so
...
- CLI: `cli/orbit.ts` (complete command dispatch, socket client)
...
| `skill/SKILL.md` | The orbit skill (teaches Claude Code how to use the CLI) |
...
2. **Confirm the working name "Orbit" with Geoff before Stage 5.** The name was coined by a previous Claude instance, not by Geoff. Don't embed it in user-facing strings until confirmed.

3. **The CLI is the spec.** Every time a new CLI command is added, update `cli/orbit.ts`, `skill/SKILL.md`, and the command reference in `duo-brief.md §9`.
...
IPC. The `orbit` CLI (a standalone Node.js script) connects over the Unix socket to
...
| Question | Priority |
|---|---|
| Confirm "Orbit" as the app/skill name | Before Stage 5 |
