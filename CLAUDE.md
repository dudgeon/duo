...
| Scheduled (cron) sessions (ENH-223) | Shipped #103 (v0.11.2 batch). In-app next-fire timer (NOT a system daemon — fires only while Duo is open); scheduler starts only after `SESSION_STATE_RESTORE_SETTLED`; store is §D9-clean. Create/manage from Home; `duo cron …`. **Two job kinds (ENH-237, #112):** a **claude** job is interactive (headless `-p` behind a default-OFF flag); a **shell** job (`--run`) runs a raw single-line command in a background tab — no Claude session, no headless gate. Shell jobs are CLI-created + UI-edited (deliberate asymmetry, PRD § 12). ADR in `docs/DECISIONS.md`. |
