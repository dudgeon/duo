...
**Owner directive (2026-06-20, verbal — captured so it survives compaction):** in-editor **Cmd+Z must work as a user expects in EVERY scenario** (typing+autosave, agent write, disk reload, restore). That is **table stakes, independent of and prerequisite to the history UI.** The history modal is an **additional, richer rewind** (cross-session timeline + agent/you attribution + restore-any-point), layered on top of native undo — NEVER a replacement for it.

**Open / deferred (next phases):**
- **[P0 — TABLE STAKES] In-editor Cmd+Z correctness (independent of the modal).** The original complaint. Acceptance criteria: (A) typing+autosave → Cmd+Z reverts my edits, save is invisible to undo; (B) an agent/`duo doc` write → one Cmd+Z reverts the whole agent change (redo restores it); (C) a disk-reconciliation reload → Cmd+Z reverts to the prior visible state, never a no-op; (D) a modal **Restore** → Cmd+Z reverts the restore; (E) **anti-pollution** — Cmd+Z NEVER appears to "do nothing" by consuming an invisible programmatic transaction; (F) undo survives tab-switch (BUG-046), dies only on tab close / restart (the volatility the modal addresses).
  - **✅ LANDED (code, 2026-06-20):** the pollution class (criterion E) — `applyCriticMarkupFromText` ([markdownCriticMarkup.ts](renderer/components/editor/markdownCriticMarkup.ts)) and the sidecar comment re-anchor (`applyCommentMark` gained an `addToHistory` param; [CommentMark.ts](renderer/components/editor/extensions/CommentMark.ts) + [markdownComments.ts](renderer/components/editor/markdownComments.ts)) now dispatch with `addToHistory:false`, so a load/reload mark pass adds **zero** undo steps. Verified by [undoHistory.test.ts](renderer/components/editor/undoHistory.test.ts) (4 jsdom live-editor tests measuring `undoDepth`; **teeth-checked** — drop the guard → the test fails). Confirmed by code-read: B holds (agent `setContent(_,true)`+`markJustAdded`(decoration) = one step), C holds (reload `setContent(_,false)` = one step + guarded passes). Product call locked: reconciliation reloads ARE undoable, as ONE step. Note: the ENH-221 capture hook is main-process + fire-and-forget → cannot affect the renderer's undo stack.
  - **⏳ STILL OWED:** **live Electron keystroke verify** of A–F with real Cmd+Z routing through the keymap (synthetic events don't always route; unit tests prove the stack-depth invariant, not the live keybinding). Owed to the Electron-holding agent. (F is structurally true via BUG-046 mounting; D verifies once the modal exists.)
- **[P1, UI] History view modal.** **DECIDED 2026-06-20:** surface = **B · modal**; restore = **overwrite-with-confirm**; rows = **standard** (time · who · Δsize); diff = **rendered inline-highlight**. Decision playground: [docs/research/enh-221-history-view.html](docs/research/enh-221-history-view.html).
  - **✅ LANDED + VISUALLY VERIFIED (2026-06-20):** [HistoryModal.tsx](renderer/components/editor/HistoryModal.tsx) + toolbar `◷` button ([EditorToolbar.tsx](renderer/components/editor/EditorToolbar.tsx)) + wiring in [MarkdownEditor.tsx](renderer/components/editor/MarkdownEditor.tsx), on the `window.electron.history.*` bridge. Parks the browser WCV while open. Verified live end-to-end (`duo dom` drive + computer-use **screenshots**, via `request_access(["com.github.Electron"])`): timeline renders newest-first (time · who · Δsize), Restore disabled on newest / enabled on older, two-step confirm, restore reverts disk AND the open editor reconciles (criterion D), restore snapshot captured. The modal renders cleanly above DOM/terminal content. typecheck clean, 1605/1605.
  - **✅ Rendered inline-highlight diff** — the preview pane is a read-only TipTap instance; selecting an older version shows the current content with the old→current diff marked inline (green insertions / struck-red deletions) via ENH-197 `applyTrackedDiff`; newest selected = plain. Screenshot-confirmed. Granularity is block-level (same engine as the reload-diff; finer on multi-paragraph docs).
  - **🟡 v0.11.2 smoke walk-1 (owner, 2026-06-20): 2 PASS / 2 FAIL.** PASS: OPEN-RESTORE, LOOK. FAILs below.
    - **✅ FAIL→FIXED — Cmd+Z dead (the REAL original complaint).** Owner: "clicking undo/redo works, cmd+shift+z works; cmd+z does not work." Root cause (NOT the pollution I fixed earlier — a separate, bigger bug): [MarkdownEditor.tsx](renderer/components/editor/MarkdownEditor.tsx) `handleKeyDown` called `matchGlobalShortcut(e, { inEditableSurface: true })` WITHOUT `inAnyTextInput: true`, so ENH-179's ⌘Z reopen-last-closed-tab matched *inside the editor* and `return true` blocked ProseMirror's undo keymap (⌘⇧Z has shift → no match → redo worked; toolbar undo is a direct command → worked). **⌘Z has reopened tabs / no-op'd instead of undoing since ENH-179 (v0.7.7) — likely the dominant cause of the original "impossible to undo" report.** Fix: pass `inAnyTextInput: true` (mirrors the document capture handler in `useKeyboardShortcuts.ts:423`). Test-gap: matcher tests cover the ctx but nothing asserts the editor *passes* it → a real-keystroke/integration guard is owed. typecheck + 1605/1605 green. **Needs owner re-walk (real ⌘Z).**
    - **✅ FAIL→FIXED — WCV occlusion / dead footer Close = BUG-209.** Owner: "occluded by split view if the window is small enough; MORE concerning the modal 'Close' button is non-functional/no-op; the 'x' works." Both close buttons call the same `onClose`; the footer Close sat under the un-parked split-view aux WCV. Fixed by parking the aux WCV on modal-open ([AuxBrowserSlot.tsx](renderer/components/AuxBrowserSlot.tsx), see BUG-209). **Verified live:** aux parks when the modal opens (no occlusion), footer Close clicks through, aux restores on close.
    - **ℹ️ Copy buttons (the trigger for this thread) = BUG-211 — NOT a real persistent bug.** Copy WORKS when the page has keyboard focus (verified live: the button shows "Copied!"). The failure is a `Document.hasFocus()` race — the FIRST click after the browser-pane WCV loses keyboard focus fails the clipboard write; a click in the page body first (which focuses the WCV) makes it succeed. NOT a permission issue (no permission change was needed). Non-blocking: results persist in localStorage (agent reads them directly), "Send to Claude" bypasses the web clipboard, and click-page-first is a workaround. Proper fix (browser-pane WCV focus management) deferred — see BUG-211.
  - **✅ Agent-vs-you source tagging (2026-06-20):** Duo-mediated agent writes now capture as `source:'agent'` (the "who" column shows **Agent** + iris dot, distinct from **You**/orange and **Restored**/green). Editor-mediated (`duo doc write`/`edit` on an open file) via a renderer `agentWritePendingRef` threaded through `files.write({historySource})`; disk-direct (`duo doc insert`/CriticMarkup ops, closed-file edits, `duo json set`) tagged at the socket-server write sites. User typing stays `'save'`. Screenshot-confirmed in the timeline.
  - **Deferred follow-ups:** save-chip popover (D) for one-click quick-restore; **canvas parity** — the HTML canvas captures saves but has no History modal yet → mirror-port is **(c) Deferred** per the renderer-surfaces parity rule; frontmatter/CriticMarkup-token handling in the diff preview (currently rendered as raw markdown); external/raw-`Edit` writes still read as no-source until the watcher-capture P2 below lands.
...
### ENH-222: Worktree lifecycle UX — PM-friendly create + graceful removal recovery (ENH-210 D5-C escalation)

**Status:** 🚧 In progress — **all 6 decisions locked; BOTH enhancements BUILT + live-pre-walked** (branch `claude/eloquent-albattani-7c44d4`); **owner smoke-walk 1 done (2026-06-20): 2 PASS / 1 SKIP** (banner-legibility fix `4475df8`). **NOT cutting** — see the renumber note below. Full suite green (99 files / 1607 tests), typecheck clean. **PRD:** [`docs/prd/enh-222-worktree-lifecycle.md`](docs/prd/enh-222-worktree-lifecycle.md) (decisions + implementation + findings/deviations). **Priority:** Strategic — agents-in-worktrees is Duo's reason to exist; a non-technical PM can't drive `git worktree add`, so Duo needs first-class Create + teardown. **Effort:** M–L. **Filed:** 2026-06-18 (owner verbal directive: "keep advancing the worktree controller UI"). **Parent:** ENH-210 (worktree-aware Duo). **Renumbered ENH-221 → ENH-222 (2026-06-20):** the *other* agent's ENH-221 (`claude/enh-221-file-history` — durable file version history) **landed first**, so per the multi-agent-collision rule this (unmerged) work took the next free id, **ENH-222**, and retargets a later minor than the file-history line's v0.11.2 (version TBD; not cutting). Commits `fc9ea43`…`4475df8` still say "ENH-221" in their messages (immutable history); all docs + code refs are now ENH-222.
...
**Build log — Enhancement 2 (removal recovery), 2026-06-19:**
- `1431800` **renderer** — worktree-removal recovery. `useNavigator`'s vanished-cwd self-heal is now worktree-aware (D5/D6): FileTree feeds the live worktree identity via `setWorktreeRevertTarget` while alive; when the cwd dies, the heal reverts to the captured **mainRoot** (not the nearest path ancestor, which for a nested worktree would strand the user in `.claude/worktrees/`) and raises a dismissible **"back on main" banner** (`state.removedWorktree`), auto-cleared on any navigate. `pathIsWithin` (pruneDeadPaths) is the boundary-safe match (a sibling that prefixes the worktree name is NOT within it) — 3 regression tests. FilesPane wrapped in a scoped `ErrorBoundary` (defense-in-depth → panel + Reload, never a blank window).
- **Live-verified (dev):** rooted nav in `recovery-two`, removed it under-foot → reverted to main (`/…/duo`, NOT `.claude/worktrees/`) in ~1s + banner *Worktree "recovery-two" was removed — you're back on main* + dismiss ✕ works; no crash, no ErrorBoundary, no log errors. (This is the exact failure that hit THIS session's worktree purge on 2026-06-18 — now handled.)
- **Next:** `/smoke-walk` covering all of ENH-222 (owner option (a) — one combined walk), then propose a cut.

**Findings + follow-ups (from the live pre-walk, 2026-06-19) — full detail in the PRD § C/E:**
- `35f7c3a` **fix** — two gaps the pre-walk caught: (C-1) lone-repo dropdown showed an empty "Switch worktree · 0" (FileTree discarded the worktree list unless >1 — a pre-ENH-210 optimization the always-on pill exposed); now stores the full list so the main row shows. (C-2) removal recovery only fired off the fs-watcher; added the window-focus backstop D6 had specified (extracted `recoverDeadCwd`), for the busy-dir / whole-repo-`rm` case the watcher can miss.
- **Owner smoke-walk 1 (2026-06-20): 2 PASS / 1 SKIP.** LONE-MAIN ✅, REMOVAL ✅ (with a caveat — *the banner was illegible*), CREATE ⏭️ SKIP (owner didn't walk it). `4475df8` **fix** — the removal banner hardcoded a dark bg but used theme ink vars for text → dark-on-dark in **light mode**. Hardcoded light text/glyph colors (legible dark toast in both themes); verified live in light mode (body `rgb(216,205,180)` on bg `rgb(42,32,26)`). **Owed:** owner re-glance of the banner (or trust the verify); CREATE later confirmed successful by owner (the SKIP was a not-walked, not a failure). **Collision resolved (2026-06-20):** the file-history ENH-221 landed first → this work renumbered ENH-221 → **ENH-222** + retargets a later version than v0.11.2 (see the status-line renumber note). Not cutting; updating the PR/branch only.
- **Open follow-ups (none gate the cut):** (C-3) D6's "live terminal in a dead folder" in-terminal notice is NOT implemented — the terminal stays open (history intact) but shows no notice. (C-5) the dropdown doesn't refetch worktrees on open, so an agent-created worktree doesn't show in a PM's already-open dropdown until focus/cwd-change. (C-4) the inline form has no base-branch picker (CLI `--from`) and opens in this-window only (CLI `--window` + per-row button) — deliberate v1 asymmetries.
- **Process note (C-6):** computer-use cannot drive the dev Electron (access-grant hangs even by bundle id `com.github.Electron` — two 300s timeouts); all UI verification was renderer-driven (`duo dom`/`duo eval`), no screenshots. Reconfirms `feedback_computer_use_cannot_reach_dev_electron`.
...
2. **Survive removal under-foot** — when the agent merges + `git worktree remove`s the worktree the navigator is rooted in, Duo must **not throw a render error**: tell the user, auto-revert the navigator to main. (Live evidence: 2026-06-18 a broad worktree purge deleted this very session's worktree while its dev app was running from it → socket died + renderer degraded. Exactly the failure this enhancement designs against — now also implicates the *running app*, not just the navigator.)

**Decision record (review task — keep surfacing in every smoke walk until owner closes).** Playground: [`docs/research/worktree-lifecycle-ux.html`](docs/research/worktree-lifecycle-ux.html) — high-fidelity dark-chrome flow study, 4 create-flow options + the removal-recovery state, 6 decisions with Copy round-trip. Follow-up UI study (D1 form treatments): [`docs/research/worktree-create-ui.html`](docs/research/worktree-create-ui.html).

**Owner picks so far (walk 1, 2026-06-18):**
- **D1 = inline approach + form UI Variant A** (one-line, type-and-go) ✓✓ — 4 treatments explored in `worktree-create-ui.html`; owner picked **A** (2026-06-18). **+ slug validation** (owner ask, 2026-06-18): live-sanitize the typed name into a folder/branch-safe slug — lowercase · spaces/underscores → `-` · **allow-list** `[a-z0-9-]` (strip everything else as typed) · collapse + trim hyphens · cap ~50 chars · fall back to auto-name ("Name it for me") if it sanitizes to empty. Allow-list (not blacklist) so the result is safe for BOTH the directory name AND the git ref (git ref rules exclude ` ~ ^ : ? * [ \ .. ` etc., all already excluded by the whitelist). Field keeps the raw text; the `→ creates claude/<slug>` preview line shows the safe name live.
- **D2 = describe → auto-slug (B)** ✓ **+ owner add:** must offer an auto-generated name when the user doesn't want to type (🎲 / leave-blank). Folded into all D1-UI variants.
- **D3 = start a Claude session, toggle default-ON (B)** ✓
- **D4 = open in this window, re-root (A)** ✓ (new-window stays on the existing per-row button)
- **D5 = auto-revert to main + dismissible banner (B)** ✓
- **D6 = watcher/focus re-probe + vanished-cwd guard on every nav read (B)** ✓ — the crash-prevention spine; needs a regression test (recurring-regression rule).

**Implies on accept:** new `duo worktree new "<desc>" [--from] [--here|--window] [--claude]` + `duo worktree remove <path>` verbs (CLI-parity rule 4; 4-surface doc sync + rebuild binary; Duo *writes* git worktree state for the first time, next to `core/git/worktree.ts`); pill becomes an always-on dropdown trigger (reachable from a lone main); navigator ErrorBoundary + revert-to-main path in `useNavigator`. Smoke matrix gains: create-from-lone-main, create-from-worktree, removal-under-foot revert, terminal-in-dead-folder.
...
### ENH-223: Scheduled ("cron") Claude Code sessions — create / view / manage from Home

**Status:** 🚧 In progress 2026-06-20 — **spec locked** (10 decisions, all recommendations + 2 owner notes) · **Tier 1 (engine + CLI) built + LIVE-VERIFIED** (typecheck clean; 56 cron unit tests + full suite 1647 green; CLI + 4-surface sync done). **Local-session work (2026-06-20):** (a) D8 deviation re-raised → owner delegated, **dependency-free engine kept** (revisit `cron-parser`+`cronstrue` at Tier 2 when F3's cron preview is built); (b) no-window fire now records **`"missed"`** per D10(3) (was `"error"`), unit-test locked; (c) **live walk DONE** — fresh run, D3 resume+fallback, scheduled fire, D5 catch-up (+negative), D10 match+fallback, verb cluster, D4 gate all verified via JSONL + per-window `term tabs`; (d) **2 live-only bugs FIXED**: **F1 focus steal** (cron tab activated → now opens background via a `NewTabRequest.background` flag) and **catch-up clobbered at launch** (raced session restore's wholesale `setTabs` → gated scheduler start on a new `SESSION_STATE_RESTORE_SETTLED` signal from the primary window). **Tier 2 increment 1 (2026-06-20) — DONE + live-verified:** engine→renderer plumbing (CronService change-emitter → `CRON_JOBS_CHANGED` broadcast + `CRON_INVOKE` handler + preload `cron` namespace) and the Home **"Scheduled"** block (`CronSection`) — status chips (ran/never/missed/error/paused), next-fire, project, row actions (Run now / Pause / Resume / Delete with a 2-click confirm guard); verified live via `duo dom` (render, live push on add, button→invoke round-trip, delete-confirm). Also fixed a **pre-existing flaky** cron-service teardown race (a tick's fire-and-forget persist landing mid-rmdir → ENOTEMPTY) via `CronStore.whenIdle()`. **Tier 2 increment 2 (2026-06-20) — DONE + live-verified:** create/edit dialog (`NewCronJobModal`: preset segmented control + conditional fields + debounced F3 live preview, session radio, catch-up) + entry points (D7) — Home "+ New job"/Edit + project-rail right-click (a `duo-open-cron-modal` CustomEvent) + `File ▸ New Scheduled Job…` (IPC push); new `handleCli` ops `edit`/`preview`; **CLI parity verb `duo cron edit`** (+ 4-surface docs, binary rebuilt). Verified via `duo dom` (open→fill→preview→create, Edit→prefill→save, CLI edit→push→UI). **Adversarial audit (2026-06-20, multi-agent: 14 findings → 8 confirmed / 6 refuted) — 7 FIXED + live/unit-verified:** (HIGH) modal persisted a non-absolute cwd → silent HOME-dir run — modal now requires an absolute path + a server-side `assertCwdAbsolute` guard; (MED) `fireJob` catch didn't reschedule → tight every-tick retry on a persist throw — catch now reschedules; (LOW) fire-time runner stat (no spawn into a vanished dir), reject unschedulable cron (Feb 30) on add/edit/preview, `fromSchedule` defensive default, `canSave` preview-pending gate, bare-`duo cron` usage string + IPC-error prefix strip. **Audit #8 → RESOLVED 2026-06-21** (hand-rolled `describeCron`, no dep, per owner D8; renders cron in natural English for the F3 preview + Home rows + CLI list/show; honesty-biased — echoes anything it can't render faithfully. A second adversarial multi-agent pass — 47 agents grounding every claim against the engine's real fire times — caught + fixed two lie-classes my happy-path tests missed: non-dividing `*/N` steps that reset at the hour/day wrap, and impossible calendar dates like "31st of April"; commit `6dd555c`). **Tier 2 increment 3 (2026-06-21) — DONE + live-verified (commit `c2571ba`):** D6 per-project nesting — jobs nest under their hero/spine card (deepest-enclosing root via `assignCronJobs`); the aggregated block holds only the remainder; D7 per-card "+ Schedule" (seeds project cwd); reusable `CronJobRow` extracted; spine "⏱ N" badge + expand-nesting. Verified via `duo dom` (subdir→hero nest, /tmp→aggregated, "+ Schedule" seeds cwd, Pause round-trips). **Tier 2 complete.** **Integrated** `origin/main` (commit `58953e9`) → PR #103 **MERGEABLE** (owner: #103 may merge before #102 if it wraps first). **Tier 3 — ENH-225 attention badge (2026-06-21) — DONE + live-verified (commit `7af4fcd`, on #103 per owner):** the F2 "waiting on you" tab badge. Duo-managed Stop/Notification (set) + UserPromptSubmit (clear) hooks → `duo attention` (keyed on a new `DUO_TAB` env stamp) → socket → transient per-tab flag → amber pulse dot in the tab strip (never on the active tab; clears on activity OR focus). `install-service` registers the hooks via the proven `_duo`-marker merge; works on cron's `kind:'shell'` tabs. New `duo attention` verb (4-surface synced). **Quality pass + smoke-walk (2026-06-21):** +12 ENH-225 unit tests (`planManagedHooksMerge` settings-merge + `attentionForEvent`); a multi-agent adversarial review of ENH-225+inc3 found+fixed 3 bugs (active-tab false-badge via `activeTabIdRef`; prune-on-close ×2). **Owner smoke-walk v0.11.2 — 3 PASS / 1 FAIL→resolved:** the badge FAIL was an install precondition (this dev never ran `installService.run()`), NOT a code defect — installed the managed hooks + **verified the real-Claude-`Stop` leg end-to-end** (fired a cron job in the trusted `duo` repo → background tab → Claude idle → amber dot lit; focus cleared it). Create-dialog polish from the walk (commit `95c845e`): Browse picker (new `dialog:pick-directory` IPC) + "Project (working directory)" relabel + interactive-only note. **Status: ✅ COMPLETE — Tier 1+2+3+ENH-225, reviewed, suite 1714, PR #103 MERGEABLE. Only the version cut remains** (can merge before/after #102). Reviewer brief: `docs/dev/enh-223-handoff.md`. **Priority:** owner-TBD (feature sprint). **Effort:** L. **PRD:** `docs/prd/enh-223-scheduled-sessions.md` (§9 = impl notes + live-walk record).

**Pre-merge review fixes (2026-06-21, this PR — applied before merge).** A multi-agent review + completeness critic surfaced two **missed requirements**, both fixed on the branch. **(a) Theme legibility (recurrence #3).** The cron dialog's error text was light-on-light in light mode (`NewCronJobModal.tsx:402` bare `text-red-300`; `:433` `bg-red-950/30 + text-red-200`) — the inverse of ENH-222's banner (`4475df8`) and #104's History legend (`dfc7593`). Fixed via theme-aware `color-mix` classes in `globals.css` (`duo-banner-{error,warn,ok,info}` + `duo-text-*`, light + `[data-theme="dark"]` split); per owner "fix all banners now" the **whole repo-wide banner family** was migrated (NewVaultModal · CloneModal · MarkdownEditor · PageTab · JsonView · SaveControl + the orphaned `text-fail` in HistoryModal that resolved to nothing). Tailwind `dark:` is unusable (no `darkMode` key). A **durable rule** was added to `.claude/rules/renderer-surfaces.md` ("Theme-legibility", both directions). **(b) CLI-currency.** `scripts/check-skill-currency.mjs`'s allow-list omitted `cron edit` (a silent 5th-surface drift — the green check was NOT evidence of `cron edit` coverage); added it + fixed two stale ENH-221→223 comments. Plus **new regression tests** (year/month rollover · overlap `firing` guard · catch-up collapse + boot-catch-up guard · command-quoting security · DST) and an **owner-flag**: spring-forward wall-times are **silently skipped** and D5 catch-up won't recover them (the DST test pins current behavior; accept-or-special-case TBD). Full detail: PRD §11.

**Tier 1 modules (built).** `core/cron-schedule.ts` (pure schedule math) · `core/cron-command.ts` (pure command-build + `-p` gate) · `core/cron-store.ts` (`cron-jobs.json` persistence) · `core/cron-service.ts` (Electron-free orchestrator: tick scheduler, catch-up, fresh/same decision, CLI dispatch) · wired into `electron/main.ts` (CronService lifecycle + runner via `dispatchNewTabToWindow` + D10 `resolveCronLandingWindow`), `core/socket-server.ts` (`case 'cron'` + `NavBridge.cron`), `cli/duo.ts` (`cron` cluster + `Scheduling` help group), `shared/types.ts` (`CronJob`/`CronJobsFile`/`CronSchedule`/`CronJobView`), `shared/feature-flags.ts` (`FEATURE_HEADLESS_CRON=false`). **D8 deviation (tracked):** self-contained cron engine instead of a dependency (offline/bundle-safe; swappable) — see PRD §9.
...
**Decision playground:** `docs/research/enh-223-scheduled-sessions.html` (decision-capture record). **Locked picks (2026-06-20):** D1 app-global `cron-jobs.json` · D2 Claude positional prompt · D3 pre-allocate uuid + resume, fresh-fallback · D4 validate-&-reject `-p` (flag default-off, not in UI) · D5 default skip / opt-in run-once-on-launch · D6 nested-under-project + aggregated "Scheduled" block · D7 Home/rail/**File ▸ New Scheduled Job…**/CLI entry points · D8 small cron-parser dep · D9 attention badge as sibling ENH-225 (Stop-hook→socket) · D10 run-landing = single-window-with-matching-active-project, else primary, else missed. CLI: `duo cron list|add|run|pause|resume|rm|show`.

**Logged sibling/future ENHs (not in scope):** **ENH-222** — system-scheduler (`launchd`) that *launches Duo* at a job's time (the path to unattended-ish without going headless). **ENH-225** — "waiting on user action" tab indicator (Stop/permission hook → socket; benefits all sessions). Headless `-p` runs and a full run-history view are explicitly deferred (the feature flag exists for the former).
...
### BUG-209: New Vault / Clone / History modals are occluded by the SPLIT-VIEW AUX WebContentsView (the F6 park covers only the main browser pane)

**Status:** ✅ FIXED 2026-06-20 (re-surfaced via the ENH-221 History modal in the v0.11.2 walk — owner: "the modal 'Close' button is non-functional/no-op; the 'x' works"; root cause was this occlusion: the footer Close sat under the un-parked aux WCV). Fix: [AuxBrowserSlot.tsx](renderer/components/AuxBrowserSlot.tsx) now listens for `duo-wcv-park` (→ `setAuxBounds(1×1)`) / `duo-wcv-restore` (→ re-send real bounds), mirroring [BrowserRenderer.tsx](renderer/components/BrowserRenderer.tsx). Covers ALL DOM modals (History, New Vault, Clone). **Verified live:** with a browser pinned in the aux + the History modal open, the aux WCV parks (content blanks) so the modal is fully visible and the footer Close is clickable; on close the aux content restores. **Priority (was):** P2. **Parent/related:** PR#98 F6, FOLLOWUP-025, ENH-216, ENH-191 (split view).
...
**Status:** 🚧 **Phase 0 (merged ⌘O Open bar) code-complete + agent-walked live (2026-06-21)** — PR [#102](https://github.com/dudgeon/duo/pull/102) (rebased on `main`, MERGEABLE; typecheck clean · 1676 tests · currency 75/75). **Renumbered ENH-221 → ENH-224** (collision with main's file-history ENH-221 #104). **3 Phase-0 follow-ups ✅ DONE + live-verified (2026-06-21):** FU1 CloneModal "Choose…" destination folder-picker · FU2 modal width/placement consistency (CloneModal now matches the Open bar — 640px + top-anchored) · FU3 ⌘O opens the bar from terminal focus (menu accelerator) — see PRD § 6c. *Deferred from FU2:* full-inline modal merge (D15/DM1) + NewVaultModal geometry audit. **DR1–DR6 resolved (2026-06-21)** — DR6 = depth-1 managed checkout (sparse deferred), DR2 = always-ask. **Phase 1 ("open just this doc") ✅ DONE + live-verified (2026-06-21):** a github-file URL pulls into the opaque managed checkout (`~/.claude/duo/checkouts/`) + opens like a local file + records the recent (Spoon-Knife/README.md walked). **Phase 1 CLI twin (`duo open <github-url>` → same checkout) ✅ DONE + tested (2026-06-21):** socket `open` branch classifies github-file targets → `NavBridge.runManagedCheckout` (shares the UI IPC's checkout engine) → `nav.edit` + `nav.reveal` + records the recent; auth-missing bounces to `gh auth login`; bare-host CLI resolve added; 1696 tests · currency 75/75 · 5-surface doc sync. ✅ **live-verified** (`duo open` Spoon-Knife/README → editor + folder focus + canonical recent). **Phase 2 (share-back → PR) — CORE PLUMBING 🚧 BUILT + tested (2026-06-21):** net-new git-WRITE core (`core/git/{divergence,proposal-meta,branch,commit,push,fork,pr,share-back}.ts`) + `duo pr create|status|view` (socket `case 'pr'` → core, like `clone`) + 5-surface sync; `runShareBack` = context→divergence→branch→commit→push-access→**auto-fork (D3)**→push→create-or-update PR (D13), all live via gh/git (§12), D4-guarded to `~/.claude/duo/checkouts/`. 42 pure tests · 1738 total · currency 76/76. **Phase 2 UI affordance (D10–D13) 🚧 BUILT (blind — no Electron):** `renderer/components/ProposeBar.tsx` (footer bar + confirm sheet + morph) mounts under the markdown editor, polls SHARE_BACK_STATUS on save (inert for ordinary files), confirm sheet → SHARE_BACK_CREATE → "Proposed · View PR"; new IPC → the same `core/git/share-back` engine. typecheck clean · prod bundle compiles · 1744 tests. **✅ FULLY LIVE-VERIFIED (2026-06-21, owner granted Electron):** walked the whole driving use case against octocat/Spoon-Knife → open → edit/save → footer appears → confirm sheet (heading title + `duo/…` branch + real diff) → `duo pr create` → **auto-fork + cross-fork PR [#40238](https://github.com/octocat/Spoon-Knife/pull/40238)** (D3) → footer morphs to "Proposed · View PR" (D13). Live testing caught + fixed a real bug (status-gate `16a23b7` — fresh checkout no longer shows a stranger's head:main PR). **Phase 3 (already-local detection D6) + Phase 4 (format breadth D8 + escape hatch D4) ✅ BUILT + LIVE-VERIFIED (2026-06-21, owner "build all phases"):** P3 = `matchLocalClone` over navigator git-roots → open from the user's existing clone (UI offers a 3rd choice; CLI prefers-and-reports, `--checkout` overrides; never edits their tree) — walked `duo open dudgeon/duo/blob/main/CLAUDE.md` → `via:local-clone` opened the LOCAL file; P4 = `ProposeBar` footer now also in the JSON/YAML viewer + HTML canvas (footer rendered+morphed live in both) + `duo pr export <path> <dest>` (D4 "save a local copy", refuses to clobber) + binary view-only. 1750 tests · currency 76/76 · prod bundle compiles · adversarial-review-hardened (`2074c8d`). UI "Save a copy…" affordance deferred (owner UX choice). **ALL 5 PHASES BUILT + LIVE-VERIFIED — ENH-224 DONE.** **Owner smoke walk-1 (2026-06-21): 3 PASS / 2 FAIL / 1 SKIP — both FAILs were a WALK-FIXTURE DEFECT, not feature bugs.** PASS: Open-bar choices · Clone-success theme · Cron/attention. FAIL: Propose footer+sheet · Footer-in-JSON/canvas — **both because the manifest told the owner to open a no-extension `octocat/Hello-World/blob/master/README`, which Duo's classifier can't open** (owner notes: *"README has no extension so Duo cannot open it"* / *"cannot edit file; see prior step"*). The features themselves were agent-live-verified against Spoon-Knife earlier the same day. SKIP: Morph (deferred). **Re-walk queued** at `docs/dev/smoke-walks/v0.11.2-enh224-rev2.{json,html}` with corrected `.md` fixtures + the fixed Copy button. **Process fix landed (separate from ENH-224):** the recurring broken-Copy-button + bad-fixture classes are now defended in the worksheet generator (always-surface a pre-selected textarea), `generate.test.ts` regression tests, the smoke-walk SKILL HARD RULES, and a memory — so neither recurs. **Owner smoke walk-2 (2026-06-21, rev2): 2 PASS / 1 FAIL.** PASS: footer+sheet (✅ + spawned [ENH-227](docs/prd/enh-224-file-open-flow.md) — "show PR success + a working link"), **Copy button (✅ owner-confirmed FIXED — the original thread's complaint).** FAIL: FOOTER-FORMATS again — AGAIN my fixture defect (a non-existent `.github/FUNDING.yml`), and the HTML verb was wrong (`duo open` = browser mode, no footer; needs `duo edit` = canvas). **Root-cause-of-the-root-cause fix:** added a **mechanical fixture guard** ([.claude/skills/smoke-walk/verify-fixtures.mjs](.claude/skills/smoke-walk/verify-fixtures.mjs), 7 unit tests) that runs every `duo open|edit` in a manifest against the live app and exits non-zero on any that won't open — the smoke-walk SKILL now MANDATES a clean guard run before handoff (a prose rule kept getting skipped; a failing script can't be). **Walk-3 (rev3) queued + guard-passed:** FOOTER-FORMATS (corrected `duo edit …/index.html` canvas verb) + ENH-227. All footer surfaces (markdown/canvas/JSON) + ENH-227 success+link live-verified by agent. **Owner ⌘O-autofocus request → ENH-226.** **Priority:** P1 (differentiating collaboration flow — turns Duo from "IDE I drive" into "easiest way to co-edit a doc"). **Effort:** L (multi-phase). **PRD:** [docs/prd/enh-224-file-open-flow.md](docs/prd/enh-224-file-open-flow.md) (§ 6a status · § 6b change log · § 6c follow-ups).
...
**Locked (AUQ 2026-06-19).** D1 unified ⌘O Open bar (path OR url; CLI twin = extended `duo open`) · D2 Save=local, explicit affordance on divergence-from-baseline · D3 auto-fork → cross-fork PR when no push access · D4 opaque managed checkout under `~/.claude/duo/checkouts/` · D5 depth-1 shallow whole-repo clone (assets/siblings/real diff) · D6 detect+offer-reuse of an existing local clone (else managed) · D7 prefilled one-tap editable PR confirm sheet · D8 markdown round-trip first, machinery extensible (json/yaml/html), binary view-only · D9 lean on `gh` auth.

**Locked (UI-study walk + owner, 2026-06-20).** D10 affordance = **footer "propose" bar** (OQ-1 resolved; zero toolbar/Save-pill/tab-strip impact) · D11 label = **"Propose changes"** (PR jargon only in the confirm sheet + "View PR"; avoids colliding with Duo's existing Suggesting mode) · D12 confirm sheet = title + branch + fork-note + **inline diff** · D13 post-PR = **morph in place** (View PR / Update PR) · **D14 Open Recent** — persist last ~10 Open-bar targets (local + GitHub), machine-global, File ▸ Open Recent + empty-Open-bar list; pointers only, self-healing; CLI twin `duo recent`. UI study: [docs/research/file-open-submit-pr-ui.html](docs/research/file-open-submit-pr-ui.html).

**Net-new plumbing.** `core/git/{branch,commit,push,fork,pr}.ts` (today's git support is read-only — clone/auth/status/worktree, NO write path), `core/open/{resolve,checkout}.ts`, Open-bar UI + Open Recent (recents store), "Propose changes" footer affordance, `duo pr create|status|view` + `duo recent` verbs.
