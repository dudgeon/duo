...
**Status:** 🟡 **Decisions OPEN — rev 5 (2026-07-09) supersedes the rev-4 decision set. Walk [`docs/research/agent-agnostic-duo-v2.html`](research/agent-agnostic-duo-v2.html) (11 cards, D1–D11)**; the rev-4 playground ([`agent-agnostic-duo.html`](research/agent-agnostic-duo.html), D1–D7, never walked) is folded into it and kept for its mockups (§5a/5b/5d). Surfaces in every smoke walk until the owner walks the v2 playground and pastes back the decision set (rule 11 + research-report-review-task rule). **Priority:** Strategic / owner-decision-gated. **Effort:** research only; implementation scope depends on v2 D1/D11.

**Rev 5 — the runtime audit (2026-07-09, branch `claude/duo-agent-agnostic-review-aebd6b`).** Owner ask: rev 4 focused on repo docs/skills but "did not catalogue the myriad ways duo features presume Claude Code" at runtime (session storage, hooks, presence, etc.). Six parallel code audits at v0.13.7-dev re-catalogued the whole tree: **46 consolidated touchpoints** (rev 4 had 19), full file:line evidence in [`docs/research/agent-agnostic-audit-2026-07-appendix.md`](research/agent-agnostic-audit-2026-07-appendix.md). Headline findings: (1) **the coupling deepened after rev 4** — Home Projects rollup (ENH-212), the Catch-Up board + session digests (ENH-231), and scheduled sessions (ENH-223) are all built on Claude's *private* surfaces (JSONL transcript schema, `comm==='claude'`, `--session-id/--resume`); (2) the dominant failure mode is **silent no-op** — a non-Claude user gets an empty Home/board, dead presence/attention/resume, cron resume silently downgrading to fresh, a close-guard that stops protecting, a doctor false-red, and an agent tab that types `claude` into their shell — with zero errors; (3) tree-wide grep for `~/.codex` / `~/.gemini` / `~/.agents/skills` = **0 hits** — no non-Claude install path exists; the "universal fallback" (managed `~/.claude/CLAUDE.md` block) is itself Claude-only; (4) the neutral core is confirmed (all ~100 `duo` verbs, vault engine, PTY/env layer) and the functional coupling concentrates in ~5 single-point seams (presence string, resume builder, spawn payload, tab-kind enum, install-dest constants) — well-shaped for a data-driven `AgentProfile` registry; (5) new strategic option: a **Duo-owned PTY observation layer** (Duo already sees every byte/cwd/process in its terminals) can feed Home/attention/labels agent-agnostically, demoting the private-JSONL reader from foundation to Claude-only enricher — which also de-risks Claude transcript-format drift. v2 recommends: honesty pass (T1) → AgentProfile seams (T2) → observation layer (T3), per-agent private-store readers avoided (unchanged burden law).

**v2 recommended throughline (owner to confirm):** D1=B (honesty + profile seams) · D2=A (data-driven registry) · D3=A (`kind:'agent'`+`agentId`, migrate) · D4=B (profile presence + idle-heuristic attention fallback) · D5=B (Duo-owned observation layer; JSONL becomes enricher) · D6=A (skills-standard-first priming) · D7=A (dual-write `~/.agents/skills/`) · D8=A (AGENTS.md joins markers/wash) · D9=A (storage root stays, revisit at major) · D10=A (full user-facing copy sweep) · D11=A (T1 next sprint, T2 after).
