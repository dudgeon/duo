...
- ~~D1-part2 Thread A~~ ✅ **implemented (commit f6fe51a):** "open worktree in new window" via the dropdown button + `duo window new --cwd <path>` (initialCwd threaded CLI→main→preload→useNavigator with forceInitial over stale per-window LS). Verified: windows rooted at enh-208-vault (CLI) + distracted-kilby (button). **Thread B (titlebar hue+branch chip) remains split out + owner-gated** (undecided scope — needs an explicit yes).
