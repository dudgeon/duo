...
**Status:** 🚧 In progress — implementation landed across all 11 PRD steps (data primitives, snapshot service, open-session join, IPC contract, renderer `Home/` surface, App wiring, PinnedNav slot 0, CLI `home` / `session open` / `term tab` verbs, docs sync, vocab/tracker). **Typecheck + 1283 unit tests green; awaiting live verification + `/smoke-walk` (PRD § 5 step 11 — the focus/resume/cross-window legs and confirm-gate have NOT yet been exercised against a running app).** PRD at [docs/prd/enh-212-home.md](docs/prd/enh-212-home.md). Decision trail: three studies ([round 1](docs/research/home-screen-study.html) → [round 2](docs/research/home-screen-study-v2.html), #13 Two-Up Spread won → [round 3 variations](docs/research/home-screen-study-13.html)) + AUQ locks (naming "Home", activate-only-when-nothing-restored, spine fold after ~8, os.userInfo greeting). Architecture from ultracode workflow wf_27e8ee52-257 (3 competing designs, judged, 3 adversarial verifiers — D9 mtime-heuristic ban, Pin/Move-to-split persistence leaks, exponential JSONL tail reads, PR #75/#91/#93 merge anchors all folded into the PRD). **Priority:** P1. **Effort:** L. **Filed:** 2026-06-12.
