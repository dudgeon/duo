...
**Status:** 🚧 In-progress 2026-06-23 (P0–P7 **built** on `claude/async-catchup-home-view-657v0x` / PR #108; typecheck clean, 2008 tests green incl. the §D9 rebuild gate + the BUG-046 fetch gate; **awaiting live smoke-walk → cut**). **Priority:** P2. **Effort:** L. **PRD:** [docs/prd/enh-231-async-catchup-home.md](docs/prd/enh-231-async-catchup-home.md). **Implementation plan:** [docs/prd/enh-231-implementation-plan.md](docs/prd/enh-231-implementation-plan.md) (phased P0–P7, file:line anchors, 16 adversarial-review fixes applied). **Decision playgrounds:** [async-catchup-home.html](docs/research/async-catchup-home.html) · […-directions.html](docs/research/async-catchup-home-directions.html) · [async-catchup-board.html](docs/research/async-catchup-board.html) (PR #108). **v1 = full build in one PR.** **Extends:** ENH-212 (Home re-entry surface), ENH-225 (attention hooks), the `claude-session-tracker.ts` JSONL reader. **Ticket note:** allocated above this branch's committed max (ENH-230); siblings ≤ ENH-230 — renumber on collision.

**Build progress (commits on PR #108).** P0 types/channels + P1 deterministic digest extractor (`electron/session-digest.ts`) + the two §D9 stores (`core/session-digest-store.ts` rebuildable cache · `core/home-state-store.ts` Duo-owned annotations) `edfdb53`. **JSONL shapes confirmed against live transcripts** (TodoWrite/ExitPlanMode/`toolUseResult`/PR — all match). P2 Stop-hook digest trigger (`duo-attention.sh` fires `duo session digest`, no new hook entry) `9bb5acf`. P3 catch-up assembly (`buildCatchupSnapshot`) + the `sessionIdForTab` tab→uuid primitive + `HOME_CATCHUP`/`HOME_MODE_*`/`SESSION_DIGEST` IPC + `HOME_MODE_PUSH` all-window fan-out `6d70875`. P5 CLI sub-ops (`session digest|note|next` · `home mode|catchup`) + 4-surface sync + the mandatory `check-skill-currency.mjs` SUBCOMMANDS edit `0decb91`. P4 the renderer board (ModeToggle · CatchupBoard · SessionDigestCard · CompactSessionRow, theme-var-only) `34c465f`. P6 cron `scheduled` badge `cd05b02`.

**Live-walk fixes (owner, 2026-06-24).** (1) Goal ladder — title/summary → recap → `/command` → first prompt (was raw first prompt → machinery) `b78a2f3`. (2) Card previews — markdown-stripped + 2-line clamp (was dumping ` ```bash ` fences + the whole last turn) `90a4491`. (3) **Done-IA fix** `e32513f` — Done meant "inactive"; now splits **finished** (PR opened / completed plan / produced doc) as full cards vs **stalled** (compact), per the prototype's actual signal table; tightened PR detection (real `gh pr create`/mcp result, not a mentioned URL) + dropped generic-file-edits/tests from the completion signal (live-data over-marking). PRD D6a documents it.

**Parity-audit P0s (owner walk #2, 2026-06-24).** Owner ran a parity worksheet (`docs/dev/worksheets/enh-231-parity.json`) prioritizing prior-Home affordances. Shipped the 5 P0s in one pass: **(1+2) live/running pills** — the green "live"(duo→focus) / amber "running"(external→fork) pill is back, with the primary action labelled "Focus session →" for a Duo-hosted session (root cause of "offered to fork a running session": the session was genuinely external — `DUO_SESSION` empty — and catch-up just wasn't surfacing duo-vs-external like Projects Home does). **(3) worktree-gone** → struck through + "⚠ worktree removed" + a disabled re-entry (no more dead-end `claude --resume … No conversation found`); rich recovery is ENH-232. **(4) file chips** are now clickable → open in Duo. **(5) compact rows** expand in place into the full card (+ ✓ done / ⏸ stalled badges). Verified live via DOM probes; tests added. P1 queue: done-badge (shipped alongside), title ladder, worktree subpath badge, dismiss (ENH-233). P2: cron management. SKIP: greeting, empty-state.
...
**Status:** 🚧 In-progress 2026-06-23 (this worktree). **Priority:** P2 (closes the in-vault arm of ENH-229's problem statement). **Effort:** M. **PRD:** [docs/prd/enh-230-okf-listing-convergence.md](docs/prd/enh-230-okf-listing-convergence.md). **Depends on / extends:** ENH-229 (portable `duo rollup` artifact), ENH-216 D8 (OKF static listings). **Ticket note:** allocated above this worktree's committed max (ENH-229); siblings ≤ ENH-229 — renumber on collision.

**Provenance.** Owner (2026-06-23), reviewing the Obsidian-vs-OKF rollup asymmetry: *"the rollup mechanism for obsidian vs OKF vaults is different… is this true? What are some pragmatic options that reduce overall complexity (and don't just move it)… optimize for vaults being expressive, flexible, and non-brittle."* Investigation confirmed the asymmetry is **not** in querying — `core/vault/engine.ts` + `render.ts` are format-agnostic and `parseLinkish` already folds OKF rel-md links — but in `core/vault/listings.ts`, a **parallel, less-expressive** generator (`generateIndex` groups by `type` only) that shares nothing with the engine. The intrinsic split is *live-at-rest (Obsidian, via its app) vs materialized-at-rest (OKF, plain md)*; Obsidian's `.base` gives Duo a *file convention*, not rendering infra (Duo wrote `engine.ts` from scratch). Owner accepted the impact analysis and said *"document the feature and then build it."*

**The change.** Make the OKF root `index.md` listing **query-driven through the one shared engine**, opt-in and back-compatible:
- An OKF root `index.md` may carry a **`listing:` base spec in its frontmatter** (same schema as a `.base`: `filters` / `formulas` / `views`). When present, `duo vault publish` evaluates it via `evaluateBaseDef` + serializes through the existing `render-markdown.ts`, splicing the markdown below the `<!-- duo:listing -->` fence. The owner's real shape ("tasks grouped by initiative, status chips, each row links its note") becomes expressible **in-vault**, not just in a portable `out/` artifact.
- **No `listing:` spec → unchanged.** Falls back to `generateIndex` (today's group-by-`type` bullets) byte-for-byte. The frontmatter byte-preserve splice contract is **untouched** (spec lives in frontmatter, which was already preserved verbatim), so existing vaults publish identically until someone authors a spec.
- `log.md` stays bespoke (date-grouped by mtime — not a natural frontmatter `groupBy`).
- The generated stamp gains a `· regenerate: duo vault publish` hint (the markdown analog of the rollup artifact's stamp; one-time stamp-line rewrite on next publish, idempotent after).
- Malformed `listing:` spec → **warn-and-render** (engine emits ⚠ cells, never throws; an unparseable spec falls back to `generateIndex` so publish never breaks). This is the one new failure mode, and it only exists once a user opts in (D15-consistent).

**Acceptance.** (1) Existing OKF vault with no `listing:` → `duo vault publish` output byte-identical to pre-change (golden test). (2) A vault whose `index.md` frontmatter carries a `listing:` base spec → `index.md` body is the engine-rendered listing (grouped, chipped, with rel-md entity links resolving to notes). (3) Obsidian vaults still throw from `writeListings` (frozen-fixture invariant). (4) Idempotent re-publish writes nothing. (5) `npm run build:cli` + `sync:claude` + typecheck + vault tests green; `skill/references/vault.md` + `.claude/rules/vault.md` document the `listing:` spec.
...
**Status:** ✅ **Shipped v0.12.0 (2026-06-23)** (MD/HTML variants + entity links + `duo rollup render`/`diff`; change summary on regenerate; `--style`; refresh-from-MD; skill `references/rollup.md` + vault-guide ch9 + sample prompt). **Owner verifies the Markdown in-editor refresh click on the work machine via the DMG (smoke-walk waived).** Commits `3d246ad`, `b4c2188` (P1 + review), `53853c8`, `04b58a9` (P2 + comment-safety), `151912e` (P2 review fixes). 219/219 vault tests, typecheck clean, `check:skill-currency` PASS (79 verbs), `sync:claude`; P1 + P2 each adversarially reviewed **and the findings fixed** (removed-view diff, multi-base view collision, `--style` `</style>` breakout, `--summary` flag-guard, format-switch history portability, the req #4 Copy/Refresh toolbar in the real HTML artifact); live-verified end-to-end (render→diff→summary→history→no-summary→style; OKF rel-md links resolve). **Fast-follow done (`c9af1df`):** refresh-from-Markdown — MD artifacts carry a `[↻ Refresh](duo://rollup/refresh?base=…)` link; MarkdownEditor allows the `duo:` protocol + intercepts it on a plain click → emits `rollup:refresh {base}` (same as the HTML button; payloads unified — fixed a real bug where the HTML button's bare-string data-payload would've been rejected). **Owner reqs (2026-06-23):** rollups now default to `<vault>/rollups/` (added to SKIP_DIRS + sourceHash SKIP — no corpus pollution, verified) + every artifact opens with an **agent-visible doc comment** explaining it's a generated rollup. **Remaining to verify:** the in-editor click→event flow + TipTap `duo:` protocol survival need a **live editor walk** (smoke-walk) — renderer change, not headlessly verifiable. Not cut/merged. **Priority:** P2. **Effort:** L. **PRD:** [docs/prd/enh-229-rollup-maturity.md](docs/prd/enh-229-rollup-maturity.md). **Depends on:** ENH-228. **Ticket note:** allocated above this worktree's committed max (ENH-228); siblings ≤ ENH-228 — renumber on collision.
...
**Status:** ✅ **Shipped v0.12.0 (2026-06-23)** (docs/skill/agent — `check:skill-currency` PASS, `sync:claude` run, synced copies verified; cut + signed DMG). **Priority:** P2 (a using-agent that can't find a shipped feature is a silent product regression). **Effort:** S. **Ticket note:** allocated above this worktree's committed max (ENH-227); siblings sit ≤ ENH-227 — renumber if a concurrent agent collides.

**Provenance.** Owner (2026-06-22): *"my agent at work does not understand the full graphbook featureset — e.g. making rollup views of `class:task`."* Audit found the vault/graphbook feature (internal: `core/vault/**`, the `duo vault` / `graph` / `base` verbs) **is** documented thoroughly (`skill/references/vault.md`, `agents/duo.md`, `docs/CLI-COVERAGE.md`, `docs/guide/vault-guide.html`, the PRD + intent doc) — but is effectively **invisible to a *using*-agent** because the routing fails at the top:

- **Gap 1 — the skill never triggers.** `skill/SKILL.md`'s `description:` lists browser/editor/navigator/canvas/Google-Docs/Figma/Notion/page/playground — but says nothing about notes, vault, knowledge graph, rollups, or tasks. A work agent asked "roll up my tasks" has zero signal to invoke Duo. Same for `agents/duo.md`'s examples. **#1 cause.**
- **Gap 2 — depth is two hops away.** Even once SKILL.md loads, rollup mechanics live in `references/vault.md` behind a pointer.
- **Gap 3 — CLAUDE.md is silent.** A repo-dev session gets no orientation — vault absent from "Where to look", the glossary, and there's no path-scoped rule for `core/vault/**`.
- **Gap 4 — vocabulary mismatch.** The corpus keys typing on **`type:`** frontmatter (`core/vault/corpus.ts:34`), not `class:`; and there are TWO rollup mechanisms by format (`.base` views in Obsidian mode vs static listings via `duo vault publish` in OKF mode).

**Scope (this PR — a/b/c/d/e):**
- **(a)** Expand the `description:` triggers in `skill/SKILL.md` + `agents/duo.md` to cover vault / notes / knowledge-graph / rollup / by-type queries.
- **(b)** Add a graphbook/vault row to CLAUDE.md "Where to look" + glossary, and a new `.claude/rules/vault.md` path-scoped rule for `core/vault/**`.
- **(c)** Add a terminology + mechanism note (`class` → `type`; `.base` vs static listings by format) near the top of `skill/references/vault.md`.
- **(d)** Promote the "Authoring a rollup — the loop" into the SKILL.md body (one hop, not two), keeping the deep `references/vault.md` pointer for full detail.
- **(e)** Fix the Vault Guide Ch 9 ("Generating a rollup") — it taught the **Obsidian `.base` path only**, with no OKF branch, so an agent in a (default) OKF vault concluded the vault had *no rollup primitive*. Added a mode gate + the OKF `duo vault publish` path + an honest "richer view via a rendered `.base` works in OKF too (render isn't mode-gated)" note. Also fixed the misleading footer ("Phase 3 brings rollups in-app" → "rollups ship today; Phase 3 adds in-app *live* re-render").
- **(f)** **Ship the Vault Guide.** It lived at `docs/guide/vault-guide.html` — but the DMG's `extraResources` ships only `skill/` + `cli/` + `agents/`, NOT `docs/`, so `duo open docs/guide/vault-guide.html` was a **dead pointer for every installed (non-repo) user**. Moved it (`git mv`) to `skill/references/vault-guide.html` so it rides the existing skill-install path → `~/.claude/skills/duo/references/vault-guide.html`; `sync:claude`'s `cp skill/references/*` carries it with zero new wiring. Repointed every live reference (vault.md, SKILL.md, vocabulary.md, CLAUDE.md, the guide's own footer, `what-duo-does.html`, the PRD) to the installed path (agent-facing) or repo path (dev-facing). Bumped `packs/duo-default/PACK.json` 1.0.27→1.0.28 so existing users re-receive the corrected `what-duo-does.html`.

**Implementation soundness — verified live (2026-06-22), NOT half-baked.** Owner worried the rollup featureset was incomplete. Checked empirically: ~1,400 lines of real code in `core/vault/{render,engine,listings,corpus}.ts` (zero stub/TODO markers); **98/98 vault tests pass**; and a throwaway OKF vault exercised both paths end-to-end — `duo vault publish` generated a real `## Task` listing (GitHub-portable, hash-stamped), and `duo base lint`+`render` produced a 4 KB HTML artifact (1 view, 3 rows grouped by status) confirming the `.base` path is NOT mode-gated. The only genuinely-deferred piece is **in-app *live* auto-re-render** (re-render-on-focus = fast-follow; file-watcher = deferred, by design). The feature shipped (ENH-208 + ENH-216); the failure was discoverability/docs, not the implementation.

**Confirmed by a real 0.11.2 session (2026-06-22, owner screenshot — vault "brainkit-gd", an OKF vault).** A Sonnet-4.6 agent asked to build a tasks rollup: (1) first declared *"There's no 'graphbook' concept in the Duo docs — that term isn't in the Duo codebase at all"* (→ Gap 1, fixed by a/b/c); (2) handed `~/Downloads/vault-guide.html`, correctly identified the OKF vault but concluded *"no `.base` file, no `duo vault` integration set up"* and offered only keep-the-manual-table / author-a-`.base` / hybrid — **never mentioning `duo vault publish`** (→ the Ch 9 defect, fixed by e); (3) cited the footer's "Phase 3 … not here yet" to argue rollups aren't shipped (→ fixed by e). The agent got `type: task` right, so (c)'s field-name trap didn't bite here — the discoverability + Ch 9 gaps were the actual failure. **Distribution:** the work machine read a *downloaded* `vault-guide.html`; the repo fix reaches it only on re-download / Duo update.

**Acceptance.** `npm run check:skill-currency` still passes (no verb set changed — descriptions only). `npm run sync:claude` run after the skill/agent edits. A fresh using-agent reading only SKILL.md can author + render a `type: task` rollup without opening `references/vault.md`. **Distribution note:** the owner's work machine picks this up on its next Duo sync / managed-install upgrade — flag if a faster path is wanted.
