...
**Status:** ⏳ Open — decisions locked (owner, 2026-06-24); PRD written; build not started. **Priority:** TBD (owner-requested). **Effort:** M (view) + S (rollup-lifecycle change — MVP). **Filed** 2026-06-24. **Playground:** [`docs/research/vault-inbox-rollups-view.html`](docs/research/vault-inbox-rollups-view.html) (decision artifact, rule 11). **PRD:** [`docs/prd/enh-228-vault-view.md`](docs/prd/enh-228-vault-view.md).

**Decisions locked (owner, 2026-06-24 — MVP path; overrode 2 recs).** **D1 discovery = scan `rollups/`** (no `type:rollup`, no manifest/sidecar) via a new `duo rollup list` verb (live scan, §D9-clean). **D2 lifecycle change = codify `rollups/` as the canonical rollup home** in `skill/references/vault.md` + `skill/references/rollup.md` + `agents/duo.md` (steer authoring through `duo rollup render`, default `--out rollups/`; reserve `out/` for transient `base render`) — this is what makes the scan authoritative (owner: *"as long as the agent/skill will know to make rollups in the rollups/ dir"*). **D3 view subject = the active file's enclosing vault** (`findVaultRootAndMode`; sticky-last-vault to avoid tab flicker — confirm in walk), NOT the default vault. **D4 "+ New rollup" = prefilled Claude session** (authoring-loop prompt). **D5 inbox = all notes, stale>1wk flagged.** **D6 first slice = whole view + scan discovery.** Build order in the PRD § B.
