...
**Live verification (2026-06-11, fresh dev on this worktree, `duo eval` MutationObserver — the sub-frame flash a screenshot can't catch).** Observer teeth proven (catches an injected `Loading…`). Burst-add 10 files in cwd → **0 Loading flashes**, all 10 rows render in place; burst-delete → **0 flashes**. M3 probe: modifying a TRACKED file in a git repo → **0 Loading flashes AND 0 navigator DOM mutations** — the git-tick re-render (M3) reconciles to identical DOM, so it is **wasted CPU, not visible flicker**. Conclusion: P0 resolves the reported flicker; **P1 (D3 watcher churn, D4 row memo, D5 git-Map guard) is a perf/efficiency follow-up, not a visible-flicker fix — safe to defer.** (Separately noted during the walk: a `MaxListenersExceededWarning: 11 files:changed listeners` after heavy HMR — a watch-accumulation smell D3 would also address; filed mentally as P1 motivation, not a flicker cause.)
