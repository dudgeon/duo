...
**Status:** 🟡 PR #124 open (branch `claude/zealous-germain-541cec`), awaiting UI walk 2026-07-06 — fix implemented + core logic live-verified, interactive Rollups-tab gesture (Enter/Escape/inline-error) not yet walked in the running app (computer-use access was denied this session; see § Verification below). **Priority:** P2 (data-integrity — the note's on-disk name silently drifts from its displayed title, and ENH-243's own § 12 G6 gap-list already assumed rename "exists (Title field)" — it doesn't). **Ticket note:** allocated after grepping sibling worktrees (max was BUG-252).
...
**Verification.** `core/vault/builder.test.ts` covers `saveRollupNoteWithRename` (same-slug no-op, available rename, taken-name rejection leaves the original byte-untouched, artifact cleanup, inbound-link rewrite) — full suite 2178/2178 green, typecheck clean, `check:skill-currency` PASS. Live end-to-end via a scratch vault + `duo rollup set --title` (the CLI twin, same core function the GUI calls): rename-to-available landed on disk, rename-to-taken rejected with a clear message + exit 1 + both notes byte-untouched. **Not yet walked:** the Rollups tab's actual Enter/Escape/blur/inline-error interaction — computer-use access to Electron was denied this session, and no CLI verb currently switches a window to the Rollups top-level tab (click-only today), so it couldn't be driven headlessly either. Needs an owner walk or a future session with Electron access before flipping to Shipped.
