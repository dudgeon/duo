...
- **[P1, UI] History view modal.** **DECIDED 2026-06-20:** surface = **B · modal**; restore = **overwrite-with-confirm**; rows = **standard** (time · who · Δsize); diff = **rendered inline-highlight**. Decision playground: [docs/research/enh-221-history-view.html](docs/research/enh-221-history-view.html).
  ...
  - **⏳ STILL OWED:** **WCV-occlusion edge case** — the modal dispatches `duo-wcv-park` (handles the main browser pane), but the **split-view aux** browser WCV isn't parked (BUG-209 class). Verify the modal isn't occluded when a browser is pinned in the aux. Also: real-click on a SECONDARY monitor doesn't map via computer-use (drove via DOM) — a real-click smoke needs the window on the primary display.
