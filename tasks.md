...
- **[P1, UI] History view modal.** **DECIDED 2026-06-20:** surface = **B · modal**; restore = **overwrite-with-confirm**; rows = **standard** (time · who · Δsize); diff = rendered inline-highlight (deferred — see below). Decision playground: [docs/research/enh-221-history-view.html](docs/research/enh-221-history-view.html).
  - **✅ LANDED (DOM-verified, 2026-06-20):** [HistoryModal.tsx](renderer/components/editor/HistoryModal.tsx) + toolbar `◷` button ([EditorToolbar.tsx](renderer/components/editor/EditorToolbar.tsx)) + wiring in [MarkdownEditor.tsx](renderer/components/editor/MarkdownEditor.tsx), on the `window.electron.history.*` bridge. Parks the browser WCV while open. **Verified live end-to-end via `duo dom`:** opens against the active editor, timeline renders newest-first (time · You · Δsize), preview shows the selected version, Restore disabled on newest / enabled on older, two-step confirm, restore reverts the file on disk AND the open editor reconciles (criterion D), modal closes, a `restore` snapshot is captured. typecheck clean, 1605/1605.
  - **⏳ STILL OWED:** **VISUAL verification** — computer-use can't reach the dev Electron (it runs as "Electron", unregisterable), so I verified behavior at the DOM level but NOT appearance. Owner/visual pass must confirm: layout/styling against the Atelier aesthetic, and especially **WCV occlusion** (does the modal truly paint above the browser pane — the BUG-153/209 class). **Then `/smoke-walk`.**
  - **Deferred follow-ups:** rendered inline-highlight diff in the preview pane (read-only TipTap reusing ENH-197 `applyTrackedDiff`; v1 ships a plain-content preview); save-chip popover (D) for one-click quick-restore; **canvas parity** — the HTML canvas also saves through `FilesService.write` (so it's captured), but has no History modal yet → mirror-port is **(c) Deferred** per the renderer-surfaces parity rule.
