...
**Status:** 🚧 In progress — **P0 (D1+D2) implemented + unit-verified** on `claude/hungry-yalow-55f5b7` (commit `10cdd26`); **live sub-frame-flash smoke-walk owed** before any cut, and **P1 (D3/D4/D5) not started**. **Priority:** **High** (P0 user-visible jank on the single most-used pane — the navigator visibly flashes whenever an agent writes files, the exact scenario Duo exists for). **Effort:** S–M (P0 was a self-contained two-hook edit; P1 is incremental). **Sequenced after ENH-210** (worktree-aware Duo, sibling `claude/youthful-chebyshev-885712`) — rebase onto post-ENH-210 `main`; every code surface is disjoint (see PRD § C).
