...
**Status:** 🆕 Filed 2026-09-16 — **root-caused by static trace of `main` @ `1a6deb4`; no code changed, nothing executed** (owner asked for diagnosis + proposals only). **Priority:** P0 (app-unusable render loop; recurring owner report across ≥3 sessions). **Effort:** S to unblock (merge the existing fix) · M for the structural guard. **Ticket note:** `267` is triple-booked (BUG-267 = this bug in open [PR #137](https://github.com/dudgeon/duo/pull/137); ENH-267 = titlebar vault chip in open PR #136) and `268` is held by branch `claude/duo-ui-floating-search-f0ae83`, so this entry takes **269**. **When PR #137 merges, rename its `### BUG-267` heading to BUG-269 and fold it into this entry** — one bug, one ledger entry.
...
**Related.** [BUG-267 / PR #137](https://github.com/dudgeon/duo/pull/137) (the unmerged fix; same bug), BUG-192 (close-path sibling; its audit scope explains the miss), BUG-193 (D11 focus theft, same adjudication family), BUG-194 / ENH-204 / BUG-161 (release-to-All escapes that each added a writer), BUG-191 (frozen launch cwd — the F4 hole is the same design), FOLLOWUP-030 (browser-side face of the same cycle, also gated by PR #137), ENH-182 PRD D8/D11/D12.
