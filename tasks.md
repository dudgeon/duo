...
### ENH-246: Vault — rename the rendered-artifact folder `out/` → `output/` (dual convention, same pattern as ENH-245)

**Status:** ✅ Shipped (this branch, 2026-07-02). **Priority:** P2 (owner request, same session as ENH-245). **Ticket note:** filed as ENH-244, renumbered to ENH-246 on rebase — PR #119 (`claude/magical-shtern-8b7bc7`) independently claimed ENH-243 for the Rollups tab; grepping sibling worktrees again post-rebase put 245/246 next-available.
...
### ENH-245: OKF vault — dual `_index.md`/`index.md` + `_log.md`/`log.md` convention

**Status:** ✅ Shipped (this branch, 2026-07-02). **Priority:** P1 (owner updated the primary work vault's convention live; blocked `duo vault publish` on that vault until fixed). **Ticket note:** filed as ENH-243, renumbered to ENH-245 on rebase (see ENH-246's note — PR #119 independently claimed ENH-243).
...
**Review fixes (2026-07-02, same session, `/review` at medium effort × 2 passes).** Two rounds of multi-angle review surfaced and fixed: (1) `readOkfVersion` was short-circuiting to `null` on the first existing-but-non-conformant candidate instead of trying the next, missing a legacy marker sitting next to an unrelated stray file; (2) `electron/main.ts`'s `VAULT_CREATE` (File → New Vault…) handler hardcoded `'index.md'` for its post-create auto-open, silently breaking that flow for every OKF vault created after this ENH shipped, since it never matched the new `_index.md` default and OKF mode writes no `README.md` fallback; (3) `NewVaultModal.tsx`'s OKF preview string was stale; (4) both resolvers hardened from bare `fs.existsSync` to `isFile()`/`isDirectory()` checks so a directory/file-type collision on the marker name can't select the wrong candidate; (5) a redundant `resolveLogFilename` call (`writeListings` + `generateLog` each independently resolving it) de-duplicated. 8 new regression tests. Full details: PR discussion.
...
### ENH-247: The Rollup Guide — user manual for the rollup ecosystem
**Status:** ✅ Shipped 2026-07-03 (PR #119 branch). **Ticket note:** allocated after our ENH-246 (now ENH-251); 247–249 clear of main's max (246) post-#120.

Owner (2026-07-03 walk): *"this space is getting pretty complex — do a loop to update the spec, then make a rich, detailed user manual/guide, like the guide that was made for duo vaults, that explains the rollups mechanisms and ecosystem… spot gaps in the UX/IA and user flows. Make numbered recommendations inline… include html or svg illustrations for both current state and proposed changes."*

Shipped: **(a)** PRD refreshed — `docs/prd/enh-243-rollup-gui.md` gained § 11 (shipped fast-follows ENH-244/250/251 + doctor/BUG-214, per rule 13) and § 12 (the 2026-07-03 gap batch G1–G9 → ENH-248). **(b)** [`skill/references/rollup-guide.html`](skill/references/rollup-guide.html) — Vault-Guide-style walkthrough (ships with the skill; `duo open`-able for installed users): the one-definition/three-faces model (live tab vs snapshot artifact vs glance row), rollup-note anatomy, freshness mechanics (plain-language "stale" = `last_hash` ≠ current corpus hash, SVG diagram), the two Refresh buttons + why pre-ENH-250 artifacts' buttons are inert (chrome baked at render time), hand-authored/broken states, sharing on GitHub/Pages (what survives today, verified against the link-emission code), the full actor-lane ecosystem map — with **nine numbered recommendations R1–R9 inline** in their sections, each with current-vs-proposed mock pairs. Pointers added: `rollup.md` header, CLAUDE.md vault bullet. **2026-07-03 close-out rewrite (owner):** the guide was rewritten user-first — opens with "what is a rollup", plain-language chapters (create → shape → edit-from-rows → the page → fresh/stale → manage → Claude authoring → sharing), internals + CLI demoted to a final chapter, and ALL decision apparatus (R1–R9 blocks, status chips, before/afters) removed now the batch is closed. Human review pass tracked as ENH-249.

---

### ENH-248: Rollup UX gap batch — 2026-07-03 owner walk (R1–R9)
**Status:** ✅ Shipped 2026-07-03 (PR #119 branch; owner walked + confirmed: "great — looks like we are done"). R2–R8 built + walk-2 fixes; **R1 closed-unbuilt** — superseded by the walk-2 refresh rework (Refresh now makes a rollup current everywhere, which answers the "what does stale mean / how do I clear it" confusion R1 targeted; the guide's § 6 explains the concept in plain language); **R9 closed-unbuilt** — owner reviewed the before/after and ended the batch without adopting it (Vault row keeps its format pill; decision apparatus removed from the guide per owner). Owner's two new requirements shipped in the same batch: Copy-as-Markdown removed from the Rollups tab header (artifact-only now), and the editor shows the artifact's click-to-open path under the rollup name. Implementation summary in PRD § 12; CLI twins `duo rollup delete --force | duplicate | render --github | set --links github\|relative` (4-surface sync green, 80 verbs). Tests: +13 (`rollup-manage.test.ts` + reworked artifact-chrome assertions); suite 2140 green. **Walk-2 fixes (2026-07-03):** (a) the editor's Refresh now also re-renders + re-stamps the selected rollup's artifact when stale — the original refresh-split (editor = re-evaluate only) read as broken ("rollups seem to stay stale, even when I click refresh"); one click now makes the rollup current everywhere, and a `duo-vault-refresh` ping updates the Vault tab's chips instantly instead of on the next 30s poll. (b) the rail's ⋯ lifecycle menu is always visible (was hover-only opacity:0 — "I don't see the delete function"). Both live-verified: stale Broken Four → one editor-Refresh click → fresh across editor outlink, `duo rollup list`, and Vault chips.

Owner-reported issues → analyzed root causes → proposals: **R1** explain "stale" (tooltip + shared staleness IPC) · **R2** Duo-native artifact toolbar detected via the artifact's marker comment (root cause of "refresh still looks like a no-op": button behavior is baked into each artifact at render time — the user's `open-milestones.html` predates the ENH-250 fix and carries the old inert button; embedded buttons stay as the GitHub-portable fallback) · **R3** editor-refresh feedback (works today but zero feedback — timestamp + spinner + row-delta) · **R4** grey both Refresh buttons when fresh (same IPC as R1) · **R5** "Normalize with Claude" on view-only hand-authored rollups (owner-endorsed: primes Claude to rewrite the spec into the builder's canonical dialect, semantics-preserving, making it GUI-editable) · **R6** rail management tools (Delete w/ confirm + artifact removal, Duplicate, Reveal, Open note; CLI twins `duo rollup delete|duplicate`) · **R7** Entities section on the Vault tab (per-type live counts; click → instant rollup view; needs counts added to the schema DTO) · **R8** `--github` render variant (blob-URL entity links via ENH-244's repo detection; hide Duo-only chrome off-Duo; verify checklist) — analysis: artifact links are vault-relative `.md` hrefs, structurally intact on GitHub but Pages serves raw markdown + blob view shows HTML as source · **R9** finish the IA split: Vault tab column = glance-only, Rollups tab = workbench.

---

### BUG-214: Default-vault pref watcher never told the renderer — Vault/Rollups tabs go stale on any out-of-band change

**Status:** ✅ Shipped 2026-07-03 (PR #119 branch, with ENH-243; owner-reported: "re-pointed to vault but rollup tab not showing"). **Ticket note:** allocated after grepping sibling worktrees (max was BUG-213).

`installDefaultVaultPrefWatcher` (electron/main.ts, ENH-208) watches `~/.claude/duo/vault.json` and rebuilds the native menu on any write — but a write from OUTSIDE the app's own header switcher (CLI `duo vault default`, a hand-edit, another window, `VAULT_CREATE`) never reached the renderer. `App.tsx`'s Vault/Rollups tab-presence gate and both views' own `defaultVault` state only refresh from an in-renderer `duo-vault-default-changed` CustomEvent that nothing outside `VaultView`'s switcher / `NewVaultModal` ever dispatched — so the tabs silently vanished or went stale until a manual tab-switch or app restart. **Fix:** the watcher now broadcasts a new `VAULT_DEFAULT_CHANGED` IPC push to every window (`broadcastAll`); `App.tsx` re-dispatches it as the SAME CustomEvent every existing listener (tab gate, `VaultView`, `RollupsView`, the editor's `vaultIndex.ts`) already consumes — one push fixes all four call sites. Live-verified: `duo vault default --clear` dropped both tabs with zero renderer interaction; re-setting it restored them rendering the new vault's live content.

---

### ENH-243: Rollup viewer &amp; editor GUI — a Rollups tab beside the vault surface

**Status:** ✅ Shipped 2026-07-03 (PR #119 branch; owner walked the full surface across ENH-244/250/251/248 and closed it: "great — looks like we are done"). PRD: [docs/prd/enh-243-rollup-gui.md](docs/prd/enh-243-rollup-gui.md). Core+IPC+UI+CLI shipped on this branch; typecheck + 2114 tests green; 4-surface sync PASS. **Live-verified in dev 2026-07-02** (DOM-probe walk: tab gate · list · nested 2-level grouping · flip→file-write→regroup→undo · builder live-save · row click-open · doctor card + parent-cwd Claude spawn — all PASS). **Doctor hardened 2026-07-02 (owner walk feedback):** (a) prompt now repairs DECISIVELY (recover intent from surviving frontmatter/fragments/siblings, apply a sensible default, report guesses) instead of stalling on questions; (b) seeded prompts auto-submit — the `claude\n<cmd>\n` stdin-buffer trick races claude's boot, so post-spawn writes are chased with staggered `\r` retries (4–45s; harmless at every timing). Verified fully hands-free in a fresh dev boot: doctor click → repair → healthy+editable, zero keystrokes. **ENH-244 folded in 2026-07-02 — "Copy as Markdown":** a header button on the selected rollup renders it as one GFM table (title cells linked to the entity's GitHub blob when the vault root is inside a GitHub-remote repo — one git probe for the whole table, realpath-safe for symlinked tmp dirs — else a vault-relative `./path` link, OKF-safe); copies via the main-process clipboard write. CLI twin `duo rollup markdown <note>` prints raw GFM to stdout. `core/vault/rollup-markdown.ts` (+8 tests using a real `git init` fixture). **BUG-214 fixed 2026-07-02 (owner walk feedback — "re-pointed to vault but rollup tab not showing"):** the default-vault pref watcher (`installDefaultVaultPrefWatcher`) only ever rebuilt the native menu on an out-of-band write (CLI `duo vault default`, hand-edit, another window) — it never told the renderer, so the Vault/Rollups tab gate and their mounted views went stale until a manual tab-switch or restart. Fixed by broadcasting `VAULT_DEFAULT_CHANGED` to every window (`broadcastAll`), re-dispatched in `App.tsx` as the existing `duo-vault-default-changed` CustomEvent so every current listener (tab gate, VaultView, RollupsView, editor `vaultIndex.ts`) picks it up for free. Live-verified: `duo vault default --clear` made both tabs vanish with zero renderer interaction; re-setting it made them reappear and render the new vault's real content. **Deferral (D5):** engine-side n-level groupBy emitters — GUI groups client-side v1. **Ticket note:** allocated after grepping sibling worktrees (max was ENH-242); renumber on collision.

**ENH-250 folded in 2026-07-02 (né ENH-245 — renumbered: merged PR #120 claimed 245/246 first) — IA reconciliation between the Vault tab's Rollups column and the Rollups tab (owner walk feedback: "slight conflict, IA wise... there are a couple gaps").** Five concrete gaps closed: **(1)** every builder-driven save now auto-renders + stamps the HTML artifact (`core/vault/rollup-render.ts::renderAndStampRollup`, shared by save and Refresh) — a GUI-created rollup was previously unopenable without a terminal. **(2)** the rendered artifact's own Refresh button now self-renders deterministically (no watching Claude required — a re-render needs no inference; only an optional change-summary narrative does) via a new `vault:rollup-render` IPC the button's embedded `vaultRoot` payload routes to, then reloads the browser pane; still also emits to the event bus for an optional Claude-narrated summary. **(3)** "+ New rollup" auto-focuses + selects the Title field (rAF-deferred past a sibling effect's same-tick DOM write that was collapsing the selection — caught live). **(4)** a Vault-tab rollup row's name now opens the rendered artifact, not the raw note (falls back to the note only if never rendered). **(5)** a new "Edit" button on that row switches to the Rollups tab with the note selected there (`duo-vault-edit-rollup` → `duo-rollups-select` CustomEvents, App.tsx owns the tab-switch). All 5 live-verified end-to-end in dev (create→auto-render, artifact Refresh→fresh rows, row-click→HTML not markdown, Edit→Rollups-tab-selects-note); +7 new tests (`core/vault/rollup-render.test.ts`). **Noted for owner decision, not silently changed:** (a) the Markdown rollup's `duo://rollup/refresh` link (Duo's own editor) is a SEPARATE code path (`rollupRefreshLink.ts` → `MarkdownEditor.tsx`) still only emitting the bus event, unfixed — still a no-op without a watching Claude; (b) ~~"+ New rollup" exists in BOTH...~~ **resolved by ENH-246 below**; (c) whether the Vault tab's Rollups column should shrink to a pure glance-list now that full construction lives in the Rollups tab.

**ENH-251 folded in 2026-07-03 (né ENH-246 — same renumber) — "+ New rollup" defaults to the Rollups tab, Claude session moved to a right-click.** Resolves open item (b) above. The Vault tab's "+ New rollup" button now DEFAULTS (left-click) to switching to the Rollups tab and running its instant GUI-builder create flow there — matching the Rollups tab's own button, one creation concept instead of two. The seeded-Claude-authoring-session flow is UNCHANGED code, not deleted — moved to a right-click native context menu (`window.electron.menu.popup`, the same ENH-050 mechanism FileTree/TabBar use) with one item, "Start a Claude authoring session…". New CustomEvent relay: `duo-vault-new-rollup-gui` (VaultView → App.tsx, which owns the tab-switch, same pattern as the ENH-245 Edit button) → `duo-rollups-new` (App.tsx → RollupsView, unconditional listener like `duo-rollups-select`). `RollupsView.onNewRollup` was hardened to accept a `vaultRootOverride` and fetch schema live on demand, since this trigger can land on a tab that was never visited before (its own `defaultVault`/`schema` state would otherwise still be null) — fixed a genuine race, not just plumbing. Live-verified end-to-end via computer-use on a hard-reloaded (never-visited-Rollups-tab) session: left-click → tab switches → rollup created + auto-rendered + title focused, zero prior visit; right-click → native menu shows the one item → click → identical Claude terminal spawn with the unchanged `ROLLUP_AUTHORING_PROMPT` as before. Full suite green (2129 tests), typecheck clean.
