...
- **[P1, UI] History view modal.** **DECIDED 2026-06-20:** surface = **B · modal**; restore = **overwrite-with-confirm**; rows = **standard** (time · who · Δsize); diff = **rendered inline-highlight**. Decision playground: [docs/research/enh-221-history-view.html](docs/research/enh-221-history-view.html).
  ...
  - **Deferred follow-ups:** agent-vs-you **source tagging** (today all saves read "You"); save-chip popover (D) for one-click quick-restore; **canvas parity** — the HTML canvas captures saves but has no History modal yet → mirror-port is **(c) Deferred** per the renderer-surfaces parity rule; frontmatter/CriticMarkup-token handling in the diff preview (currently rendered as raw markdown).
