...
**Status:** ⬜ DRAFT — needs refinement before code. **Sprint 10 research recommendation (2026-05-07):** use TipTap's first-party [`@tiptap/suggestion`](https://tiptap.dev/docs/editor/api/utilities/suggestion) utility — the same primitive that backs `Mention` (`@user`) and `Emoji` (`:smile`). Don't hand-build the popover. Pair with ENH-096 B2 (`[[` wikilink autocomplete) — they share both the popover primitive AND the vault-walking source, so they ship together as ONE PR rather than separate sprints. **Larger surface than the others — likely a multi-day sprint item.**
...
### ENH-107: Tab should detect file deletion and close-with-alert
...
### ENH-106: Distro Pack Builder Workshop — repo-resident playground doc + assistant skill
...
### BUG-103: ⌘T new browser tab — caret not in URL bar (regression, surfaced 2026-05-07 walk-1)
