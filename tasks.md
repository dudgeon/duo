...
- **[P1, UI] History view modal.** **DECIDED 2026-06-20:** surface = **B · modal**; restore = **overwrite-with-confirm**; rows = **standard** (time · who · Δsize); diff = **rendered inline-highlight**. Decision playground: [docs/research/enh-221-history-view.html](docs/research/enh-221-history-view.html).
  ...
  - **🟡 v0.11.2 smoke walk-1 (owner, 2026-06-20): 2 PASS / 2 FAIL.** PASS: OPEN-RESTORE, LOOK. FAILs below.
    ...
    - **⏳ FAIL — WCV occlusion = BUG-209.** Owner: "version history occluded by split view if the window is small enough (nonblocking); MORE concerning the modal 'Close' button is non-functional/no-op; the 'x' button works." Diagnosis: both close buttons call the same `onClose` — the footer "Close" (bottom of modal) is **occluded by the un-parked split-view AUX WebContentsView** (BUG-209), so real clicks land on the WCV; the × (top) is above the occlusion. ONE fix (park the aux WCV on modal-open, like the main pane) resolves BOTH the occlusion and the dead footer Close. Pre-existing P2; the History modal is a new consumer.
...
**Status:** 🆕 Filed 2026-06-20 (owner hit it during the v0.11.2 smoke walk — "none of the copy buttons in the walk sheet work"). **Priority:** P2 (affects every smoke-walk / worksheet / playground "Copy" — but the agent can read results directly via `duo eval`/localStorage, so not a blocker). **Effort:** S–M (needs live-click verification).

**Symptom.** In a Duo browser-pane tab loaded from `file://` (e.g. a generated smoke-walk page), clicking **Copy results** / per-command **Copy** does nothing — the clipboard isn't written. Verified via `duo eval` in the live tab: `isSecureContext: true`, `navigator.clipboard.writeText` exists but the async write is rejected, AND the worksheet's `document.execCommand('copy')` fallback returns **false** (`queryCommandSupported('copy')` is true). So BOTH copy paths fail.

**Root cause (suspected).** The browser-pane `WebContentsView` (session `persist:duo-browser`, [browser-manager.ts:297](electron/browser-manager.ts:297)) has no permission handler granting clipboard-write, and/or the child WebContents isn't the focused frame at click time — so `clipboard-sanitized-write` is denied and `execCommand('copy')` is refused. The worksheet already ships a writeText→execCommand fallback ([.claude/skills/worksheet/generate.mjs:891](.claude/skills/worksheet/generate.mjs)); both legs are blocked by the WCV, not the page.
