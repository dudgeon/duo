...
**Status:** 🆕 Filed 2026-06-16 (owner-reported — happened three times on `lobs.md`). **Priority:** P1 (silent data loss on a core surface). **Effort:** M (fix written; **live smoke-walk owed** — root-caused + fixed in a cloud session that can't run Electron). **Surface:** markdown editor (`renderer/components/editor/`). **Parity:** (b) skipped — surface-specific; the HTML canvas stores tables as live DOM, not markdown source, so there is no round-trip to shatter.

**Symptom (owner).** A table where some cells hold a bulleted list of several people (e.g. POC = `- Ashwin Katikapalli` + `- Navmeet Venkatesh` in ONE cell) renders fine, but after the agent runs *any* operation against the open file — in the report, an SSG generator that "should have had no effect" — the table breaks: the header + first rows survive, then everything from the first multi-line cell spills out below the table as broken plain-text rows (`| — | — | Business Cards and Payments | – Aram Ter-Minassian`, `– Hassan Chagani`, …). Each subsequent op degrades it further (seen three times).

**Root cause.** The corruption is a lossy markdown round-trip in Duo's editor, NOT anything the SSG did. TipTap's `TableCell` allows block content (`block+`), so a multi-person cell is stored as a real `bulletList` node. tiptap-markdown's stock table serializer (`nodes/table.js`) renders a cell with `state.renderInline(cell.firstChild)`, which emits blank lines (`\n\n`) between list items / paragraphs. A GFM table row MUST be a single physical line, so those newlines terminate the table — the row shatters; on re-parse the spilled lines become stray paragraphs after the table, and re-serializing compounds it. The trigger is the editor's autosave / disk-reconciliation write path (`MarkdownEditor.save` → `serializeWithCriticMarkup`): the open file's watcher fires on the agent's op, reconciliation re-serializes the in-memory doc, and the shattered markdown is written back. (Reproduced headlessly against the exact `tiptap-markdown@0.8.10` + `markdown-it` stack.)

**Fix (this branch).** New `renderer/components/editor/extensions/TableMarkdownRoundtrip.ts`, wired into `MarkdownEditor.tsx`:
1. **`TableWithMarkdownCells`** replaces `@tiptap/extension-table`'s serializer — each cell is flattened to ONE line, joining block children / list items with `<br>` (the GFM-canonical in-cell break), keeping `- `/`N. ` markers as literal text, escaping `|`, and collapsing any stray newline to `<br>` so a row can never span lines.
2. **`HardBreakWithMarkdown`** replaces StarterKit's hardBreak — a break inside a table serializes as raw `<br>` (the stock node emitted a useless `[hardBreak]` placeholder under `html:false`); default `\`-newline elsewhere.
3. **`TableCellBrParse`** (parse-side, scoped to `<td>`/`<th>`) rehydrates literal `<br>` text back into real breaks, so a multi-line cell loads as a multi-line paragraph. Scoped to cells → prose `<br>` untouched.
4. **`tableRowsSurviveSerialize`** backstop in `save()` — re-parses the body and refuses the write (surfacing an error) if a table lost rows, so any future serializer regression can't silently corrupt a file.

Round-trip is byte-faithful + idempotent (`onDisk === serialize === serialize²`). 10 unit tests in `TableMarkdownRoundtrip.test.ts`; full editor suite (228 tests) green; typecheck clean. **Owed:** live `/smoke-walk` of a real multi-line-cell table edit before the cut (cloud session couldn't run Electron).
