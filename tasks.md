...
### ENH-244: Rollups — "Copy as Markdown" for a rendered rollup

**Status:** ✅ Shipped 2026-07-02 (folded into the ENH-243 batch, PR #119 branch). **Ticket note:** allocated after grepping sibling worktrees (max was ENH-242); renumber on collision.

A header button on the selected rollup renders it as one GFM table (title cells linked to the entity's GitHub blob when the vault root is inside a GitHub-remote repo — one git probe for the whole table, realpath-safe for symlinked tmp dirs — else a vault-relative `./path` link, OKF-safe); copies via the main-process clipboard write. CLI twin `duo rollup markdown <note>` prints raw GFM to stdout. `core/vault/rollup-markdown.ts` (+8 tests using a real `git init` fixture). (Later removed from the Rollups tab's own header per [ENH-248](#enh-248) — the artifact's embedded button is the sole surviving copy affordance.)

---

### ENH-250: IA reconciliation between the Vault tab's Rollups column and the Rollups tab

**Status:** ✅ Shipped 2026-07-02 (folded into the ENH-243 batch, PR #119 branch; owner walk feedback: "slight conflict, IA wise... there are a couple gaps"). **Ticket note:** filed as ENH-245, renumbered to ENH-250 — [PR #120](#enh-245) independently claimed ENH-245/246 first and merged before this branch.

Five concrete gaps closed: **(1)** every builder-driven save now auto-renders + stamps the HTML artifact (`core/vault/rollup-render.ts::renderAndStampRollup`, shared by save and Refresh) — a GUI-created rollup was previously unopenable without a terminal. **(2)** the rendered artifact's own Refresh button now self-renders deterministically (no watching Claude required — a re-render needs no inference; only an optional change-summary narrative does) via a new `vault:rollup-render` IPC the button's embedded `vaultRoot` payload routes to, then reloads the browser pane; still also emits to the event bus for an optional Claude-narrated summary. **(3)** "+ New rollup" auto-focuses + selects the Title field (rAF-deferred past a sibling effect's same-tick DOM write that was collapsing the selection — caught live). **(4)** a Vault-tab rollup row's name now opens the rendered artifact, not the raw note (falls back to the note only if never rendered). **(5)** a new "Edit" button on that row switches to the Rollups tab with the note selected there (`duo-vault-edit-rollup` → `duo-rollups-select` CustomEvents, App.tsx owns the tab-switch). All 5 live-verified end-to-end in dev (create→auto-render, artifact Refresh→fresh rows, row-click→HTML not markdown, Edit→Rollups-tab-selects-note); +7 new tests (`core/vault/rollup-render.test.ts`). **Noted for owner decision, not silently changed:** (a) the Markdown rollup's `duo://rollup/refresh` link (Duo's own editor) is a SEPARATE code path (`rollupRefreshLink.ts` → `MarkdownEditor.tsx`) still only emitting the bus event, unfixed — still a no-op without a watching Claude; (b) ~~"+ New rollup" exists in BOTH...~~ **resolved by [ENH-251](#enh-251) below**; (c) whether the Vault tab's Rollups column should shrink to a pure glance-list now that full construction lives in the Rollups tab.

---

### ENH-251: "+ New rollup" defaults to the Rollups tab, Claude session moved to a right-click

**Status:** ✅ Shipped 2026-07-03 (folded into the ENH-243 batch, PR #119 branch). **Ticket note:** filed as ENH-246, renumbered to ENH-251 — same collision as [ENH-250](#enh-250) above.

Resolves ENH-250's open item (b). The Vault tab's "+ New rollup" button now DEFAULTS (left-click) to switching to the Rollups tab and running its instant GUI-builder create flow there — matching the Rollups tab's own button, one creation concept instead of two. The seeded-Claude-authoring-session flow is UNCHANGED code, not deleted — moved to a right-click native context menu (`window.electron.menu.popup`, the same ENH-050 mechanism FileTree/TabBar use) with one item, "Start a Claude authoring session…". New CustomEvent relay: `duo-vault-new-rollup-gui` (VaultView → App.tsx, which owns the tab-switch, same pattern as ENH-250's Edit button) → `duo-rollups-new` (App.tsx → RollupsView, unconditional listener like `duo-rollups-select`). `RollupsView.onNewRollup` was hardened to accept a `vaultRootOverride` and fetch schema live on demand, since this trigger can land on a tab that was never visited before (its own `defaultVault`/`schema` state would otherwise still be null) — fixed a genuine race, not just plumbing. Live-verified end-to-end via computer-use on a hard-reloaded (never-visited-Rollups-tab) session: left-click → tab switches → rollup created + auto-rendered + title focused, zero prior visit; right-click → native menu shows the one item → click → identical Claude terminal spawn with the unchanged `ROLLUP_AUTHORING_PROMPT` as before. Full suite green (2129 tests), typecheck clean.
