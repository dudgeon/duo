...
**Status:** 🆕 Filed 2026-07-03 (discovered during the v0.13.3 cut; owner deferred as a follow-on task rather than blocking the cut). **Priority:** P3 (cosmetic — doesn't affect any shipped behavior). **Effort:** S–M. **Ticket note:** allocated after grepping sibling worktrees (max was ENH-251).

The `cut-version` skill's Step 4 item 6 has a "D4 HARD-GATE" mandating the sidebar's `<div class="stats">` block (Done / In progress / Pending / Open issues counts) get recomputed on EVERY cut, even a stamp-only one. `git log -1 -L '/class="stats"/,+6:docs/roadmap.html'` shows the block was last touched at the v0.7.1 cut (2026-05-18) — every cut since (v0.7.2 through v0.13.2, 20+ releases) skipped this step. The numbers currently shown (12 Done / 2 In progress / 14 Pending / 15 Open issues) plus the attached legacy `<p>` note (still describing the v0.5.4 cut) are stale relics from an early roadmap structure that's been superseded by the header `.sub` running-history line and the per-stage cards in the document body. **Fix direction:** either (a) recompute the counts against the CURRENT stage-card body + `tasks.md` open-issue count and refresh the attached note, or (b) if the sidebar "Status" panel is genuinely superseded by the header `.sub` line's running history, remove it entirely rather than let it keep silently drifting. Needs an owner call on (a) vs (b) before implementing — this is a scope decision, not just an update.
