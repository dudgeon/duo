...
**Status:** 🚧 In progress 2026-06-20 — **spec locked** (10 decisions, all recommendations + 2 owner notes) · **Tier 1 (engine + CLI) built** (typecheck clean; 56 cron unit tests + full suite 1647 green; CLI + 4-surface sync done). **Local-session updates (2026-06-20):** (a) D8 deviation re-raised → owner delegated, **dependency-free engine kept** (revisit `cron-parser`+`cronstrue` at Tier 2 when F3's cron preview is built); (b) no-window fire now records **`"missed"`** per D10(3) (was `"error"`), locked by a unit test. **Owed:** live socket/tab-spawn verification — **blocked, owner's dev in use**; queued for the next free dev session, then `npm run sync:claude`, then Tier 2 (Home surface) + ENH-223 (attention badge). **Priority:** owner-TBD (feature sprint). **Effort:** L. **PRD:** `docs/prd/enh-221-scheduled-sessions.md` (§9 = Tier 1 impl notes).
