...
**Symptom (owner).** `duo --help` never listed `duo vault *` / `duo graph *` / `duo base *` or their summaries — discoverable only via the `duo vault <bad-subcmd>` usage die + the doc surfaces (SKILL.md, agents/duo.md, CLI-COVERAGE.md, cli-reference.md). Long-standing gap from ENH-208, surfaced while implementing ENH-216 (whose new vault verbs were also hidden by it).

**Root cause + fix.** `printHelp()` iterates a `GROUP_ORDER` allow-list and skips any group not in it. The vault/graph/base `VERBS` entries carry `group: 'Vault'`, but `GROUP_ORDER` (`cli/duo.ts` ~:3042) never listed `'Vault'` → the whole family was silently dropped. Fixed by adding `'Vault'` to `GROUP_ORDER` (after `'Repo & git'`), rebuilding + restaging the tracked `cli/duo`. Verified: `node cli/duo --help` now renders the Vault group incl. the ENH-216 `init --format`/`mv`/`relink`/`publish`/`promote` verbs + the OKF/Obsidian at-rest summary; typecheck + check:skill-currency (73 verbs) clean.

### BUG-207: `.md.duo.json` editor sidecars pollute vaults (breaks OKF GitHub-portability)

**Status:** ✅ Committed `c32ee48` + verified live (2026-06-14, owner green-lit) — created + opened a person stub in an OKF vault, NO `.md.duo.json` materialized. S1+S4. **S1:** `frontmatterPanelCollapsed`+`suggestingMode` relocated to per-path localStorage (`renderer/components/editor/docUiPrefs.ts`, keyed on abs path; CLAUDE.md §12 UI-pref carve-out), with a one-time read-from-legacy-sidecar fallback on open so existing users keep their state. `MarkdownEditor` toggles + load-path rewired; the two fields stay TYPED in `SidecarV1` (deprecated) so legacy reads/migration still work but are NEVER written. **S4:** `writeSidecar` now gates on a new `sidecarHasLoadBearingContent` predicate (scripts/comments/recentEdits/resolvedThreads/properties) — an empty-but-for-UI-prefs sidecar is never materialized, and an existing empty one is trashed. KEPT untouched: `readSidecar`, `migrateSidecarCommentsToInline`, `resolvedThreads`, the canvas sidecar. New unit test `Page/sidecar.test.ts` (8 cases). Typecheck clean; sidecar + migration tests green. S2/S3/S7 (comment-lead/reply/dead-field cleanup) remain follow-on (not blocking the cut). **Live FS check pending** (verify creating a person note leaves no `.md.duo.json`). **Priority:** P1 for OKF. **Effort:** S–M.

**Symptom (owner).** Creating a person note in an external vault produced `…/people/alina.md` **and** `…/people/alina.md.duo.json` containing only `{"version":1,"frontmatterPanelCollapsed":false}`. A `.duo.json` next to every note clutters the directory, gets committed to git, and shows in Obsidian/GitHub — directly breaking ENH-216's OKF "clean portable markdown" promise.

**Root cause.** The per-file editor sidecar (`renderer/components/Page/sidecar.ts`, Stage 17b / PRD H22) stores per-note UI state — inline comment threads + the metadata-panel collapsed/expanded flag. `MarkdownEditor.tsx` writes it on save when dirty (`:1189`); main's **ENH-215** ("typed entities default the metadata panel to shown") sets `frontmatterPanelCollapsed`, marking the sidecar dirty even when the value is the default → a sidecar is written for *nothing meaningful*. Pre-existing behavior (NOT ENH-216), but the vault/OKF context makes it harmful.

**Investigation (2026-06-14, multi-agent + adversarial verify).** Owner hypothesis ("sidecars are a vestige of the old comment impl") is **PARTIALLY correct**: comments WERE migrated to inline CriticMarkup (BUG-138 Phase 2), so the markdown comment-LEAD dual-write (`MarkdownEditor.tsx:1592-1602`) is genuinely **vestigial**; `scripts`+`properties` fields are dead (zero reads). BUT the sidecar still carries **load-bearing** state with no inline home: the observed fresh-note pollution `{version:1,frontmatterPanelCollapsed:false}` is caused SOLELY by two **UI prefs** — `frontmatterPanelCollapsed` (ENH-215/BUG-139) + `suggestingMode` (BUG-138 P4). Also load-bearing: markdown **replies** (sidecar-only post-migration — dropping their write = silent data loss, HIGH risk), `resolvedThreads`, and **ALL canvas comments/replies/recentEdits** (the canvas has NO inline store — out of scope).

**Safe fix plan (eliminates `.md.duo.json` for the common case, no data loss).** **S1 (the BUG-207 fix):** relocate `frontmatterPanelCollapsed`+`suggestingMode` to Duo-owned per-path storage (localStorage / a keyed map — CLAUDE.md §12 carve-out), with a one-time read-from-sidecar fallback on first open. **S2:** drop the redundant markdown comment-lead dual-write (vestigial). **S3:** inline markdown replies into the lead mark body (the `collapseRepliesIntoBody`/`parseRepliesFromBody` round-trip already exists) so they stop needing the sidecar. **S4 (after S1–S3):** gate the markdown autosave sidecar write off when nothing sidecar-resident remains, AND delete the now-empty `.md.duo.json`. **KEEP (do NOT remove):** `readSidecar` + `migrateSidecarCommentsToInline` (back-compat for pre-migration vaults — removing the READ path strands old comments); `resolvedThreads` write (no inline home until a Phase-4 mark-attr scheme); the entire CANVAS sidecar (separate ENH). **S7:** drop dead `scripts`/`properties`. Recommend shipping **S1+S4** first (fixes the pollution the owner saw, lowest risk); S2/S3 are follow-on cleanup. Caveat: localStorage UI-prefs are per-machine (don't follow a synced vault) — acceptable since the point is to stop polluting the portable vault. Cross-ref ENH-216, ENH-215, BUG-138, `MarkdownEditor.tsx`, `Page/sidecar.ts`, `Page/PageTab.tsx`.
...
**Status:** ✅ Shipped 2026-06-15 — walk-3 **PASS** (owner saw the "Template" badge on template results, no badge on entity notes; commit `af9434a`). Owner raised no badge-vs-section objection → **inline badge is the accepted form**. Data path verified live (CLI + IPC return `isTemplate`; `cli/duo` rebuilt — it bundles core in-process). Search has its own `SEARCH_SKIP_DIRS = {'.obsidian','.trash','out'}` (templates NOT skipped) in `core/vault/search.ts`; `parse.ts:18 SKIP_DIRS` (incl. `templates`) is UNCHANGED so the graph/corpus walk is untouched (D5 intact). `isTemplate` added to `SearchHit` + `VaultSearchHitDto` (carried through the IPC handler unmapped); `VaultSearchPalette` paints an **inline "Template" badge** on template-file group headers (badge, not a separate section — my recommendation; state-and-proceed). `duo vault search` rides the same code path so its JSON now surfaces templates + the `isTemplate` field (CLI-COVERAGE + agents/duo.md updated). New tests in `search.test.ts` (skip-set + flag) and `VaultSearchPalette.test.ts` (group flag). Typecheck clean; tests green. **Live ⌘⇧F check + badge-vs-section owner confirm pending.** **Priority:** P1 (rides the cut). **Effort:** S.

**Symptom (owner).** "⌘⇧F should also surface templates so I can easily find and edit them." Verified (2026-06-14, post-rebase): `core/vault/search.ts:93` searches via the shared `walk()` (`core/vault/parse.ts:22`), which applies `SKIP_DIRS = {'.obsidian','.trash','out','templates'}` (`parse.ts:18`) — the D5 query-exclusion. Search reuses that skip-set, so excluding templates from the *entity corpus* also excludes them from ⌘⇧F. No quick open path for `templates/person.md`. Applies to OKF vaults too (they scaffold `templates/` identically).

**Fix (option a recommended; owner confirmed templates in-scope, badge-vs-section still open).** Give **search** its own skip-set `{'.obsidian','.trash','out'}` (templates NOT skipped) so the palette walks them; keep `readNotes`/corpus on the full `SKIP_DIRS` (templates stay out of the graph/lint — D5 intact). Tag each hit with `isTemplate` (path under `templates/`) so the palette shows a small "template" badge — distinct from entity hits, one ranked list. Search-surface only; no corpus/graph change. Touch: `core/vault/search.ts` (skip-set + `isTemplate` on `SearchHit`), the palette renderer (`VaultSearchPalette`) for the badge, `shared/host-api.ts` `VaultSearchHitDto`. Rec: inline badge (smaller, one ranked list) over a separate `Templates ▸` section — pending owner confirm.
...
### FOLLOWUP-051: Frontmatter edge storage form — rel-path (ENH-216 D7) drops the Duo/Obsidian graph edge; flip to `[[wikilinks]]`

**Status:** ✅ Shipped 2026-06-15 — walk-3 **PASS** (owner verified the `[[ ]]` keystroke + create flow live; commit `e105b85`). Spun off two non-blocking ENHs from the walk: **FOLLOWUP-052** (cmd-click nav on frontmatter `[[ ]]`) + **ENH-220** (View ▸ navigate to vault folder). Frontmatter `[[ ]]` autocomplete now inserts `[[basename]]` in BOTH modes (`useFrontmatterWikilink.tsx`); `okfFrontmatterValue`/`okfLinkRelTarget`/`yamlQuote`/`rewriteFrontmatterWikilinks` deleted from `okfLinks.ts`; `FrontmatterPanel.commit()` is synchronous (no rewrite — persists `[[ ]]` as `[[ ]]`); the orphaned `resolveFrontmatterWikilink` + `resolveWikilinkInVault` import dropped from `MarkdownEditor.tsx`; D7 lock + playground D7 card + summary diagram/table flipped; `okfLinks.test.ts` D7 block removed. Independently re-verified the graph-edge premise (regex extractors `extractLinkRefs`/`extractLinkKeys` in `core/markdown/vaultLinks.ts` match only `[[ ]]` + `[text](href)`; bare YAML path → no edge, in Duo AND Obsidian). Typecheck clean, editor tests green. **Priority:** P1 (gates FOLLOWUP-050's insert form + the ENH-216 cut). **Effort:** S (subtractive).

**Finding (verified empirically).** D7 stores OKF frontmatter edges as a YAML-quoted rel-path (`team: "../notes/2026/06/kids.md"`). Tested both forms in a scratch OKF vault: `duo graph backlinks kids` returns ONLY the note using `team: "[[kids]]"`; the rel-path note does NOT register and shows in `duo graph orphans`. So the rel-path form **silently drops the relationship from Duo's knowledge graph**. Cause: `extractLinkRefs`/`extractLinkKeys` (`core/markdown/vaultLinks.ts`, called from `core/vault/parse.ts` over the full raw file) match `[[…]]` (+ body markdown links), NOT a bare frontmatter path — D7's recommendation premise ("Duo resolves it to a graph edge") is **false**.

**Three-tool truth.** Duo graph: `[[kids]]` = edge, rel-path = orphan (verified). Obsidian: `[[kids]]` = clickable link + graph edge, rel-path = inert string. github.com: renders frontmatter as a NON-clickable table either way — so the OKF "markdown links render on GitHub" rationale (correct for the BODY, D3) does NOT transfer to frontmatter. The only consumers that USE a `team` edge (Duo + Obsidian) both parse `[[ ]]`, not paths.

**Recommendation.** Frontmatter edges → `[[basename]]` in BOTH modes; body stays markdown rel links (D3). Principled split: use the form the actual consumer parses. Subtractive change: FOLLOWUP-050's autocomplete inserts `[[basename]]` (drop `okfFrontmatterValue`); **delete** the D7 commit-rewrite (`rewriteFrontmatterWikilinks`). Reverses **ENH-216 D7** → update the lock + the `docs/research/okf-vault-mode.html` D7 card. Cross-ref FOLLOWUP-050, `renderer/components/editor/{okfLinks.ts, useFrontmatterWikilink.tsx, FrontmatterPanel.tsx}`.
...
### ENH-216: OKF vault mode — a second at-rest serializer (standard markdown links) so Duo vaults publish to GitHub

**Status:** ✅ Walk-complete 2026-06-15 — across walk-1 → walk-3, **5 PASS** (New Vault dialog/OKF default · body `[[ ]]`→md-link expand-on-resolve · cmd+click rel-link nav · frontmatter `[[ ]]` autocomplete [FOLLOWUP-050/051] · ⌘⇧F template badge [ENH-214]), **0 open FAIL**, **2 deferred-test SKIPs** (dialect-flip + Obsidian-compat → **FOLLOWUP-053**, owner-marked non-blocking). The walk-1 FAIL (frontmatter half-feature) was fixed by FOLLOWUP-050 + FOLLOWUP-051 and re-PASSed in walk-3. Code: rebased onto `main` (0.10.4), typecheck clean, **1555 tests**, `cli/duo` rebuilt, vault:detect IPC-handler gap found+fixed. Pre-cut batch (FOLLOWUP-051/BUG-207/ENH-214) all landed + verified. **READY TO CUT v0.11.0** (OKF = new vault format → minor bump). **Priority:** P1 (owner-initiated). **Effort:** L (multi-tier).

**Pre-cut batch (2026-06-14 → v0.11.0).** ALL THREE PENDING ITEMS LANDED (owner green-lit "do all of those things"): **FOLLOWUP-051** `e105b85` (frontmatter `[[ ]]`→`[[name]]` both modes), **BUG-207** `c32ee48` (sidecar S1+S4, verified live), **ENH-214** `af9434a` (templates in ⌘⇧F + badge, data path verified). Plus earlier: ENH-216 core+UI, FOLLOWUP-050, BUG-208 (✅), worksheet copy fix. Suite **1555 green**, typecheck clean, `cli/duo` rebuilt. **Walk-3 DONE 2026-06-15: 2 PASS** (FOLLOWUP-051 keystroke + ENH-214 badge — owner accepted the inline badge), **2 non-blocking SKIPs** (dialect-flip + Obsidian-compat → FOLLOWUP-053), **0 FAIL**. Owner spun off 2 non-blocking ENHs from the walk: **FOLLOWUP-052** (cmd-click nav on frontmatter `[[ ]]`) + **ENH-220** (View ▸ navigate to vault folder). **PR [#98](https://github.com/dudgeon/duo/pull/98) opened + review round-2 addressed** (2026-06-15): 7 findings fixed incl. 2 Medium data-correctness bugs in the shipped ENH-216 code — `promoteSection` dropped content on a slug collision (now refuses) and `publish --index-only/--log-only` rewrote both files (now write-scoped); +5 Low/nit (relink slug-first docs, `vault init --no-default`, mode-aware not-a-vault messages, `targetKey` ext-strip guard, `MDLINK_RE` note). 1567 tests green, all live-verified, resolution table in the PR body. **READY TO CUT v0.11.0** (after PR merge).

**Locked decisions (2026-06-13).** D1 MENU-DIALOG (File ▸ New Vault + CLI twin) · D2 OKF-DEFAULT (dialog defaults OKF; CLI `--format` required) · D3 EXPAND-ON-RESOLVE (gesture input-only; markdown rel link on disk, no `[[ ]]` persists) · D4 MARKER-WINS (root `index.md` `okf_version` is the OKF marker, wins over `.obsidian/`; registry is a pointer) · D5 DUO-REWRITES (`duo vault mv` clean path + `duo vault relink` repair) · D6 SLUG (slug stem + `title:`; link text = slug for existing-file picks in v1) · ~~D7 RELPATH~~ **REVERSED by FOLLOWUP-051** → frontmatter edges persist as `[[ ]]` in both modes (a bare rel-path isn't a graph edge in Duo or Obsidian) · D8 INDEX-MD (generate `index.md`/`log.md` listings) · D9 LINK (promote leaves a markdown link) · D10 STAMP-ALL (type on every note; captures `type: note`; OKF `index.md` replaces frontmatter-less README) · D11 ALL-AT-ONCE (Tiers 1+2 one milestone). **Follow-ups (AUQ):** (a) stamp a stable short base36 `id:` on every OKF note → deterministic out-of-band-move self-heal (`relink` keys on id, slug fallback); (b) link text = slug for existing-file picks in v1, human title for freshly-created links, title-parsing a tracked follow-up; (c) **auto-relink on vault open** (+ explicit `duo vault relink`). Full build order + risk register: design workflow output; PRD playground at `docs/research/okf-vault-mode.html`.

**Problem.** Duo's vault feature (ENH-208) is wired entirely to Obsidian `[[wikilinks]]`, which render as **literal text** on github.com (confirmed against GitHub Docs + community #73062 — wikilinks render only in Gollum-backed GitHub *Wikis*, never in normal repo `.md`). The owner's thesis: the highest-value future use case is knowledge bases that render/resolve on GitHub and in any tool — i.e. standard markdown links with relative paths, the shape the proposed **Open Knowledge Format (OKF v0.1)** standardizes. A default Duo vault is neither OKF-conformant (untyped captures + frontmatter-less README fail §9.2) nor OKF-link-shaped, and an OKF bundle isn't even detected as a vault (no `.obsidian/`); its markdown links produce zero backlinks in Duo (`parse.ts`/`graph.ts` are wikilink-only).

**Proposal.** Add an **OKF vault mode** — a second *serializer + parser* over the same `core/vault/` graph model (entities/types/edges are format-agnostic; only the at-rest link syntax + markers change). Keep the exact authoring ergonomics (`[[ ]]` hotkey, type-picker, silent-stub via `filing.ts`/`wikilinkCreate.ts`) but write `[Text](./rel.md)` at rest. Format is chosen **per vault**; the active/primary vault toggles Duo between Obsidian and OKF mode. Includes a new **File ▸ New Vault** dialog (location · name · format picker) + `duo vault init --format=okf|obsidian` CLI twin. Markdown links render in all three targets (Duo / Obsidian / GitHub) — wikilinks only fail on GitHub — so OKF mode is strictly more portable and **still opens in Obsidian** (does not break the ENH-208 "opens in Obsidian always" guarantee).

**Decision artifact (rule #11).** Interactive playground at [`docs/research/okf-vault-mode.html`](docs/research/okf-vault-mode.html) — 11 decision-cards interwoven with each explanation, a New-Vault dialog mock, render matrix, and a Copy-decisions round-trip. **Open in Duo (not the preview panel) to walk it.** Decisions D1–D11: (1) New-Vault entry points, (2) default format, (3) storage model — *when* `[[Thing]]`→markdown link [rec: expand-on-resolve], (4) mode detection [rec: in-vault marker wins], (5) move resilience [rec: Duo rewrites inbound links], (6) filenames [rec: slugify + `title:`], (7) frontmatter edges, (8) rollups→`index.md` listings, (9) promote/transclusion, (10) type-stamping/§9 conformance, (11) build sequencing [rec: Tier-1 reader first].

**Build tiers.** T1 Reader (graph/resolver/click-nav learn the markdown-link dialect + OKF detection marker → Duo *consumes* GitHub-style KBs) · T2 Author (insert-seam transform + slug filenames + type-on-everything + New-Vault picker → Duo *produces* GitHub-ready vaults) · T3 Native ops (move/rename link-rewrite, `index.md`/`log.md` generation, embed-free promote). ~90% of wikilink machinery is reusable; new surface area is two seams (`command()` insert string + the parse/resolve/click layer).

**Cross-refs.** ENH-208 (vault), the OKF v0.1 spec, `core/vault/{detect,parse,graph,filing,scaffold,corpus}.ts`, `renderer/components/editor/extensions/WikilinkSuggestion.ts`, `renderer/wikilinkCreate.ts`. **Review task:** this entry stays open and surfaces in every smoke walk until the owner closes the playground decisions.
...
**Status:** 🟢 **P0 (D1+D2) done — unit-verified AND live-verified** on `claude/hungry-yalow-55f5b7` (commit `10cdd26`). **P1 (D3/D4/D5) deferred as perf-only** (see live finding below). Owed only: owner's own walk of real projects + a cut/PR. **Priority:** **High** (P0 user-visible jank on the single most-used pane). **Effort:** S–M (P0 was a self-contained two-hook edit). **Sequenced after ENH-210** (worktree-aware Duo, sibling `claude/youthful-chebyshev-885712`) — rebase onto post-ENH-210 `main`; every code surface is disjoint (see PRD § C).

**Live verification (2026-06-11, fresh dev on this worktree, `duo eval` MutationObserver — the sub-frame flash a screenshot can't catch).** Observer teeth proven (catches an injected `Loading…`). Burst-add 10 files in cwd → **0 Loading flashes**, all 10 rows render in place; burst-delete → **0 flashes**. M3 probe: modifying a TRACKED file in a git repo → **0 Loading flashes AND 0 navigator DOM mutations** — the git-tick re-render (M3) reconciles to identical DOM, so it is **wasted CPU, not visible flicker**. Conclusion: P0 resolves the reported flicker; **P1 (D3 watcher churn, D4 row memo, D5 git-Map guard) is a perf/efficiency follow-up, not a visible-flicker fix — safe to defer → tracked as FOLLOWUP-049.** (Separately noted during the walk: a `MaxListenersExceededWarning: 11 files:changed listeners` after heavy HMR — a watch-accumulation smell D3 would also address; filed mentally as P1 motivation, not a flicker cause.)

**P0 shipped (commit `10cdd26`, renderer-only — no electron changes).** D1 stale-while-revalidate (`useNavigator.ts` + `useUserClaudeNavigator.ts`: stop deleting/nulling an already-loaded listing before refetch — overwrite on resolve; null sentinel reserved for first-ever load; removed from `handleEvent` / `refresh()` / the resubscribe loop) + D2 coalesce (100ms trailing-edge debounce in a recreation-surviving `useRef`; removed-event selection prune stays synchronous). Regression tests at `renderer/hooks/useNavigator.flicker.test.ts` (T1 stale-while-revalidate — proven to FAIL when the SWR guard is reverted; T2 coalesce). typecheck clean; flicker 4/4; full hooks suite 29/29. **Owed:** macOS dev-session walk of the sub-frame flash (agent-writes-files-in-cwd repro + `duo eval` DOM/mutation probe — a screenshot won't catch it). **P1 remaining:** D3 incremental `updateWatchPaths` (touches electron — needs permission), D4 `React.memo` rows + D5 deep-equal git-Map guard (both `FileTree.tsx`, renderer-only).
