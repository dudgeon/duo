...
**Status:** 🟡 Smoke-walked v0.10.4 (2026-06-14, post-rebase onto `main`): **3 PASS** — New Vault dialog (OKF default) · `[[ ]]`→md-link expand-on-resolve · cmd+click rel-link nav (the headline OKF behavior, owner-verified live) · **1 FAIL** — frontmatter `[[ ]]` gesture is a half-feature → **FOLLOWUP-050** · **4 SKIP** — CLI-init (owner's PATH `duo` = stale installed release w/o `--format`, so it ran the OLD Obsidian scaffold; the code is correct — agent-walked via `./cli/duo`), dialect-flip / Obsidian-regression / Obsidian-compat untested. Code: rebased onto `main` (0.10.4), typecheck clean, **1542/1542 tests**, check:skill-currency 73 verbs/0 failures, `cli/duo` rebuilt @0.10.4, vault:detect IPC-handler gap found+fixed. **Cut (rec v0.11.0) blocked on FOLLOWUP-050.** Also surfaced **BUG-207** (`.duo.json` sidecar pollution — planned, S1+S4). **Priority:** P1 (owner-initiated). **Effort:** L (multi-tier).

**Pre-cut batch (2026-06-14 → v0.11.0).** ALL THREE PENDING ITEMS LANDED (owner green-lit "do all of those things"): **FOLLOWUP-051** `e105b85` (frontmatter `[[ ]]`→`[[name]]` both modes), **BUG-207** `c32ee48` (sidecar S1+S4, verified live), **ENH-214** `af9434a` (templates in ⌘⇧F + badge, data path verified). Plus earlier: ENH-216 core+UI, FOLLOWUP-050, BUG-208 (✅), worksheet copy fix. Suite **1555 green**, typecheck clean, `cli/duo` rebuilt. **Walk-3 pinned** (`docs/dev/smoke-walks/v0.10.4-rev3.html`, aux): 2 owner-eyes items (FOLLOWUP-051 keystroke + ENH-214 badge/section call) + 2 carry-forward SKIPs; BUG-207 + ENH-214 data path agent-verified. **Cut v0.11.0 once walk-3 passes.**

**Locked decisions (2026-06-13).** D1 MENU-DIALOG (File ▸ New Vault + CLI twin) · D2 OKF-DEFAULT (dialog defaults OKF; CLI `--format` required) · D3 EXPAND-ON-RESOLVE (gesture input-only; markdown rel link on disk, no `[[ ]]` persists) · D4 MARKER-WINS (root `index.md` `okf_version` is the OKF marker, wins over `.obsidian/`; registry is a pointer) · D5 DUO-REWRITES (`duo vault mv` clean path + `duo vault relink` repair) · D6 SLUG (slug stem + `title:`; link text = slug for existing-file picks in v1) · D7 RELPATH (frontmatter edges stored as rel-path strings; `[[ ]]` stays the input gesture) · D8 INDEX-MD (generate `index.md`/`log.md` listings) · D9 LINK (promote leaves a markdown link) · D10 STAMP-ALL (type on every note; captures `type: note`; OKF `index.md` replaces frontmatter-less README) · D11 ALL-AT-ONCE (Tiers 1+2 one milestone). **Follow-ups (AUQ):** (a) stamp a stable short base36 `id:` on every OKF note → deterministic out-of-band-move self-heal (`relink` keys on id, slug fallback); (b) link text = slug for existing-file picks in v1, human title for freshly-created links, title-parsing a tracked follow-up; (c) **auto-relink on vault open** (+ explicit `duo vault relink`). Full build order + risk register: design workflow output; PRD playground at `docs/research/okf-vault-mode.html`.
