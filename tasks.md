...
**Systemic fix.** (a) `scripts/materialize.sh`: retry stragglers with `brctl download` + a short wait before declaring loss, and check `git ls-files --error-unmatch` before claiming a path is not in HEAD. (b) `check-materialization.sh`: also scan `$(git rev-parse --git-common-dir)/objects/pack` — it only looks at the working tree, so the packfile eviction was invisible until a commit failed. (c) `duo doctor`: on `ETIMEDOUT_RESPONSE` with a live pid, say "app is running but not answering — check for evicted files (`npm run check:materialization`)". (d) **`node_modules` is evictable too** — found the same day: the main checkout's tree had hundreds of dataless files (jsdom 105, @tiptap 288, vitest 33…), so `vitest` sat at 0% CPU forever instead of failing. `check-materialization.sh` excludes `node_modules` by design; it should at least sample it. Worktrees resolve the main checkout's `node_modules` by ANCESTOR lookup, so every worktree inherits the problem. Workaround used: `npm ci --ignore-scripts` into `~/.cache/duo-deps/<name>` + `node node_modules/electron/install.js` + `npx electron-rebuild`, symlinked as the worktree's `node_modules`. (e) The real cure is owner-side: move the repo out of iCloud-synced `~/Documents`, or turn off Optimize Mac Storage.
...
**Fix applied — option (a).** `MarkdownEditor` nulls `stubPicker` when `isActive` flips false; `useFrontmatterWikilink`'s `active` effect now clears `typePicker` alongside `match`. Deliberately a plain state clear, NOT the picker's `onCancel` — that refocuses the host editor / restores the textarea caret, which would steal focus back into the tab the user just left. Outcome matches a click-outside cancel: the inserted `[[name]]` / OKF placeholder stays, no stub. **Tests:** `useFrontmatterWikilink.test.tsx` (2 — popover and type picker both close on hide, picker does not reopen on return); removing the `setTypePicker(null)` line fails it. **Live (dev build of this branch, real TipTap instance + real `duo edit` / `duo goto` switches):** body picker → switch → 0 boxes, focus lands in the new tab, Home → 0, return → stays closed, placeholder kept, no stub file; frontmatter picker (Enter on the `New:` row) → switch → 0, return → 0, draft intact. The body host has no unit test (mounting `MarkdownEditor` whole is heavy) — live-verified only.

**Options considered.** (a) **Chosen — cancel on tab-hide, host-side:** `MarkdownEditor` effect `if (!isActive) setStubPicker(null)`, and `setTypePicker(null)` alongside `setMatch(null)` in the hook's `active` effect. Same outcome as today's click-outside cancel (the inserted `[[name]]` / OKF placeholder text stays, no stub). (b) Suspend-and-restore the picker on return — nicer, but needs the anchor re-measured and the OKF placeholder range re-validated; not worth it for a rare path. (c) Do nothing beyond click-outside — rejected, the agent path is Duo's core premise.
...
**Root cause (confirmed live via read-only `duo dom` probes).** It is the `[[` wikilink autocomplete popover, orphaned. Inactive file tabs stay mounted under `display:none` (`WorkingPane.tsx`). TipTap's Suggestion plugin only ever closes a session from a transaction *inside its own editor* — which a hidden editor never gets — so typing `[[` and switching tabs leaves the session open forever. The popover portals to `document.body`, so it escapes the hidden wrapper; its anchor (a decoration span in the hidden editor) measures as an all-zero rect, and `positionStyle` clamps that to `top: 8px; left: 0`. The text is a stale snapshot: `loading` is only re-read in the plugin's `onUpdate`. Live evidence: exactly one `.duo-suggestion-popover` in `<body>` at rect `[0, 8]`; its React fiber chain runs `SuggestionPopover → Portals → PureEditorContent → MarkdownEditor`; the owning editor was a hidden tab whose whole content was `test 123[[`. Third recurrence of the "persistent popover" family (walk-1 v4, walk-2 v2/v3) — each prior fix was applied twice, to two byte-identical copies of the lifecycle.

**Fix (two layers).**
- **Lifecycle (the intended behavior).** New `extensions/suggestionLifecycle.ts` is now the ONE popover state machine for both `WikilinkSuggestion` and `AtMention` (−180 duplicated lines). It adds a `suspend()` host hook, exposed via extension storage; `MarkdownEditor` calls it when its `isActive` prop flips false. Suspend ≠ dismiss: Escape/pick stay closed for the session, but a suspended session **resumes at the caret** when the user comes back and types on. The remount is gated on the `suspended` flag specifically — the plugin calls `onExit → onUpdate → onStart` for a moved+changed session, so remounting on a bare null component would double-mount and leak.
- **Primitive guard (the net).** `SuggestionPopover` renders nothing when its anchor is not laid out (`isAnchorLaidOut`: zero width AND zero height — width alone is legitimately 0 for a collapsed caret). Covers every consumer and any future re-render path.
...
3. `renderer/App.tsx:776` subscribes to `browser.onTabsChange` but (pre-fix) never pulled `browser.getTabs()` on mount, so App's `browserTabs` stays `[]` until main next emits — hence `browserTabsCount: 0` and an empty strip, which also hides the stranded tab from the user.
...
**Fix (2026-09-18) — part 1: a WCV rectangle is only valid while the renderer that measured it is alive.** New pure contract in `shared/wcv-bounds.ts` (`planHostRendererReloadReconcile` + `PARKED_WCV_BOUNDS`), executed by `BrowserManager.reconcileForHostRendererReload()` (`electron/browser-manager.ts:1119`) and wired to the host window's **`did-start-loading`** (`electron/main.ts:980`) — which fires at the START of every top-level renderer load (Vite full reload, ErrorBoundary Reload, crash recovery), strictly before the new renderer can publish, so the park can never clobber fresh bounds. A child `WebContentsView`'s own loads never reach that emitter, and it is a no-op on first load. The reconcile parks every view at 1×1, resets both cached rectangles, drops `auxTabId`, and clears the BUG-047 overlay mute (owned by the renderer that asked for it — latched across a reload it would make every later `setBounds` cache-but-never-apply, i.e. a permanently blank browser pane). It deliberately does **not** touch `activeIndex` or focus — the reloaded renderer restores its own active surface, and re-homing here would hijack it (that is why this is not just a `releaseAuxTab()` call: release also re-activates + focuses the released tab). Parked views come back the instant the new renderer mounts `BrowserRenderer` / `AuxBrowserSlot` and publishes. 7 unit tests.

**Fix (2026-09-18) — part 2: App pulls the browser-tab list on mount.** `renderer/App.tsx:780` now calls `browser.getTabs()` once on mount (superseded-by-broadcast guarded) before subscribing, the same snapshot-on-mount pattern `useBrowserState` already used. Without it, main's surviving tabs stay invisible to App until the next emit — the empty strip / `browserTabsCount: 0` above — which would also hide the tab part 1 just un-pinned.

**Fix (2026-09-18) — part 3: `duo split-view resize` drives whichever slot is filled.** `renderer/App.tsx:4815-4829` now applies the clamped ratio to BOTH `auxState` and `auxBrowserTab` (mutually exclusive, so setting both is safe) — the same shape as the ENH-099 walk-3 fix for ⌘⌥4. The clamp itself moved to `shared/split-view.ts` — `clampAuxSplitFraction` (measured geometry) + `clampAuxSplitPct` (typed CLI input, tolerates `35` for `0.35`) + `AUX_SPLIT_MIN/MAX/EVEN` — and now backs all five former copies — `electron/main.ts` `splitViewResize`, the App subscriber, the divider drag in `renderer/components/WorkingPane.tsx:1026`, and the two persistence validators (`core/session-state-service.ts`, `core/workspace-file-service.ts`, which already clamped on load — that is why a corrupt persisted ratio was never a candidate cause) — plus it maps a non-finite input to 0.5 instead of letting `NaN` reach the columns' `flex-grow` (a bad ratio invalidates the shorthand on both columns and the main pane collapses under the aux). The fraction/percent split matters: a divider drag past the pane's left edge computes a fraction > 1 and must land on the MAX aux width, never be re-read as 1.1 % (the drag-extreme inversion a single percent-tolerant clamp would have introduced). 9 unit tests.

**Verification.** `npm run typecheck` clean; `npx vitest run` **2406/2406** (16 new). No live-app verification: another session's dev Electron holds the shared socket, so this branch was built with the app untouched (read-only `duo` probes only) — the smoke walk is owed. Durable coverage: the pure parts are unit-tested; the native-view path can't be, so `docs/dev/smoke-checklist.md` § 4a pins the reload + resize + follow-the-column cases.

**Live-repro recipe (for the coordinator, on a dev build of this branch).** Before: `duo open <any .html>` → `duo tabs` (note the id `N`) → `duo split-view open-browser N` → `duo split-view` shows `aux.kind: "browser"`, `duo layout` shows the aux block. Then `duo dom --js "location.reload()"`. **Pre-fix:** the page keeps painting over the working pane at its old width, `duo tabs` still says `inAux: true`, `duo split-view` says `aux: null`, `duo layout` says `browserTabsCount: 0`. **Post-fix:** nothing floats over the editor; `duo tabs` shows `inAux: false` for N (and lists every tab), `duo split-view` → `aux: null`, `duo layout` → `browserTabsCount: 4`-ish and `main` populated; clicking N in the strip repaints it in the main pane at the right rectangle. Second case: `duo split-view open-browser N` → `duo split-view resize 0.3` → `duo split-view` reports `splitPct: 0.3` **and the divider visibly moves** (pre-fix: state unchanged, divider frozen).

**Workaround for the owner's currently-stranded app (no restart needed).** `duo split-view close` — BUG-195's unconditional `releaseAuxTab()` reconcile re-homes the stranded tab and clears the ghost.
...
**Symptom (owner, recurring).** Click a tile in the project rail for a project that has open working tabs but "no active terminal" → the rail highlight, title-bar focus chip, navigator root, working-pane active tab (and, once a terminal is involved, the terminal strip) all flip several times per second. Clicking **All** only sometimes escapes; the user has to mash it until a click lands.

**Root cause — a two-effect tug-of-war between `focusedProject` and `activeWorking`.** Two `App.tsx` effects each "correct" the same discrepancy in opposite directions and re-fire on every commit:

| | Effect | Deps | What it does on focus = P while the active file F ∈ Q |
|---|---|---|---|
| **D11 auto-switch** | `App.tsx` ≈ 1330 (ENH-182 Phase 3c) | `[activeWorking, tabMembership, focusedProject]` | F's project Q ≠ P → `setFocusedProject(Q)` |
| **Keep-visible** | `App.tsx` ≈ 1785 (ENH-182 Phase 2 auto-spawn effect) | `[focusedProject, visibleTerminals, visibleFileTabs]` | F is hidden under P → `setActiveWorking(P's first visible file G)` |

Both fire in the same passive-effect flush, so the next commit is `(focus = Q, active = G ∈ P)` — the exact mirror of the entry state. Both fire again → `(P, F' ∈ Q)` → forever. Every flip also re-runs the navigator re-root effect (`nav.actions.navigateTo(focusedProject)`, ≈ 1755), the FOLLOWUP-030 `pendingBrowserRedirect` machine, the `projects.pushState` IPC, and (after a terminal is spawned) the `setActiveTabId` branch of keep-visible — that fan-out is the "whole app flickers madly." D11 was written to follow a file *activation* but its deps make it re-adjudicate the **pre-existing** active file after every focus change; keep-visible has no notion that the active-file move is its own programmatic convergence step rather than a user activation. **React never throws** — the "Maximum update depth" guard only trips on synchronous nested updates, not passive-effect ping-pong, so the loop runs at frame rate instead of crashing (unlike the BUG-192 nested-updater variant that force-quit).

**Why "no active terminal" is a selector, not the mechanism.** The loop needs exactly one thing: the active working tab is a **file from another project** at click time. A project open only as reference tabs guarantees that (you are, by construction, working elsewhere). Static trace of the three sub-cases on `main`:

1. **P has zero terminals under its root** → keep-visible auto-spawns a shell at P (same flush as D11's `set(Q)`). Next commit: ENH-204 sees the new member terminal while focus = Q → `setFocusedProject(null)` lands *after* D11's `set(P)` in the same batch → focus releases to **All**. Net: one visible flip, a spurious new terminal, the active file silently changed to Q's first file. A blip, not the mad loop — but it explains why the repro feels intermittent.
2. **P has no *member* terminal but some tab's launch `cwd` is under P** (shell exited, `cd`'d away — BUG-191 live-cwd drops its membership — or lives in a nested sub-project) → the spawn is suppressed by the `hasTerminalUnderRoot` guard, which reads the **frozen launch cwd** (`t.cwd`), not membership. No new terminal → ENH-204 never fires → **pure infinite loop**. This is the owner's "no *active* terminal" case.
3. **P has visible member terminals** and the active file is foreign → no spawn, **pure infinite loop**. Hiding terminal-less projects from the rail would not close this path.

**Why the All click is hard to land.** React flushes the pending passive effects (which queue `set(P)`/`set(Q)`) before processing the click's `set(null)` in some interleavings and after it in others; only the "after" ordering wins the batch. Hence "mash All until one sticks."

**Why past "solutions" did not resolve it.**

1. **The correct fix exists and was never merged.** PR #137 (BUG-267, 2026-07-17, `2a2f246` + `5de7b9c`) pins exactly this 2-cycle with the pure `adjudicateActiveSurfaceFocusSwitch` gate (switch focus only on a genuine activation change, never inside the focus-entry settling window, programmatic keep-visible/redirect moves pre-seed the refs) — 9 unit tests, live-verified in dev, PRD amendment included. As of 2026-09-16 it is **OPEN, zero reviews, zero comments, `mergeable: CLEAN`; no tag contains it; `main` (v0.13.7) does not have it.** Its ledger entry, PRD amendment, and RESUME/active-sprint pointers live only on the branch, so every later session (this one included) started from `main`, saw no record, and re-diagnosed from scratch. The memory note that called it "fixed" compounded the confusion.
2. **Every earlier fix targeted a *different* loop or trap in the same cluster and added another writer.** BUG-192 (`c215226`) fixed the close-path nested-`setState` jitter and its audit proved only **P↔null** loops convergent; BUG-193 fixed a membership mis-mapping; BUG-194 (`573fe3e`), ENH-204 (`e05cda2`), and BUG-161 each added a *release-to-All* escape for one specific trap. `focusedProject` now has **eight** independent writers (rail click, CLI push, D11-file, D11-browser, BUG-194, ENH-204, BUG-161, close-project) and `activeWorking` has three programmatic ones, with no arbitration and no distinction between "user intent" and "programmatic convergence." Each patch made the cluster harder to reason about without touching the P↔Q cycle.
3. **The "dummy terminal" idea was already tried on day one.** Auto-spawn-on-focus (`dfb0b52`, 2026-05-25 — the same day D11 landed in `26cfd03`) is precisely "open a new terminal in that project's cwd." It does not address the working-pane cycle at all, and its frozen-launch-cwd guard means it does not even fire in sub-case 2.
4. **Nothing durable guards the invariant.** `shared/project-lifecycle.test.ts` on `main` covers only the P↔null helpers; `docs/dev/smoke-checklist.md` has no rail-focus item; there is no runtime tripwire, so any future writer can re-open a cycle silently (the recurring-regression rule: a bug that returns needs a test, not a checklist line). *(Both addressed on this branch: 10 new `project-lifecycle` tests — including a convergence-contract regression that models both effects and pins the fixpoint at ≤2 passes with the gate vs. a period-2 cycle without it — plus `docs/dev/smoke-checklist.md` § 2a “Project rail — focus”. The runtime tripwire is still absent — see “deliberately left out” below.)*

**Assessment of the owner's terminal-strip options (mutually exclusive; rev 2, 2026-09-16).** The loop lives in the *working-pane* half of the lens (focus ↔ active file), so none of the three options can close it on its own — the D11 activation gate from PR #137 is required under every option. What the options really decide is what the **terminal strip** does while focused, which is still a real product choice: that half of the filter is where BUG-161/194, ENH-204 and auto-spawn's terminal litter all came from.

| Option | UX | Implementation | Closes the loop? | Verdict |
|---|---|---|---|---|
| **(a) Hide terminal-less projects from the rail** | Rail degrades into a terminal switcher; reference-only projects (vault notes, docs opened from another repo) lose their tile — contradicts D12 and the lens premise; `duo project focus <name>` can't reach them. | Trivial (one filter in the rail memo). | **No** — a project *with* terminals clicked while a foreign file is active still loops (sub-case 3). | Reject. |
| **(b1) Placeholder empty state instead of auto-spawn** | Focused project with no terminal shows *"No terminal in ‹name› — Open shell · Open Claude here"*; one click gives today's spawn **on demand**. Ends the spurious-terminal litter (auto-spawn re-fires after every release to All because `autoSpawnedForRef` resets on `null`). Keeps D8 intact. | **S** — delete the spawn branch + `autoSpawnedForRef` + the frozen-launch-cwd guard; render the placeholder in `TerminalPane`; keep-visible's terminal branch and ENH-204 stay. | **No** on its own (removing the spawn also removes the accidental ENH-204 escape in sub-case 1) — **needs the gate**. | **Recommended, paired with the gate.** |
| **(b2) Auto-open a real terminal at the project cwd** | This is the shipped status quo (`dfb0b52`); the loop exists with it, and its guard silently does nothing in the exited-shell case. | Already built. | **No.** | Status quo — reject as a fix. |
| **(c) Never filter terminals** | Strip becomes a global session list; the lens applies only to canvas + navigator. Kills the entire terminal-side trap family (BUG-161/194, ENH-204, auto-spawn, ⌘T/⌃Tab target loss) but halves the D8 payoff (⌃Tab cycling only visible tabs; clutter with many Claude sessions). | **M** — `visibleTerminals = tabs`, delete four effects, D8 PRD amendment, `duo project list` counts, full smoke walk. | **No** — the cycle is in the working-pane half. | Not now — a D8 relitigation. Revisit if terminal-side traps keep recurring after (b1). |

**Recommendation (rev 2).** One change set, one PR: **(1)** land PR #137's `adjudicateActiveSurfaceFocusSwitch` gate (rebase onto `main`; its 9 tests) — this alone closes the loop in all three sub-cases and is not optional under any option; **(2)** replace auto-spawn with the **(b1) placeholder** — ends the terminal litter and deletes the frozen-launch-cwd guard hole rather than patching it. Optional, cheap, not required: a flip-count circuit breaker around `setFocusedProject` as insurance against a future ninth writer. Deferred: the single-reducer refactor of the focus cluster, only if the cluster regresses again.

**Deliberately left out of the 2026-09-16 change set.** (a) The flip-count circuit breaker around `setFocusedProject` — cheap insurance against a ninth writer, but it is a tripwire for a class of bug the gate now closes, and adding a ninth writer to prove that point is the wrong reflex; file it if the cluster regresses. (b) The single-reducer refactor of the eight-writer focus cluster — still the real structural answer, still not urgent. (c) Option (c) “never filter terminals” — a D8 relitigation, revisit only if terminal-side traps keep recurring after the placeholder. (d) Any live-app verification: this branch was built with the Duo Electron instance untouched (other agent sessions share it), so **typecheck + suite + static reasoning only** — the smoke walk is owed.

**Live walk (2026-09-16, dev build from this branch, driven over the `duo` CLI).** Fixtures: two git repos P and Q each holding only markdown working tabs, active file in Q. (1) `duo project focus P` → focus stable across 10 polls, placeholder `No terminal in projP` rendered, active file landed on P's own note. (2) `duo edit` a Q file while focused on P → focus followed to Q once and held; the edited file stayed active. (3) Placeholder **Open shell** → member terminal spawned, P counts `terminals: 1`, placeholder cleared, focus held. (4) `focus --all` released. (5) `exit` in P's only shell → after the live-cwd poll P counts `terminals: 0`, refocus P → placeholder, focus stable (the sub-case the old frozen-cwd guard silently no-oped). `duo errors` empty throughout. **Two findings fixed during the walk** (commit follows the six-commit set): (a) the keep-visible fallback landed on a *pinned* foreign `tasks.md` instead of P's note — pinned cross-project tabs sort first in `visibleFileTabs`; now `chooseKeepVisibleFileTab` prefers a true member (mirrors the FOLLOWUP-030 browser rule); (b) on a `duo edit` of a foreign file, keep-visible ran in the same flush as D11's switch against the *stale* focus and moved the just-activated file away — a `focusSwitchQueuedRef` set by both D11 adjudicators makes keep-visible stand down for that one render (cleared by the `[focusedProject]` effect once the switch commits). Both unit-pinned (`shared/project-lifecycle.test.ts`, 3 new cases).

**Verification plan (for whoever executes).** Repro matrix on the fix build: (i) sub-case 3 — project P with a live terminal + file tabs, active file in Q, click P → focus stable across 10 polls of `duo project list`, active file becomes P's first tab; (ii) sub-case 2 — same but P's only terminal has `exit`ed → focus stable, placeholder shown (no silent no-op); (iii) sub-case 1 — P has no terminal under root → focus stays on P (no release to All), placeholder shown, no spawn until clicked; (iv) contract — while focused on P, `duo edit` a Q file → focus follows to Q, once; (v) contract — with the placeholder, ⌘T from the focused empty strip opens a member shell and focus holds.


**Fix (2026-09-16) — part 1: adjudicate activations, not focus changes.** PR #137's gate, cherry-picked onto this branch (`2a2f246` + `5de7b9c`; the code and its comments still say BUG-267). A pure `adjudicateActiveSurfaceFocusSwitch` helper in `shared/project-lifecycle.ts` is now the single decision point for BOTH follow-the-active-surface effects (the D11 file effect and its Phase 3c-browser twin). Gate order: no surface → no switch; **unchanged surface → no switch** (the loop-breaker — a focus change or a late membership-probe settle is not a user activation); focus-transition pending → no switch (the `pendingBrowserRedirect` settling window, whose `useState` is hoisted above the D11 effect that reads it); All mode → no switch; no/same-project membership → stay put; otherwise switch. Every programmatic keep-visible / redirect move **pre-seeds** the adjudication ref (`lastAdjudicatedFileRef` / `lastAdjudicatedBrowserTabRef`) so Duo's own convergence step is never mistaken for a user activation. Converges in ≤2 passes: tile click → D11 skips (unchanged surface) → keep-visible moves the active surface into P → D11 re-runs on that change → member → quiesce. Deliberate behavior changes: (a) BUG-193-style focus theft via a *pinned* foreign active file on focus entry is gone; (b) a late membership settle on an unchanged active file no longer yanks focus. `duo edit` / `duo open` / user tab-clicks still switch focus (those change the active surface). 9 unit tests pin the no-bounce-on-focus-entry invariant. **Live-verified in dev on the original branch (2026-07-17, owner-approved app cycle):** the exact repro (focus a project holding only a working tab, active file in another project) held focus rock-stable across 12 polls of `duo project list` with the keep-visible convergence firing once; the D11 contract (`duo edit` a foreign-project file while focused → focus follows) re-verified stable. *Discovered in passing there, still open:* terminals in a `/tmp`-rooted project split into a duplicate `/private/tmp/…` tile (live-cwd canonicalization — BUG-162 covered browser tabs only); fixture-only impact today.

**Fix (2026-09-16) — part 2: auto-spawn → placeholder empty state (owner option b1).** The keep-visible effect no longer spawns a terminal when a focused project has none. Deleted: the spawn branch, the `autoSpawnedForRef` guard, and the `hasTerminalUnderRoot` frozen-launch-cwd probe (sub-case 2's hole — it read `t.cwd`, not membership, so an exited or `cd`'d shell silently suppressed the spawn). Instead, while `focusedProject !== null` and the visible strip is empty, the terminal column renders a quiet empty state — *“No terminal in ‹name›”* with **Open shell** and **Open Claude here** buttons wired to the existing `openTerminalHere` / `openClaudeIn` handlers (both make the new tab active AND a member, so ENH-204 does not release focus). ⌘T / the `+` button while focused-and-empty now open the new terminal **at `focusedProject`** rather than inheriting a hidden tab's cwd (`chooseNewTerminalCwd` in `shared/project-lifecycle.ts`). Ends the spurious-terminal litter (auto-spawn re-fired after every release to All, because `autoSpawnedForRef` reset on `null`) and removes the guard hole rather than patching it. Hidden non-member terminals keep their existing keep-alive behavior — this stays visibility-only filtering; tabs never leave state.

**Related.** [BUG-267 / PR #137](https://github.com/dudgeon/duo/pull/137) (the never-merged fix, cherry-picked here; same bug, retired id), BUG-192 (close-path sibling; its audit scope explains the miss), BUG-193 (D11 focus theft, same adjudication family), BUG-194 / ENH-204 / BUG-161 (release-to-All escapes that each added a writer), BUG-191 (frozen launch cwd — the F4 hole is the same design), FOLLOWUP-030 (browser-side face of the same cycle, also gated by PR #137), ENH-182 PRD D8/D11/D12.
...
### ENH-257: `duo goto <home|vault|rollups>` — CLI verb to switch a window's top-level tab

**Status:** ✅ Shipped 2026-07-13 — merged via [PR #124](https://github.com/dudgeon/duo/pull/124). **Priority:** Medium (CLI-parity gap — surfaced concretely while verifying BUG-256's Rollups-tab UI, where there was no headless way to reach the tab at all). **Ticket note:** allocated after checking ALL open PRs, not just local tasks.md — PR #121 already claims ENH-253 (unmerged), PR #122 claims BUG-254, PR #123 claims ENH-255; this also forced renumbering the rollup-rename bug fix from BUG-253 → **BUG-256** (see that entry) to avoid colliding with PR #121.

**Verification.** typecheck clean, `check:skill-currency` PASS (81 verbs, all 4 surfaces), full suite 2178/2178 green. Live end-to-end against a freshly-opened, isolated window (`duo window new`, kept separate from the shared primary window given other agent sessions were concurrently active on it): `duo goto vault` → `duo goto rollups` → `duo goto home`, each confirmed via `duo layout` showing the correct `kind`/`path`/`id` after every switch. This is also what surfaced BUG-256's tab-content mismatch red herring — a rollup ("all-indexs.md") that doesn't exist in the vault this window pointed at appeared mid-sequence, almost certainly from another concurrently-running agent session's own `duo vault default` mutation landing on the same (global, not per-window) setting — not a defect in this verb, which had already round-tripped cleanly three times before that. **Known side effect:** testing required flipping the global default vault to a scratch path; restoring it afterward was blocked by the auto-mode classifier ("shared resource" guard) — the default vault may not be back to its pre-session value; whoever owns that setting should check `duo vault default`.

**Problem.** Home/Vault/Rollups are synthesized `FileTab`s with stable ids (`f:home`/`f:vault`/`f:rollups`) and sentinel paths (`duo://home` etc.) living in the SAME `fileTabs` array as regular file/editor tabs — but nothing in the CLI can activate them. `duo tab <n>` / `duo close <n>` only address the (separate) BROWSER-pane tab strip, not `fileTabs` at all. `duo view <path>` / `duo edit <path>` don't recognize the `duo://` sentinel — tried live against the running dev app, `duo view duo://rollups` didn't activate the Rollups tab; it silently created a broken new "unknown" tab at a mangled relative-path resolution of the sentinel string, replacing the correctly-synthesized one. If the human can click it, the CLI must reach it (CLAUDE.md rule 4) — this was a real, confirmed gap, not a hypothetical one.

**Fix.** A new main→renderer IPC push (`NAV_OPEN_WORKSPACE_TAB`), mirroring the existing `sendView`/`sendEdit` fire-and-forget pattern exactly: `sendOpenWorkspaceTab(kind)` checks the target window exists and (for `vault`/`rollups`) that a default vault is actually set — vault/rollups tabs are conditionally synthesized, so requesting them with no default vault returns a clear `{ok:false, error}` rather than silently doing nothing. The renderer's handler resolves `kind` to the tab's stable id and calls the exact same `setActiveWorking({kind:'file', id})` a sidebar/tab-strip click already uses (`WorkingPane.tsx`'s `handleSelect`) — no new activation logic, just a new trigger for the existing one. CLI: `duo goto <home|vault|rollups> [--window N]`.

**Affected surfaces:** `shared/types.ts` (IPC const), `electron/preload.ts` (renderer subscription), `electron/main.ts` (`sendOpenWorkspaceTab` + NavBridge wiring), `core/socket-server.ts` (`NavBridge` interface + `case 'goto'`), `cli/duo.ts` (verb + help), `renderer/App.tsx` (the one new `useEffect` subscriber). 4-surface doc sync: `skill/SKILL.md`, `agents/duo.md`, `docs/CLI-COVERAGE.md`.

---

### BUG-256: Rollup editor Title field doesn't rename the note's filename (silent no-op)

**Status:** ✅ Shipped 2026-07-13 — merged via [PR #124](https://github.com/dudgeon/duo/pull/124); owner smoke-walk (`v0.13.7-pr124`) confirmed Enter-commits-rename, Escape/blur-cancels, and taken-name-rejects-with-inline-error all pass against a live dev build (see § Verification below). **Priority:** P2 (data-integrity — the note's on-disk name silently drifts from its displayed title, and ENH-243's own § 12 G6 gap-list already assumed rename "exists (Title field)" — it doesn't). **Ticket note:** allocated after grepping sibling worktrees (max was BUG-252).

`updateRollupNote()` (`core/vault/builder.ts`) rewrites the note's `title:` frontmatter but never renames the underlying `rollups/<slug>.md` file — both the Rollups tab's live-save path (`electron/main.ts` → `VAULT_ROLLUP_SAVE`) and the `duo rollup set --title` CLI verb call this same function, so both surfaces share the bug. Filed live by the owner along with the fix requirements: **(1)** renaming the Title field should require an explicit save action rather than autosaving; **(2)** the save must validate the target name isn't already taken, rejecting with a clear message if so, and only renaming on disk when available.

**Owner decisions (AUQ, 2026-07-06):**
- **Save gesture:** mirrors Duo's existing file-rename convention (`FileTree`'s `RenameInput`, BUG-028 precedent) — **Enter commits** (validates + renames), **Escape or blur-without-Enter cancels** (reverts to the last-saved title, discarding the pending edit). A rejected commit (name taken) keeps the user's typed text in the field with an inline error so they can fix and retry, rather than silently reverting their work.
- **Artifact cascade:** a successful rename also moves the rendered artifact — the stale `rollups/<old-slug>.html` is removed and `out:`/`last_generated`/`last_hash` are cleared so the existing auto-render-on-save path (ENH-250) regenerates fresh at `rollups/<new-slug>.html`, rather than leaving the artifact permanently pointed at the old name.

**Fix.** New `saveRollupNoteWithRename()` in `core/vault/builder.ts`, shared by both the GUI IPC handler and the CLI verb: computes the new slug from the title (same `safeName().toLowerCase()` rule as create), no-ops to a plain content rewrite when the slug is unchanged, rejects with `{ ok: false, error }` when the target filename is already taken (no auto-uniquing — unlike create, the user is choosing this exact name), and otherwise reuses `moveNote()` (so inbound links + the `.duo.json` sidecar travel with the note, same as `duo vault mv`) before stamping the new title at the moved path and cleaning up the old artifact.

**Verification.** `core/vault/builder.test.ts` covers `saveRollupNoteWithRename` (same-slug no-op, available rename, taken-name rejection leaves the original byte-untouched, artifact cleanup, inbound-link rewrite) — full suite 2178/2178 green (2284/2284 after the post-merge rebase), typecheck clean, `check:skill-currency` PASS. Live end-to-end via a scratch vault + `duo rollup set --title` (the CLI twin, same core function the GUI calls): rename-to-available landed on disk, rename-to-taken rejected with a clear message + exit 1 + both notes byte-untouched. **Owner smoke-walk (2026-07-13, `v0.13.7-pr124.json`), agent-driven against a live dev build via `duo dom --js` synthetic events on a scratch vault:** Enter commits the rename (file renamed on disk, old file gone), Escape and blur-without-Enter both revert the field with no rename, and renaming onto an already-taken title shows the exact inline error + rejects without touching either note. `duo goto rollups`/`vault`/`home` also confirmed via `duo layout`.
...
**2026-07-09 (review follow-up) — YAML-hazard bug in the new `title:`/`aliases:` lines, fixed.** The alias-seed line (`  - ${opts.title}`) and the pre-existing `title:` line both wrote the human title UNQUOTED. A title containing a YAML hazard — most commonly `: ` (colon-space, BUG-260's exact hazard class, e.g. a title like `Track: Context and Agent Resources`, which is a real string already used elsewhere in this codebase's own fixtures) — parses as a nested YAML mapping and silently blanks the note's ENTIRE frontmatter on read (`splitFrontmatter` swallows the parse error and returns `{}`), not just the one field: the stub loses its `type:`/`id:` and becomes invisible to every corpus query. Live-reproduced via a throwaway probe test before fixing. **Fix:** added `yamlSafeScalar()` to `core/vault/scaffold.ts` — the same conditional bare-unless-hazardous, single-quote-doubling escaping `builder.ts`'s `yamlSafeExpr` already established for filter expressions (BUG-260) — applied to both the `title:` line and the new alias entry. The common hazard-free case stays byte-plain (unquoted), so existing serialized output is unchanged. New regression tests in `core/vault/filing.test.ts` cover the colon-hazard case (frontmatter still parses; both fields correctly recovered) and confirm the no-hazard case stays unquoted.
...
**2026-07-09 (review follow-up) — third write-site found + fixed: the GUI Rollup Builder.** The original pass covered `scaffold.ts`'s `INITIATIVE_TPL` and confirmed `bases/processing.base` was already clean, but missed `core/vault/builder.ts`'s `serializeBuilderBase` — the Rollups tab GUI's canonical serializer, and a THIRD live write-site for `type ==`-filtered bases. Every rollup the builder produces always filters by type (single or an or-group), so without an exclusion, EVERY GUI-built rollup phantom-rowed its own type's `templates/<type>.md` when opened natively in Obsidian — confirmed empirically (probe test showed the D15 lint correctly warning on unmodified builder output). **Fix:** `serializeBuilderBase` now always emits the `!file.inFolder("templates")` clause; `parseBuilderBase` treats that exact clause as a known framework line (silently skipped, not surfaced as an editable filter or required for legacy notes) so round-tripping and backward-compat with pre-fix notes both hold. 5 new tests in `core/vault/builder.test.ts` cover single-type, multi-type or-group, round-trip-with-exclusion-present, legacy-note-without-exclusion-still-parses, and a `duo base lint` D15-clean check on builder output.

**2026-07-09 (review follow-up, open) — `skill/references/vault-guide.html` ch. 12's migration section documents a not-yet-shipped CLI flag.** The "Migrating a pre-existing OKF vault to frontmatter markdown links" section instructs `duo vault relink --frontmatter`, but `--frontmatter` does not exist on `duo vault relink` anywhere in this repo right now (not on `main`, not on this branch, not on the empty `enh-266a-frontmatter-md-links` placeholder branch — that sibling PR has zero commits as of this review). If this PR merges to `main` before ENH-266a ships that exact flag name, ch. 12 documents a nonexistent command to end users. Not fixed here (implementing ENH-266a's frontmatter-link feature is well outside this PR's scope) — flagging so the merge order is enforced: ENH-266a before this PR, or gate ch. 12's migration subsection behind ENH-266a landing.
...
**Status:** ✅ Shipped 2026-07-13 — decisions locked 2026-07-08 (D1 same-author-or-unattributed · D2 transaction-rewrite · D3 relocate-to-end · D4 skip-caret · D5 compose · D6 CM-pure); implemented on PR #129 (PRD: docs/prd/enh-260-track-changes-composition.md). **Priority:** High — suggesting mode can corrupt the reject-baseline and silently lose original text. **Effort:** M (core fix) + S–M per follow-on phase. **Ticket note:** allocated after repo-wide max ENH-259; renumber on collision.

**Owner smoke-walk (2026-07-13, `v0.13.7-pr129.json`).** All 5 owner-facing items PASSED against a live dev build of the rebased PR: net-zero delete of your own pending insertion, type-over/cut/paste-over of existing text now tracks a deletion+insertion pair (not a silent hard-delete), a second adjacent Backspace no longer eats live original text untracked, typing inside a struck range no longer inherits the strike, and the substitution-fold save→reload→save round-trip stays clean. The CLI compose path (`duo doc delete`/`substitute` against pending `{++…++}` tokens — full-overlap nets to zero, partial overlap shrinks, in-place substitute, non-insertion overlap still refuses) was independently agent-verified before handoff.
...
A Catch-up Done card whose session ran in a since-removed git worktree currently can't be re-entered: the cwd is gone, so `claude --resume <uuid>` from the fallback parent dir fails with `No conversation found with session ID` (the transcript lives under the worktree's *encoded* `~/.claude/projects/<enc>` dir, unreachable once the cwd vanishes). **Shipped stopgap (ENH-231 P0):** the card is struck through + flagged "⚠ worktree removed" + re-entry is disabled, so it no longer dead-ends. **This ENH = the rich fix.** Options to weigh: (a) resume by pointing `--resume` at the correct encoded project dir (does claude accept a session whose cwd no longer exists?); (b) open the transcript **read-only** (a "view transcript" affordance — Duo reads the JSONL it already parses); (c) offer "resume here" — re-root the session into an existing dir / a fresh worktree. Pick after probing what `claude --resume` actually tolerates for a gone cwd.

---

### ENH-233: Catch-up — dismiss / "mark reviewed" (drop a session off the board)
**Status:** 🔵 Open (filed from ENH-231 walk #2, 2026-06-24). **P1.** Supersedes PRD D6b.

Let the user drop a reviewed/abandoned session off the Catch-up board. **Owner leans option (a):** a Duo-owned `dismissed`/`reviewedAt` annotation in `home-state.json` (review-state is a reviewer-workflow concept Claude Code doesn't model — legitimately Duo-owned per the §D9 ADR; `reviewedAt` already lives there). **Open concerns to resolve in this ENH:** (i) the annotation map **bloats over time** (owner flagged) — needs a prune/GC policy (e.g. drop annotations whose session aged out of the 7-day window / whose JSONL is gone); (ii) the alternative SOR from the flag research — Claude **Desktop**'s per-session `~/Library/Application Support/Claude/claude-code-sessions/.../local_<id>.json` carries an `isArchived` boolean (Claude-native-ish, but round-trips with Desktop only, not the CLI; CLI `sessions-index.json` has no status field); (iii) a `custom-title` sentinel (true session-SOR but co-opts the title + a guarded `.jsonl` append). Decide SOR + prune policy, then build the board affordance + the read at assembly.

**Provenance.** Owner (2026-06-23): *"prototype an alternate home view … less focused on project segregation and more focused on catching up on async work performed by your agent while you were away — eg if the user spun up 8 jobs, and then forgot what they were … review the work with a reminder of what the goal of each session was, what the most recent instruction was, … the file that a given session is working on, and then what the next steps for them are. A timeline of commingled sessions and less of an aggregation of projects."* Then the framing constraint: *"Duo does not have access to a general purpose inference API and cannot perform summarizations … at the point of open in the home view. Instead, Duo will need to anticipate those data needs while the sessions are active … to pre-hydrate that data."*

**The shape.** A second Home **mode** (toggle: today's project-aggregated "Projects" ↔ new "Catch-up"), rendering a single **commingled, reverse-chronological timeline** of Claude sessions interleaved by last activity — project is a tag, not a section. Each session is a **briefing card**: goal · your most-recent instruction · next-steps · files in flight · artifact chips · state badge (needs-you / working / done / blocked) · one-click next action. Triage filter chips + a "since you were away" watermark.

**Locked decisions (owner, 2026-06-23 playground paste-back):**
- **D1 — direction:** build as a **sibling Home mode**, not a replacement (toggle on Home; both mental models survive).
- **D2 — v1 card fields:** goal + your-last-instruction · next-steps (from `TodoWrite`) · files in flight · artifact chips (PR/tests). Label the human-instruction line **"You asked"** (owner: *"'last said' is vague on who said it — be clearer"*) so it's never confused with the agent's own status line.
- **D3 — narrative field:** **agent self-narration** via a new `duo session note/next` verb the skill calls at stopping points; deterministic last-assistant snippet as the no-narration fallback. Inference happens only inside the running agent, never in Duo at open.
- **D4 — hydration:** **materialize a per-session digest on the managed Stop hook** (already installed for ENH-225 attention), so Home reads cached digests, not hundreds of MB of transcripts, at open. The digest is a rebuildable materialized index (transcript stays source-of-truth — §D9-tolerable; the litmus + invariant are in the PRD).

**Key architecture finding.** 10 of 11 card fields are **deterministic transcript extraction** — Duo already does seek-based JSONL head/tail reads (`electron/claude-session-tracker.ts`: first-user-msg = goal, last-assistant-text = snippet, gitBranch, timestamps). The unlock: the agent's latest **`TodoWrite`** call *is* the next-steps list, free. Only the "what happened" narrative is inference-bound (→ D3).

**Acceptance (v1).** (1) A Home toggle switches Projects ↔ Catch-up; the choice persists + has CLI parity (`duo home mode projects|catchup`). (2) Catch-up renders a commingled timeline sorted by last activity across all known sessions, with the five state badges and triage filter chips. (3) Each card shows goal, "You asked" (last human turn), next-steps (todos), files, and artifact chips — all from the pre-hydrated digest, **zero inference at open**. (4) A digest is (re)materialized on the Stop hook and is provably rebuildable from the transcript (delete-cache test → identical render). (5) `duo session note/next` stamp + skill teaching; snippet fallback when absent. (6) typecheck + smoke-walk green; CLI 4-surface sync (`check:skill-currency`) PASS; RESUME.md + active-sprint.md refreshed.
...
### ENH-249: Docs quality pass — human review after the bot editing sweep
**Status:** 🆕 Filed 2026-07-03 (owner-requested at ENH-248 close). **Priority:** P2. **Effort:** S–M. **Ticket note:** allocated after ENH-248; grep siblings on merge.

The rollup docs went through several same-day bot editing passes (guide rev 1 → rev 2 → rev 2.1 → the user-oriented rewrite; `rollup.md`; PRD § 11–12; CLI docs on 4 surfaces). Owner wants a **human review pass over the bot-edited docs**: read the Rollup Guide + Vault Guide end-to-end as a new user would, check the rewrite kept everything true to shipped behavior (no stale screenshots-in-prose, no orphaned cross-references, no leftover decision language), verify tone stays user-oriented (plain language first, internals last), and confirm `rollup.md` (agent-facing) and the guide (human-facing) don't contradict each other. Also owed from ENH-248 R8: the "verify on GitHub Pages" checklist after the first real push of a `links: github` artifact. Surfaces in the next smoke walk.
...
**Provenance.** Owner ENH-224 v0.11.2 smoke walk-1 (2026-06-21): *"on cmd+O, focus/caret should be in the URL entry bar for faster/easier paste/type."* Today the merged Open bar (`renderer/components/OpenBar.tsx`) mounts without the paste/path input focused, so the owner has to click into it before pasting a GitHub URL or typing a path — the dominant first action on that surface.

**Acceptance.** Pressing ⌘O (or File ▸ Open…) puts the caret in the URL/path entry field with any prior text selected (so a paste overwrites cleanly). Verify from both renderer focus and terminal-column focus (the menu-accelerator path, ENH-224 FU3). Recents/Browse still reachable by Tab/click.

### ENH-227: Share-back footer — explicit PR-success state + a "View PR" link that actually works

**Status:** ✅ **Built + live-verified 2026-06-21** (renderer-only; HMR-verified live, owes owner eyes-on in `v0.11.2-enh224-rev3` walk). **Priority:** P2 (owner-requested polish + a real broken-link bug). **Effort:** S. **PRD:** folds under [ENH-224](docs/prd/enh-224-file-open-flow.md) Phase 2 (D13 morph).

**Provenance.** Owner ENH-224 v0.11.2 smoke walk-2 note on the (PASSing) footer item: *"upon PR success, duo should show success, and provide link to the PR."* Investigation ([ProposeBar.tsx](renderer/components/ProposeBar.tsx)) found two real shortfalls: (a) success was implicit only — the footer morphed to a *muted* "Proposed · owner/repo" with no PR number and no success affordance, reading as "nothing happened"; (b) **the "View PR ↗" button was effectively dead** — it used a renderer `window.open(...)` that silently no-ops in this WebContentsView (the same class as the Copy-button break), so the owner's click did nothing — likely why they asked for "a link to the PR." A stale code comment even claimed "the URL is also shown in the bar as text" (it wasn't).

**Shipped (renderer-only, [ProposeBar.tsx](renderer/components/ProposeBar.tsx)):**
- Explicit success line: **"✓ PR #N opened · owner/repo"** in `duo-text-ok` (both-themes-legible per the renderer-surfaces theme rule), replacing the muted "Proposed". PR number now rendered (it was already in hand via `PrInfo.number`).
...
### BUG-211: Copy buttons in the browser pane don't write the clipboard (file:// WebContentsView)

**Status:** 🟡 Filed 2026-06-20 (owner hit it during the v0.11.2 smoke walk). **Priority:** P3 — **DOWNGRADED after live diagnosis: copy actually WORKS when the page has keyboard focus** (verified live — the button shows "Copied!"). Non-blocking (results persist in localStorage; "Send to Claude" bypasses it; agent reads results directly). **Effort:** M (the proper fix needs careful WCV focus-management + live verification).

**Symptom.** In a Duo browser-pane tab, the FIRST click on **Copy results** / per-command **Copy** after the WebContentsView has lost keyboard focus does nothing; a subsequent click (after the WCV regains focus from the first click) succeeds.

**Root cause (CONFIRMED live, 2026-06-20).** NOT a permission issue — it's a `Document.hasFocus()` race. `navigator.clipboard.writeText` (and `execCommand('copy')`) require the document to be focused; the browser-pane WebContentsView isn't reliably the keyboard-focused frame at click time (the React shell / terminal can hold focus). Proven: a `left_click` in the page BODY first (focuses the WCV) then the Copy button → "Copied!" succeeds; a lone click on the button → fails the first time. `browser-manager.ts` focuses the WCV on `switchTab` (L425) but focus drifts and the first post-drift click's `writeText` runs before `hasFocus` is true. (Earlier `duo eval` results — writeText rejected, `execCommand` false — were confounded: eval is never a focused user gesture.)

**Fix direction.** Ensure the browser-pane WCV reliably holds keyboard focus when active (focus on tab-activate AND on window-focus / first interaction), so the first click's clipboard write sees `hasFocus === true`. Verify with a real first-click after defocus. Do NOT add a session clipboard-permission handler — it's unnecessary (copy works with focus) and would broadly grant permissions to all browsed sites.

**Fix direction.** Grant clipboard-write to the browser-pane session via `setPermissionCheckHandler`/`setPermissionRequestHandler` (idempotent, app-scoped), and/or ensure the WCV takes focus on click. **Verify with a REAL click** (synthetic `eval` writes throw "Document is not focused"; a real gesture is required) — ideally with the dev window on the PRIMARY display so computer-use clicks map. Workaround meanwhile: the **Send to Claude** button (Duo terminal bridge, no clipboard) should still work; and the agent can read walk results straight from `localStorage['worksheet:smoke-walk-vX']` via `duo eval`.
...
### FOLLOWUP-050: Frontmatter `[[ ]]` gesture is a half-feature — no autocomplete / type-picker / entity-creation in the raw-YAML editor (ENH-216 D7)

**Status:** 🚧 Implemented (2026-06-14, awaiting owner re-walk) — typecheck clean, 558 tests (incl. 9 new matcher tests), FrontmatterPanel mounts clean. The live `[[ ]]` popover is keystroke-driven, so owner-verify (computer-use can't type into the dev Electron). **Priority:** P1 (blocks the ENH-216 cut). **Effort:** M.

**Implementation (max reuse per owner directive).** The raw-YAML textarea now hosts a live `[[ ]]` autocomplete + silent-stub create flow at parity with the body. REUSED verbatim: `SuggestionPopover` (the dropdown + keyboard nav), `withCreateNoteRow`/`isCreateNoteItem` (the "New:" row), `rankVaultFiles` (ranking), `TypePickerPopover` (the type-pick + stub-creation IPC), the vault index (the SAME `useVaultIndex` sources MarkdownEditor threads into the body suggester), and `okfFrontmatterValue` (the serializer). NEW (textarea-only glue): `frontmatterWikilinkMatch.ts` (string trigger matcher mirroring `suggestionMatchers.findWikilinkMatch`), `textareaCaret.ts` (mirror-div caret-rect, since textareas have no `coordsAtPos`), `useFrontmatterWikilink.tsx` (the controller). Expand-on-resolve: on pick the FINAL at-rest form is inserted — OKF a YAML-quoted rel-path `"./rel.md"` (a markdown link can't live in a YAML value — a leading `[` starts a flow sequence), Obsidian `[[basename]]`. Files: `renderer/components/editor/{useFrontmatterWikilink.tsx, frontmatterWikilinkMatch.ts(+test), textareaCaret.ts, okfLinks.ts, FrontmatterPanel.tsx, MarkdownEditor.tsx}`.

**Symptom (owner, FAIL).** "In frontmatter `edit raw` mode (only way to edit frontmatter) the `[[]]` handling is broken, with no autocomplete or entity creation flow."

**Root cause.** D7's v1 (U7) implemented the frontmatter `[[ ]]` edge as a COMMIT-TIME rewrite ONLY: `FrontmatterPanel.commit()` rewrites `[[Name]]` → a quoted rel-path string on blur/Save, but ONLY when the name already RESOLVES to an existing note (`renderer/components/editor/FrontmatterPanel.tsx:142-180` via `okfLinks.rewriteFrontmatterWikilinks`). The raw-YAML `<textarea>` has NO ProseMirror suggestion engine, so there is **no live autocomplete, no `[[New Name]]` ⇥ type-picker, and no entity-creation**; an unresolvable name is left verbatim (looks like nothing happened). The architect flagged this as a known v1 limitation; the owner correctly finds it insufficient — it's half of the body gesture. The body gesture PASSED (OKF mode detection is fine), so this is frontmatter-surface-specific.

**Fix.** Bring the frontmatter editing surface to parity with the body: wire the `[[ ]]` autocomplete + silent-stub type-picker (`WikilinkSuggestion` / `createNoteRow`) into the frontmatter editor so a frontmatter `[[ ]]` gets the same fuzzy-pick + create-on-unresolved flow, then store the resolved rel-path (D7). Options: (a) replace the raw `<textarea>` with a CodeMirror/ProseMirror mini-editor hosting the suggester; (b) a textarea-anchored popover. Cross-ref ENH-216 D7, `FrontmatterPanel.tsx`, `extensions/WikilinkSuggestion.ts`, `extensions/createNoteRow.ts`.

### FOLLOWUP-052: Frontmatter `[[ ]]` entities should be click / cmd-click navigable

**Status:** 🆕 Filed (2026-06-15, owner walk-3 note on FOLLOWUP-051 — non-blocking). **Priority:** P2. **Effort:** S–M.

**Ask (owner).** "Need to be able to click or cmd-click on entities within frontmatter to navigate to them / open them." Body `[[ ]]` links + `[text](rel.md)` are already cmd-click navigable (MarkdownEditor `handleClickOn` → `resolveMdLinkInVault` / `resolveWikilinkInVault`). The Properties panel renders frontmatter values as plain text (`PropertyRow` in `FrontmatterPanel.tsx`), so a `[[Alice]]` / rel-path value isn't clickable. **Fix sketch:** in the panel's *view* (non-raw) render, detect a `[[name]]` (or markdown-link) value and render it as a clickable affordance that resolves like the body gesture (reuse `resolveWikilinkInVault` — note FOLLOWUP-051 removed its only other caller, so it'd come back). Cmd-click opens the target note. Applies to both vault modes. Cross-ref FOLLOWUP-051, `FrontmatterPanel.tsx` `PropertyRow`, `wikilinkResolver.ts`.

### FOLLOWUP-053: Deferred verification — OKF dialect-flip + Obsidian-compat (carry-forward SKIPs)

**Status:** 🟡 Deferred-test (2026-06-15, owner walk-3: "keep untested — persist task to test but does not block release"). **Priority:** P3. **Effort:** S (verification only, no code).

**What.** Two ENH-216 walk items the owner has SKIPped through walk-1 → walk-3, explicitly non-blocking: **(a) dialect-flip** — switching the active vault flips which serializer the body `[[ ]]` gesture uses (OKF rel-link vs `[[wikilink]]`); **(b) Obsidian-compat** — opening the OKF vault as a folder in Obsidian proper renders + navigates the markdown rel links. Both are code-complete; only owner-eyes verification is outstanding. Re-surface in a future smoke walk (per the carry-forward-SKIP rule) until walked, but they do NOT gate the v0.11.0 cut.

### ENH-220: View menu ▸ "Navigate to vault folder"

**Status:** 🆕 Filed (2026-06-15, owner walk-3 note — non-blocking). **Priority:** P2. **Effort:** S.

**Ask (owner).** "Need a View ▸ navigate to vault folder menu option." When editing a vault note, give a quick jump that points the navigator (`FileTree` / `useNavigator`) at the enclosing vault root (resolve via the active note's vault, like `resolveVaultForUi`). **Fix sketch:** add a View-menu item (Electron menu in `electron/main.ts`) + a matching `duo` verb (CLI/UI parity — `duo nav reveal <vaultRoot>` or similar) that sets the navigator cwd to the vault root. Gate it on there being an active vault. Cross-ref ENH-208/216, `FileTree`/`useNavigator`, the new-CLI-verb checklist.
...
### BUG-203: CriticMarkup ins/del render as raw `{++…++}` text on file parse; only live doc-verb application decorates

**Status:** 🆕 Filed 2026-06-09 (found staging about-duo screenshots in the Stoop window; renumbered from a colliding BUG-200 — the terminal-collapse fix merged as incumbent). **Priority:** High — it's the track-changes feature's persistence story. **Effort:** M.

**Symptom.** Comments (`{==…==}{>>…<<}`) parse + decorate correctly on every mount. Insertions/deletions do NOT: `{++…++}` / `{--…--}` tokens in the on-disk file render as literal brace text after any remount or chokidar refresh. Applying the same marks live via `duo doc insert` / `doc delete` decorates correctly (suggestion rail card, Accept/Reject) — until the next buffer reload degrades them to raw text. So a suggested-edits doc survives neither a tab close/reopen nor an app restart. Two adjacent finds, same session: (a) `duo doc edit --find "{~~…~>…~~}"` returned `ok:true, changed:true` but left the disk file untouched (rendered-text-space vs file-space mismatch — silent no-op, related to BUG-199 family); (b) suggestion cards show "(unattributed)" even with `DUO_AUTHOR=Claude` set on the inserting call.

**Why it matters.** CLAUDE.md § 4a routes agents to CriticMarkup for tracked suggestions; ENH-198 shipped the write path. If the marks don't survive a reload, accept/reject is only possible in the same sitting, and a raw-brace document looks corrupted to exactly the nontechnical audience the feature serves.

**Fix direction.** Parse ins/del/substitution tokens in the same markdown-load path that already handles comment tokens (substitution `{~~a~>b~~}` included — currently never decorates at all); thread author metadata from the token into the rail card. Regression test: fixture file containing all four token kinds → mount → assert decorations, not literal braces.

---

### BUG-201: Window re-follows macOS appearance on remount despite explicit `theme light` (multi-window)

**Status:** 🆕 Filed 2026-06-09. **Priority:** Medium. **Effort:** S.

**Symptom.** With macOS auto-switching to Dark in the evening, window 2 (secondary) kept reverting to dark across tab remounts and file opens even though `duo --window 2 theme` reported `{mode: light, effective: light}` throughout; each `duo --window 2 theme light` re-assert fixed the rendering transiently. Smells like a per-window theme listener applying `nativeTheme` updates without consulting the persisted explicit mode, under multi-window.

**Repro sketch.** macOS appearance Dark + Duo theme `light` → open/close working-pane tabs in a secondary window → window chrome + editor flip dark while `theme` still reports light.

---

### BUG-202: Markdown tab occasionally mounts the empty-state placeholder despite non-empty file (load race)

**Status:** 🆕 Filed 2026-06-09. **Priority:** Medium. **Effort:** S.

**Symptom.** `duo --window 2 edit roadmap/now-next-later.md` (35-line file) rendered the "Start typing — markdown shortcuts work…" empty-state with a stray `#`; close-tab + reopen loaded it correctly. One-off race between tab mount and file read; worth a defensive re-read or loading state before showing the empty placeholder, since "Duo blanked my doc" is a trust-killer for the target user.

---

### BUG-199: `duo doc edit` re-serializes the entire open document, churning untouched regions

**Status:** 🆕 Filed 2026-06-09 (second occurrence — first nearly corrupted the v0.9.2 cut commit). **Priority:** High. **Effort:** M.

**Symptom.** A single anchored find/replace via `duo doc edit` on a file open in the live editor rewrites far beyond the edited region: today's repro (this file) showed `git diff --numstat` of **83/77 lines for an 8-line insertion** — markdown links stripped to plain code spans (`[PRD](prd/…)` → "(PRD)", `[`docs/…`](…)` → bare backticks), bold boundaries re-serialized (`**…(owner-locked, via \`AskUserQuestion\`).**` re-split). Root cause family: the edit routes through the TipTap buffer, so tiptap-markdown's serializer re-emits the *whole* document with its own normalization rather than splicing the replaced span into the on-disk text. Same serializer-normalization family as BUG-107 (archived), but here it mutates *content* (links), not just trailing whitespace.

**Why it matters.** The doc-edit verb exists precisely so agents edit open files THROUGH Duo (CLAUDE.md § 4a); when the safe path silently corrupts formatting, agents must choose between fighting autosave (direct write) and lossy edits — both wrong. Recovery today: `git checkout` + direct `Edit` on the non-dirty, non-active tab.

**Fix direction.** `doc edit` should splice in text space (disk/buffer string), not through the ProseMirror model — e.g. apply the find/replace to the raw markdown and reload the editor from the result (the `core/markdown/plainEdit.ts` primitive already exists for ENH-195); or diff-minimize the serializer output against the pre-edit text and write only the touched span. Add a regression test: doc-edit an 8-line insertion into a fixture with reference links + bold runs; assert `git diff --numstat` ≈ insertion size.

---

### ENH-208: Graphbook — networked work-notes on plain Obsidian-vault conventions

**Status:** ⏳ Open — **Phases 1–2 SHIPPED** (Phase 1 CLI/model layers v0.10.1, PRs #83–#88; Phase 2 capture UX PR #91, owner-walked 8/8, shipped v0.10.2); Phases 3–4 open. PRD at [`docs/prd/enh-208-vault.md`](docs/prd/enh-208-vault.md) (D1–D22). *(This entry was stranded uncommitted in a local working tree during the Phase 1–2 sprints; grafted back into the ledger 2026-06-12.)* Playground at [`docs/research/graphbook-intent.html`](docs/research/graphbook-intent.html). **Priority:** High. **Effort:** L (phased). Filed 2026-06-09.

**Decisions locked (2026-06-09 playground walk, owner notes inline).** **D1 rollup format = Obsidian Bases** (`.base` files; pre-validated by the prototype). **D2 surface = affordance on the entity note → tab** — *owner: must work in both canvas and split-view ("should be trivial")*. **D3 processing interaction = CriticMarkup tracked suggestions + report note** — *owner: use Duo's exact CriticMarkup format (`core/markdown/criticmarkup.ts` / `duo doc insert\|substitute`), no variants*. **D4 capture = quick-capture** — *owner spec: Duo settings gains a **default vault** field (settings menu → select from vaults detected in the workspace, i.e. folders with `.obsidian/`); chord **⇧⌘N** creates a new templated inbox note (re-picked from ⌥⌘N — New Window collision) (frontmatter + formatting pre-loaded)*. **D5 first slice = conventions + skill (zero new UI)** — *owner: capture UX (B) is the fast-follow, "not useful til I'm in the habit"*. **D6 persistence = both** (live render in Duo + stamped artifact on demand). **Plus (owner directive):** ship the CLI verbs + a skill reference so Claude understands the feature — the full 4-surface sync (`cli/duo.ts`, `skill/SKILL.md`, `agents/duo.md`, `docs/CLI-COVERAGE.md`) **and** a what-duo-does.html entry. **Terminology:** "grooming" renamed → **processing** (all artifacts updated; prototype base is `bases/processing.base`).

**Round 3 locks (2026-06-09, via AskUserQuestion).** **(1) Naming: the feature set is just "vault"** — no graphbook noun anywhere user-facing: settings says "default vault", verbs are `duo vault init` / `duo graph …`, skill section is "Working in a vault". The on-disk reality (plain Obsidian vault) and the product name now match; existing research artifacts keep their historical "graphbook" filenames. **(2) Nested entities v1 = running doc + promote-on-demand** — e.g. per-initiative `Meetings.md` with `##` sections per meeting (one file, no flipping; sections addressable as `[[Meetings#2026-06-09]]`); when a section needs properties/base-visibility, a **processing op splits it out into an entity file** and leaves an `![[embed]]` in place (reversible). **Editable transclusion** (inline-editable embeds in Duo's editor — nested-ProseMirror work, uniquely Duo) is the deferred roadmap answer that makes discrete files read/edit as one doc; revisit after the capture habit exists. Note: Obsidian Bases tables ARE property-editable in place (cells write frontmatter back) — rollups are read-only for *body content* only; our renderer is read-only v1.

**Round 4 locks (2026-06-09, via AskUserQuestion — flatness concern).** **(1) D19 filing model:** per-type filing rule declared in the template — parentless types (person, theme) file in their type registry folder; parented types designate ONE frontmatter attribute as the filing parent (milestone → `initiative:`) and live under that parent's folder (loose vs `notes/` subfolder is a per-type knob); only folder-note types (initiatives) can be parents; contextless files fall back to time buckets `notes/YYYY/MM/` (the residue after processing assigns parents). All other relationships stay links — filing loses no edges. **(2) D20 archive:** processing PROPOSES moving completed initiatives (whole subtree) to `archive/YYYY/` in the report note, never automatic. Both folded into the PRD (D19/D20).

**Round 5 locks (2026-06-09, owner additions).** **(1) D21 smart insertions:** `@today`-family tokens inside the SHIPPED `@` mention suggester (extends `AtMention.ts`) — plain-text ISO date default, long-form/time variants as sibling entries; extensible via a token-provider registry; `/` reserved for a future block-insert menu; deliberate CLI asymmetry (agents write dates directly). **(2) D22 vault search:** global **⌘⇧F** searches the default vault (D11 setting; falls back to active file's vault) in a palette cloning the ⌘⇧A TabSearchPalette wiring; results open file-at-line; verified non-colliding (find-bar ⌘⇧F is input-local); CLI twin `duo vault search <query>`. Both land Phase 2 (capture-UX fast-follow).

**Pre-build assessment (2026-06-09, owner-requested).** Adversarial self-review at [`docs/research/vault-vs-obsidian.html`](docs/research/vault-vs-obsidian.html): audits all 22 PRD decisions against Obsidian 1.13 core + ecosystem (facts re-verified: Note composer extract IS core; insert-date IS core; no core global capture hotkey; ~66 community Obsidian MCP servers, none official). Findings: Phase 1 ≈ 85% divergence (processing/lint/prose-authoring/headless — no Obsidian counterpart), Phase 2 mostly justified duplication, **Phase 3 duplication-heavy** (second .base renderer vs native rendering with editable cells). Three open verdict decisions on the page: build shape (recommend: gate Phase 3 on habit evidence) · Obsidian-as-official-companion · engine spec policy (recommend: pinned subset + ⚠ unknowns). **Awaiting owner walk before Phase 1 starts.**

**Ask.** A "graphbook": interconnected work notes (themes, people, initiatives) captured as atomic markdown notes in a strict Obsidian vault — wikilinks + YAML frontmatter only, no invented syntax — with Duo supplying capture affordances (silent-stub entity creation via the existing `[[`/`@` suggesters + a type picker; `templates/` as soft schemas) and Claude supplying the agent layer (processing, synthesis, ad-hoc views) that Obsidian needs plugins for. New idea folded in: **live HTML rollups** — click a parent entity, Duo renders child-entity status from frontmatter, computed at render time and never persisted (no-sidecar rule). Locked so far: pure-Obsidian edge syntax · strict vault compat · folders+frontmatter typing · silent-stub create flow · templates/ schemas · atomic inbox capture · on-demand processing first (scheduled later). Open (round-1 cards D1–D5): rollup definition format (Bases `.base` vs Duo spec vs ad-hoc) · rollup surface/trigger · processing interaction model · capture entry point · first build slice.

**Round 1.5 (2026-06-09) — Bases deep-dive.** Owner pushbacks on round 1: (1) flat top-level `milestones/` folder rejected → per-initiative folders (Bases queries are frontmatter-driven, so folder layout is free ergonomics); (2) wants the type-picker reconciled against Obsidian's native `[[` behavior (Obsidian creates unresolved links untyped, in one global default folder — processing must catch Obsidian-side creations regardless); (3) open to *persisted* deterministic rollup renders ("mini-SSG") — revisit the sidecar litmus (build-artifact framing: regenerable + stamped, possibly gitignored); (4) requested a Bases expressiveness prototype → spec verified against Obsidian **1.13.0** docs; edge-probe vault at [`docs/research/graphbook-prototype/`](docs/research/graphbook-prototype/README.md) (open in Obsidian; README lists probes — headline: template-embedded `initiative == this` rollup blocks; known boundary: no joins, summaries terminal, child→parent aggregation unsupported). **Plus a working Duo-side renderer**: `render.mjs` (mini-SSG, zero new deps) parses the same vault + `.base` files and emits a stamped HTML artifact (`out/graphbook.html`) — verified 0 formula errors, all 7 bases render, row counts/probes hand-checked; child→parent rollup probes succeed in our renderer by design.

**Authoring plan (decisions locked 2026-06-09, via AskUserQuestion).** Three layers: **L0** schema model (`duo graph schema` — pure fn over vault frontmatter → types / entities / props-per-type / observed enums; computed live, no sidecar; same model processing needs); **L1** validator (`duo base lint` — structural + expression + corpus-resolution; emits JSON for agents); **L2** authoring surface. **Locked: (1) authoring = Claude-from-prose + validator FIRST** (zero new editor UI; editor autocomplete deferred — note existing `[[`/`@` suggesters fire in prose, NOT inside code-fenced base blocks, so in-editor completion is genuinely new CodeMirror work; form-builder is L2-v3 if needed). **(2) invalid-ref behavior = warn + render anyway** (loud ⚠ flag, never blocks; processing-proposes-fix deferred until the processing loop exists). Presentation (table/cards/list HTML) is **Duo-owned in `render.mjs`**, never user-authored — cell styling via `html()`/`icon()` formulas only. **L1 prototyped + verified**: `lint.mjs` derives the corpus from frontmatter (5 types, 22 entities, observed status enums) and statically checks `.base` expressions — clean on all 4 vault bases; on a broken fixture surfaces bad type / unresolved entity / bad enum value / unknown function / unknown view-type, each with a Levenshtein "did you mean".

---

### FOLLOWUP-049: Navigator P1 perf tier (D3/D4/D5) — deferred from ENH-211

**Status:** 🆕 Filed 2026-06-11 (deferred out of ENH-211 P0, owner-approved). **Priority:** P2 (perf/efficiency, **not** visible flicker — see finding). **Effort:** M (renderer-mostly; D3 needs a tiny preload tweak).

**Why deferred.** ENH-211 P0 (D1 stale-while-revalidate + D2 coalesce) shipped + live-verified, and the live MutationObserver walk found that the remaining mechanisms produce **no visible flicker**: a tracked-file modify in a git repo caused **0 `Loading…` flashes AND 0 navigator DOM mutations** — M3's git-tick re-render reconciles to identical DOM, i.e. wasted CPU, not a flash. So D3/D4/D5 are a **performance/churn** follow-up, correctly split from the user-visible P0 fix.

**Scope (all specified in the ENH-211 PRD § B.4 + § C).**
- **D3 — incremental watch (addresses M2).** Stop tearing down + recreating the chokidar watcher on every expand/collapse; call the already-plumbed `window.electron.files.updateWatchPaths(id, paths)` (`files-service.ts:486–512`, IPC `FILES_WATCH_UPDATE` already wired end-to-end) with the path delta instead of `stop()` + `watch()`-with-new-id, and drop the resubscribe re-list loop. Needs only a small `electron/preload.ts` tweak to expose the watch `id` to the renderer (today `files.watch` returns just `stop`). Also addresses the `MaxListenersExceededWarning: 11 files:changed listeners` watch-accumulation smell seen during the ENH-211 dev walk.
- **D5 — stable git-Map identity (addresses M3/M5/M6).** Deep-equal/return-prev guard in `setChildRepoMap`/`setDirtyFileMap`/`setGitSnap` so a `gitRefreshTick` with unchanged git state yields referentially-equal Maps (no row re-render); optional combined git-probe IPC to collapse M5's 3-probe fan-out; idempotent cwd/focus `git.status` (M6). `renderer/components/FileTree.tsx` (probe effects ~191–295, focus listener ~139–140).
- **D4 — memoize rows (addresses M3).** `React.memo` on `TreeNode`/`TreeNodes` with per-row **primitive** git props (compute `dirty?`/`repoChip?` at FileTree level; stop threading whole Maps to every row). Land **after** D5 so the comparator surface is minimal. Comparator must cover selected/expanded/active/dirty/repoChip/rename — needs a render-count RTL test.
