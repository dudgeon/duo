...
- **[P0 — TABLE STAKES] In-editor Cmd+Z correctness (independent of the modal).** The original complaint. Investigation (ADR) showed undo is *mechanically* intact, but it is **unverified live** and has at-risk paths. Acceptance criteria: (A) typing+autosave → Cmd+Z reverts my edits, save is invisible to undo; (B) an agent/`duo doc` write → one Cmd+Z reverts the whole agent change (redo restores it); (C) a disk-reconciliation reload → Cmd+Z reverts to the prior visible state, never a no-op; (D) a modal **Restore** → Cmd+Z reverts the restore (restore is just another undoable change); (E) **anti-pollution** — Cmd+Z NEVER appears to "do nothing" by consuming an invisible programmatic transaction; (F) undo survives tab-switch (BUG-046), dies only on tab close / restart (the volatility the modal addresses). **At-risk paths to verify + likely guard with `addToHistory:false`:** `applyCriticMarkupFromText` / `applyCommentMarksFromSidecar` re-dispatched after a reload (they currently dispatch WITHOUT `addToHistory:false` → can silently eat an undo step, violating E); the agent-write `setContent(_,true)` + `markJustAdded` + mark-reapply sequence (one logical change must = one undo step); whether `setContent(_,false)` reconciliation reloads *should* be undoable (product call — default: yes, but as ONE step). **Needs live Electron verify + a jsdom editor regression test** (recurring-regression rule). Note: the ENH-221 capture hook is main-process + fire-and-forget — it does NOT touch the renderer's ProseMirror history, so it cannot affect Cmd+Z.
