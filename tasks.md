...
**Post-review fixes (folded into PR #108 before merge, 2026-06-24).** Three findings from the PR code-review applied on-branch: **(a)** the digest `tests` chip now reads pass/fail from **tool-result text only** (runner stdout/stderr + `tool_result` blocks), never assistant prose or user prompts, so a transcript that merely *discusses* failures can't trip it (+2 regression tests); **(b) "You asked"** surfaces `/command args` instead of the raw `<command-name>`/`<command-args>` wrapper for slash-command turns (+1 test); **(c)** `sessionIdForTab`'s same-cwd ambiguity is now documented in code. **Non-blocking follow-ups (P2, not yet ticketed — ENH-235 taken):** **(i) robust tab→session resolution** — thread Claude Code's hook-stdin `session_id`/`transcript_path` through `duo session digest|note|next` instead of re-deriving from the cwd, eliminating the same-cwd `note`/`next` misattribution race; **(ii) poll-active-mode-only** — Home deliberately keeps BOTH the projects + catch-up snapshots warm while active (for instant mode-switching + a fresh `duo home state`); revisit gating the inactive view's 30s poll only if background IO becomes a concern.

**Merged & shipped (PR #108, 2026-06-24).** Both walk-2 FAILs (CU-DONE-IA + CU-WORKTREE-GONE) fixed + re-walked; review fixes above folded in. **Next: cut a version.** Deferred ENHs: ENH-232 (rich worktree re-entry), ENH-233 (dismiss/mark-reviewed). P2: cron management. SKIP: greeting, empty-state.
...
**Status:** ✅ Shipped 2026-06-25 ([PR #109](https://github.com/dudgeon/duo/pull/109) merged to main — owner-directed merge; the earlier-handed-off v0.12.2 smoke-walk paste-back waived by the merge directive). Decisions FINAL; PRD written. **Phases 1–4 ✅** on [PR #109](https://github.com/dudgeon/duo/pull/109) (rebased onto main = ENH-231 #108): the **`type: rollup` model** (`templates/rollup.md` both scaffolds · `core/vault/rollup-notes.ts` `listRollups`/`resolveRollupNote`/surgical `stampRollupProvenance` · `core/vault/inbox.ts` `listInbox`); **HTML-first** `duo rollup render <note>` (D2 — flips ENH-229 D3) + new `duo rollup list`; **vault IPC** (`listInbox`/`listRollups`/`get`+`setDefault`); the **pinned Vault view** (present-when-default, inbox newest-first + stale chip + `+ Capture`, rollups freshness chip + View-artifact + `+ New rollup` → seeded Claude session, header vault-switcher). **Two live-walk bugs fixed:** the Vault tab clobbered by session restore (boot race), and the New-Vault-dialog not broadcasting the default change. typecheck clean · **full suite 2049 green** (incl. 3 post-review tests) · `check:skill-currency` PASS · 4-surface sync + `build:cli` + `sync:claude` done · end-to-end CLI smoke + cold-boot DOM probes + eyes-on screenshot all confirmed. **Post-review fixes (folded into PR #109 before merge, 2026-06-25):** **(#1)** rollup provenance now stamps ONLY on the note's canonical-format render — an ad-hoc `--md`/`--html` override is a side artifact and no longer repoints `out:` (which would mismatch `format:` on the next default render) or mark the note fresh; new pure `provenanceStamp()` decision (+2 tests). **(#2)** `listRollups` confines `out:` to the vault — a `..`/absolute escape → `out: null` so the Vault view never opens a file outside the vault (+1 test). **(#3)** documented the two-signal `stale` contract (never-rendered vs out-of-date). **(#4 — not changed, intentional):** the inbox stale boundary uses a UTC date, which is internally consistent with how `captured:` is stamped (`toISOString`); switching to local time would require changing capture-dating too (out of scope, and would risk a real inconsistency). **Next:** cut a version. **Priority:** TBD (owner-requested). **Effort:** M (view) + M (rollup-lifecycle change — typed-note model). **Filed** 2026-06-24. **Extends** ENH-229 (rollup family). **Playground:** [`docs/research/vault-inbox-rollups-view.html`](docs/research/vault-inbox-rollups-view.html) (decision artifact, rule 11). **PRD:** [`docs/prd/enh-228-vault-view.md`](docs/prd/enh-228-vault-view.md).

**Decisions FINAL (owner playground submission, 2026-06-24 — recommended option on every card; supersedes the earlier quick-round answers).** **D1 discovery = rollup is a first-class `type: rollup` note** — new `templates/rollup.md` owning its spec (embedded base block / `spec:`) + render provenance (`last_generated`/`last_hash`/`out`); discovery = `type == rollup` corpus query (no scan, no sidecar; §D9-clean — the note owns its spec). **HTML-first** (owner: *"the rollups I'm most interested in producing are HTML based, not MD"*) — `format: html` default; nudges ENH-229 D3 (MD default). **D2 view subject = the default vault** (`vault.json` + header switcher among `knownVaults`), reverses the earlier active-file pick. **D3 tab = pinned beside Home, shown when a vault is selected.** **D4 "+ New rollup" = prefilled Claude session** (authoring loop → writes the `type:rollup` note, renders HTML to `rollups/`). **D5 inbox = all notes, stale>1wk flagged.** **D6 first slice = full, incl. the typed-note lifecycle change.** New verb `duo rollup list` (corpus query). Build order in the PRD § B.
