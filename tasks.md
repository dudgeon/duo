...
**Lesson:** before filing a new CLI verb, grep `docs/CLI-COVERAGE.md` for the capability. The agent-discoverability rule (CLAUDE.md § 4) means existing verbs SHOULD be visible from the cheat-sheet; if it wasn't, that's a documentation gap, not a missing verb.
...
**Known limitation — v0.6.13/v0.6.14 legacy orphans not auto-cleaned.** The diff-based cleanup requires the orphan to appear in `prevShas` (last install's SHA map). v0.6.13 retired `help/faq.html`, `help/what-duo-does.html`, and `packs/claude-code-basics/` but the v0.6.13 install_re-wrote_ installed.json with the new file map (excluding the retired paths), so the SHAs needed to safely auto-delete those legacy orphans were lost. v0.6.15+ users will have correct provenance going forward, and retirements starting from this cut get cleaned up. The legacy orphans are inert (don't break anything; `pins.json` URLs that pointed at them are migrated separately in the same commit) — existing users can remove by hand if desired.
