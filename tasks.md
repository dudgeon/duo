...
**Open sub-decision — dismiss / "mark reviewed" (PRD D6b).** Claude Code has NO native done/archive flag (researched). Pending owner call between a Duo-owned `dismissed` annotation (home-state.json — where review-state legitimately belongs per the §D9 ADR) vs a `custom-title` sentinel (session-SOR but co-opts the title + needs a guarded `.jsonl` append). Deferred — the Done-IA fix reduces urgency.

**Remaining:** finish the owner walk → merge #108 → cut from main (branch is v0.12.1, main v0.12.2).
