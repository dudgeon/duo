...
**Pre-cut batch (2026-06-14 → v0.11.0).** DONE: ENH-216 core+UI (committed, rebased on `main` 0.10.4, 1542 tests); FOLLOWUP-050 frontmatter autocomplete (implemented, awaiting walk-2); BUG-208 `--help` Vault group (✅); the worksheet copy-button fix. PENDING OWNER GO: FOLLOWUP-051 (flip frontmatter edges rel-path→`[[ ]]`, recommended); BUG-207 sidecar S1+S4; ENH-214 templates-in-search (option a). Smoke walk trimmed to walk-2 (3 unresolved items, `docs/dev/smoke-walks/v0.10.4-rev2.html`). Cut once the pending items land + walk-2 passes.
