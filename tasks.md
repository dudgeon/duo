...
**Status:** 🚧 In progress (2026-06-24) — decisions FINAL (owner playground submission); PRD written. **Phase 1 (core + CLI) ✅ + Phase 2 (skill/agent docs) ✅:** `type: rollup` model — `templates/rollup.md` (both scaffolds), `core/vault/rollup-notes.ts` (`listRollups`/`resolveRollupNote`/surgical `stampRollupProvenance`) + `core/vault/inbox.ts` (`listInbox`); `duo rollup render <note>` reads the spec → renders **HTML by default** (D2, flips ENH-229 D3) → stamps provenance surgically; new `duo rollup list`; 4-surface sync + `build:cli` + `sync:claude` (check:skill-currency PASS); 10 new core unit tests + the bounded scaffold/okf type-set updates (now 6-type) — full vault suite 243/243. **Remaining:** Phase 3 (IPC: listInbox/listRollups/get-set-default), Phase 4 (renderer VaultView + pinned tab), Phase 5 (smoke-walk + cut). **Priority:** TBD (owner-requested). **Effort:** M (view) + M (rollup-lifecycle change — typed-note model). **Filed** 2026-06-24. **Extends** ENH-229 (rollup family). **Playground:** [`docs/research/vault-inbox-rollups-view.html`](docs/research/vault-inbox-rollups-view.html) (decision artifact, rule 11). **PRD:** [`docs/prd/enh-228-vault-view.md`](docs/prd/enh-228-vault-view.md).
