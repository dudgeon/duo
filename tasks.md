...
**Status:** 🚧 Code-complete (Stages 0–5, 2026-06-13) — typecheck clean, full suite 1425/1425 green (incl. new `okf.test.ts` 40 · `move`/`listings`/`vaultLinks`), `cli/duo` rebuilt, check:skill-currency 70 verbs/0 failures. **Awaiting live-UI smoke-walk** (New Vault dialog · `[[ ]]`→md-link expand-on-resolve · cmd+click rel-link nav · frontmatter `[[owner]]` rewrite · auto-relink-on-open) before any cut. **Priority:** P1 (owner-initiated). **Effort:** L (multi-tier).
