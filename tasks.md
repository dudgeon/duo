...
- **[P1, UI — AWAITING OWNER DECISION] History view surface + per-version diff.** **DECIDED 2026-06-20:** surface = **B · modal** (owner: avoids right-rail collision with comments/tracked-changes/properties); restore = **overwrite-with-confirm**; rows = **standard** (time · who · size delta); diff = **rendered inline-highlight in the modal's preview pane** (a read-only TipTap instance reusing ENH-197 `applyTrackedDiff`; NOT raw `+/−` source, NOT side-by-side). Open companion question: pair with the save-chip popover (D) as a fast-follow for one-click quick-restore? Build items: modal shell + WCV parking (BUG-153/209), read-only diff editor inside it, line-diff fn. Decision playground: [docs/research/enh-221-history-view.html](docs/research/enh-221-history-view.html). Needs live Electron verify + smoke-walk.
