...
**Status:** 🟡 Awaiting owner decision (2026-06-14, surfaced reviewing a real OKF person note — owner asked "should `team:` be a `[[kids]]` link?"). **Priority:** P1 (gates FOLLOWUP-050's insert form + the ENH-216 cut). **Effort:** S (subtractive).

**Finding (verified empirically).** D7 stores OKF frontmatter edges as a YAML-quoted rel-path (`team: "../notes/2026/06/kids.md"`). Tested both forms in a scratch OKF vault: `duo graph backlinks kids` returns ONLY the note using `team: "[[kids]]"`; the rel-path note does NOT register and shows in `duo graph orphans`. So the rel-path form **silently drops the relationship from Duo's knowledge graph**. Cause: `extractLinkRefs`/`parseLinkish` (`core/vault/{parse,engine}.ts`) match `[[…]]` (+ body markdown links), NOT a bare frontmatter path — D7's recommendation premise ("Duo resolves it to a graph edge") is **false**.
