...
**Progress:** ✅ Phase 0 (browser-runnable renderer + mock seam) — verified live, full shell renders in a plain browser, 0 render errors (`bfc3c12`). ✅ Phase 1 infra (Storybook on the Vite builder + global mock decorator + first exemplar CSF stories) — verified (`97ed30f`). ✅ **D3 bulk story pass** — parallel 9-agent workflow authored **67 story files / 202 stories** covering the whole component library (MarkdownEditor deferred — needs a live TipTap instance). Verified exhaustively: typecheck clean, `build-storybook` clean, and **all 202 stories render with zero error overlays / uncaught errors** (render-checked against the static build via the preview harness). Harness guide: [`docs/dev/storybook-harness.md`](dev/storybook-harness.md).

**Close-out — the design loop (the ENH-238 goal) is delivered.** `npm run preview:ui` (whole-app, no Electron) + `npm run storybook` (202 isolated stories). All dev-tooling — nothing ships in the Electron bundle, so no version cut is warranted.

**Deferred follow-ups (locked-decision deviations, surfaced not dropped):**
- **`/design-sync` round-trip (D4) — owner-gated, NOT abandoned.** The DesignSync tool needs `/design-login` in an *interactive* terminal (or Claude Design's "Send to Claude Code Web") — unavailable in a headless/web session — AND a preview-card bundle build (the `/design-sync` skill). Handoff: owner runs `/design-login` in a real Claude Code terminal, then `/design-sync`. Tracked here until done.
- **`@duo/ui` extraction (D4) — recommend defer.** `/design-sync` reads the repo directly, so extraction is mostly churn (would rewrite all 202 stories' import paths) for marginal gain. Revisit only if a *published* shared package is needed. **Owner: override if you still want it now.**
- **Lost Pixel visual-regression (D5) — recommend defer until CI exists.** Its payoff is a CI gate; there's no CI in the repo, and standing it up now means committing ~202 baseline PNGs speculatively. Pairs naturally with a future "add CI" decision.
