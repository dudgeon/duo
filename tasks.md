...
- **Next:** `/smoke-walk` covering all of ENH-221 (owner option (a) — one combined walk), then propose a cut.
...
- **Owner smoke-walk 1 (2026-06-20): 2 PASS / 1 SKIP.** LONE-MAIN ✅, REMOVAL ✅ (with a caveat — *the banner was illegible*), CREATE ⏭️ SKIP (owner didn't walk it). `4475df8` **fix** — the removal banner hardcoded a dark bg but used theme ink vars for text → dark-on-dark in **light mode**. Hardcoded light text/glyph colors (legible dark toast in both themes); verified live in light mode (body `rgb(216,205,180)` on bg `rgb(42,32,26)`). **Owed:** owner re-glance of the banner (or trust the verify) + a call on the CREATE SKIP (walk it or trust the pre-walk). **Blocker before any cut:** the ENH-221 + v0.11.2 collision with the *other* agent's `claude/enh-221-file-history` (file version history) is still unresolved — needs an owner decision on which keeps ENH-221/v0.11.2 + the merge order; next free number is ENH-222.
