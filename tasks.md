...
**🛑 TWO ISSUES OPEN BEFORE THE SMOKE-WALK (owner, 2026-06-13 — resolve FIRST, then `/smoke-walk` → cut):**
- **TITLE-1 — session identifier still shows first prompt, not the Claude summary.** Owner still perceives first-prompt titles dominating. Prior finding (ai-title IS the summary + IS used at ladder rung 2; sessions-index.json deprecated + ENH-183 C1-locked) did NOT satisfy → **re-investigate, don't restate**. Angles: ai-title read window (`readSessionHeadMeta` head 16KB + tail 16KB — freshest sessions Home shows may have no ai-title yet, or it sits beyond the tail in a huge file); low ai-title coverage on recent sessions → maybe derive a better title for recent-unsummarized; confirm no newer summary source. Plumbing: `electron/claude-session-tracker.ts` (`titleFromLines`/`readSessionHeadMeta`/`cleanAndTruncate`/`TAIL_META_BYTES`), `electron/home-snapshot.ts` (title assembly). Cross-ref ENH-183 § D5/C1.
- **RESP-1 — two-hero responsive breakpoints need work.** Side-by-side → stacked is a container-query at ~720px (`renderer/components/Home/Home.css` `.duo-home-heroes` + `container-type` on `.duo-home`). Heroes likely crush at intermediate pane widths / threshold wrong for real split-pane widths. Tune + verify live across widths (`duo split <pct>`).
