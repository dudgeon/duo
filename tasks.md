...
**Status:** 🚧 Built + live-verified, awaiting owner smoke-walk (2026-06-24). Decisions FINAL; PRD written. **Phases 1–4 ✅** on [PR #109](https://github.com/dudgeon/duo/pull/109) (rebased onto main = ENH-231 #108): the **`type: rollup` model** (`templates/rollup.md` both scaffolds · `core/vault/rollup-notes.ts` `listRollups`/`resolveRollupNote`/surgical `stampRollupProvenance` · `core/vault/inbox.ts` `listInbox`); **HTML-first** `duo rollup render <note>` (D2 — flips ENH-229 D3) + new `duo rollup list`; **vault IPC** (`listInbox`/`listRollups`/`get`+`setDefault`); the **pinned Vault view** (present-when-default, inbox newest-first + stale chip + `+ Capture`, rollups freshness chip + View-artifact + `+ New rollup` → seeded Claude session, header vault-switcher). **Two live-walk bugs fixed:** the Vault tab clobbered by session restore (boot race), and the New-Vault-dialog not broadcasting the default change. typecheck clean · **full suite 2046 green** · `check:skill-currency` PASS · 4-surface sync + `build:cli` + `sync:claude` done · end-to-end CLI smoke + cold-boot DOM probes + eyes-on screenshot all confirmed. **Remaining:** owner walks the **v0.12.2 smoke-walk** (handed off, pinned in aux) → paste-back → cut. **Priority:** TBD (owner-requested). **Effort:** M (view) + M (rollup-lifecycle change — typed-note model). **Filed** 2026-06-24. **Extends** ENH-229 (rollup family). **Playground:** [`docs/research/vault-inbox-rollups-view.html`](docs/research/vault-inbox-rollups-view.html) (decision artifact, rule 11). **PRD:** [`docs/prd/enh-228-vault-view.md`](docs/prd/enh-228-vault-view.md).
