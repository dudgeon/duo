...
| `duo doc delete <file> --text "X"` | Wrap X as a CM deletion. ENH-260 D5 — composes with pending `{++…++}` insertions: X inside one shrinks/drops the token (net-zero); a range spanning insertion + plain text decomposes. `changed=false` if X overlaps an existing deletion/substitution/highlight/comment token (split the op). |
| `duo doc substitute <file> --text "X" --with "Y"` | Wrap X→Y as CM substitution. Empty `--with` = effective delete. ENH-260 D5 — X fully inside a pending `{++…++}` insertion edits that suggestion's text in place; a spanning range decomposes then appends `{++Y++}`. Same overlap refusal as delete for non-insertion tokens. |
...
| `duo pull [<path>] [--force] [--json]` | **ENH-253** — fetch + pull the latest changes for the repo at `<path>` (default: cwd). A clean, behind-only checkout fast-forwards automatically; a clean but diverged checkout auto-merges (a real content conflict aborts the merge, changing nothing, `errorKind: 'merge-conflict'`). A dirty tree (tracked modifications only — untracked files don't count) refuses with `errorKind: 'needs-confirmation'` (`{ changedCount, aheadCount, behindCount }`) — re-run with `--force` to discard local changes and hard-reset to the remote. `--json` prints the structured `PullResult` (exits non-zero on failure) `{ ok, result, commitsApplied, branch, errorKind, error, ... }` with `result` ∈ `{ up-to-date, fast-forwarded, merged, discarded-and-pulled }`. Reads/writes git directly (no running app needed). The CLI twin of the navigator's repo-root right-click "Pull latest changes". |
...
| `duo vault init <path> --format=okf\|obsidian [--name "…"] [--no-default] [--force]` | Scaffold a new vault. `--format` is REQUIRED on the CLI. **okf** = standard markdown relative links `[Display](./<note>.md)` + root `okf_version` `_index.md` marker (ENH-245 default; legacy `index.md` still detected) + static listings, no `.obsidian/`. **obsidian** = wikilinks `[[Display]]` + `.obsidian/` marker + `bases/processing.base` + README (legacy). Starter templates with D19 filing rules either way. The fresh vault becomes the default unless `--no-default` (it still lands in `knownVaults` either way). Refuses to clobber unless `--force`. JSON `{root, created[], warnings[], mode, madeDefault}`. |
| `duo vault list` | Vaults detected from the cwd (an OKF `okf_version` `_index.md`/`index.md` OR an `.obsidian/` marker). JSON `[{root, name, noteCount}]`. These vault verbs read the filesystem directly — no running app needed. |
...
| `duo vault relink --frontmatter [--dry-run] [--vault <path>]` | **ENH-266** — the frontmatter-link migration (OKF-mode only). **Runs AUTOMATICALLY on vault open** (same write/report + foreign-bundle gating as `relink`, so a legacy OKF vault just works in Obsidian); this verb runs it explicitly for `--dry-run` preview or headless use. Four categories, each id-then-slug resolved (ambiguous/broken reported, never guessed): (a) frontmatter `[[wikilink]]` values → the quoted markdown-link form (a title-based frontmatter wikilink is an unresolved phantom node in Obsidian — OKF filenames are slugs, not titles); (b) bare unbracketed frontmatter rel-paths (invisible to every link parser) → the quoted markdown-link form, only when the path resolves to a real file; (c) leftover prose-BODY wikilinks (someone edited the vault in Obsidian) → markdown links; (d) alias backfill — a `title:` that differs from its slug filename gets appended to `aliases:`. Generated-listing marker files are never touched. `--dry-run` reports without writing. JSON `{frontmatterWikilinks:{repaired[],ambiguous[],broken[]}, frontmatterBarePaths:{…}, bodyWikilinks:{…}, aliasBackfills:[{fromRel,title}]}`. |
| `duo vault publish [--index-only\|--log-only] [--dir] [--open] [--vault <path>]` | (Re)generate the OKF static listings from the corpus — root index (frontmatter byte-preserved; `_index.md` default, `index.md` legacy — ENH-245) + log (`_log.md`/`log.md`, whichever the vault already uses); `--dir` adds per-folder index files. `--open` surfaces the root index as a tab. OKF-mode only (throws in Obsidian mode — Obsidian stays byte-identical). ENH-230: a `listing:` base spec in the root index frontmatter drives the body through the shared engine; an unusable spec falls back to the default and is reported in `warnings`. JSON `{mode, written[], warnings[]}`. |
...
| `duo rollup new --type <t[,t2]> [--title "<t>"] [--group a,b] [--bucket <v[=Label]>]... [--filter <k=v\|k!=v\|k~=v\|k^=v\|@=v\|k?\|k!?>]... [--columns a,b] [--vault <path>]` | **ENH-243** — scaffold a builder-canonical `type: rollup` note at `rollups/<slug>.md` (the Rollups tab's CLI twin). The ordered `--group` list is multi-depth grouping: level 1 mirrors into the base block's `groupBy`, the full list rides the note's `group_by:` frontmatter. **ENH-255:** `k~=v` filters MEMBERSHIP on a multi-valued field (a list-of-links matches on the linked note's IDENTITY — `tracks~=q3-launch` matches `[[Q3 Launch]]` however it's labeled); **ENH-259:** `k^=v` is transitive "is under" (any_parent) — `parent^=California` keeps rows whose `parent` chain reaches California anywhere up the tree (`ancestors("parent").contains("California")`, cycle-safe); **ENH-262:** `@=v` is "links to" — the note links entity v through ANY property or prose (`file.hasLink`; unions owned-via-parent + monitored-via-tracks populations); **ENH-261:** `--group`/`--columns` accept the ancestor-TYPE token `ancestor:<prop>:<type>` (`--group ancestor:parent:goal` groups rows by the goal each rolls up to). Entity operands are canonically the note's SLUG; a display name matches when it folds to the basename (punctuation stripped); `--bucket 'value[=Label]'` (repeatable, needs `--group`) declares level-1 buckets that ALWAYS render — even empty — in flag order under their labels; splits at the first unescaped `=` (`\=` escapes a literal `=` in the value). JSON `{root, note, absPath, model}`. |
| `duo rollup show <note> [--vault <path>]` | **ENH-243** — the parsed builder model + row/group summary. `model: null` = hand-authored spec (view-only in the GUI); `error` set = broken (→ doctor). **ENH-255:** `warnings[]` lists filter expressions that ERRORED (rows dropped without being genuinely filtered — check it before trusting a zero-row rollup); `buckets[]` gives each declared bucket's label + matched group key (null = empty). JSON `{root, note, title, model, groupBy, buckets, warnings, columns, rowCount, error}`. |
| `duo rollup set <note> [--title "<t>"] [--type <t[,t2]>] [--group a,b] [--bucket …]... [--clear-buckets] [--filter …]... [--columns a,b] [--clear-filters] [--links github\|relative] [--vault <path>]` | **ENH-243/248** — mutate a builder-canonical rollup (filters APPEND unless `--clear-filters`; **ENH-255:** `--bucket` REPLACES the declared-bucket list wholesale — order is render order; `--clear-buckets` drops them); refuses a hand-authored spec rather than clobbering it. `--links github` persists the note's entity-link mode (`links:` frontmatter — rendered pages link entities to their GitHub blob; works alone, and on hand-authored notes). JSON `{root, note, model?, links?}`. |
...
| `duo rollup markdown <note> [--vault <path>]` | **ENH-244** — the rendered artifact's "Copy as Markdown" CLI twin (ENH-248 moved the button out of the Rollups tab header — the artifact carries it). Prints ONE raw GFM table to stdout (pipe to `pbcopy` or a file) — title cells link to the entity's GitHub blob when the vault root sits inside a GitHub-remote repo (branch = current HEAD), else a vault-relative `./path` link (works in OKF too). One git probe for the whole table. |
| `duo rollup delete <note> --force [--vault <path>]` | **ENH-248 R6** — remove a rollup: the definition note AND its rendered artifact (the rail menu's CLI twin). Without `--force`: dry run printing `wouldDelete` (mechanical guard; file history is the note's undo net). JSON `{root, deleted}`. |
| `duo rollup duplicate <note> [--vault <path>]` | **ENH-248 R6** — copy a rollup note as `"<Title> (copy)"` at `rollups/<slug>-copy.md`, provenance STRIPPED (the copy renders its own artifact on next save/render). Works on hand-authored notes (raw copy). JSON `{root, note, absPath}`. |
| `duo rollup render … --github` | **ENH-248 R8** — this render's entity links become GitHub blob URLs (same probe as `markdown`; stderr-warns + falls back to relative without a GitHub remote). A note with `links: github` gets blob links on EVERY render automatically — the GUI's auto-render-on-save included. |
