...
| `duo split-view <op>` | ENH-041 / Sprint 3 — Split View aux pane (canvas's right-side companion slot). Sub-verbs: `open <path>` (open path in aux; moves from main if already there — single source of truth), `close`, `promote` (move aux's tab to main, close aux), `resize <pct>` (0.20–0.80 or 20–80, clamped), `state` or no arg (prints current snapshot JSON). v1 single-slot; multi-tab aux is queued for v2. **Default opening location is ALWAYS main** — never autonomously open in split. Trigger words that route to split: "in split", "in split view", "in the split", "alongside", "side by side", "see these side by side", "in the side panel", "as a companion", "open this in the side". Anything else → main. Use when the user explicitly asks for a companion view (preview alongside source, lesson + playground, reference doc + working file, smoke walk steps + linked files). |
