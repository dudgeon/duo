...
| `duo session open <uuid> [--cwd <path>] [--force]` | The Home click contract: if a live terminal tab already hosts `<uuid>`, **focus** it (raising its window) — never duplicates; else **resume** it in a new tab in the primary window (`--cwd` required; live-outside-Duo refused unless `--force`). Returns `{ ok, action: 'focus'\|'resume' }`. |
