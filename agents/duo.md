...
| `duo rollup set <note> [--title "<t>"] [--type <t[,t2]>] [--group a,b] [--filter …]... [--columns a,b] [--clear-filters] [--links github\|relative] [--vault <path>]` | **ENH-243/248** — mutate a builder-canonical rollup (filters APPEND unless `--clear-filters`); refuses a hand-authored spec rather than clobbering it. `--links github` persists the note's entity-link mode (`links:` frontmatter — rendered pages link entities to their GitHub blob; works alone, and on hand-authored notes). JSON `{root, note, model?, links?}`. |
