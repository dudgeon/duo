  ...
  try {
    switch (cmd) {
      ...
      case 'rollup': {
        ...
        const vaultFlag = flagValue(subRest, '--vault')
        const USAGE =
          'Usage: duo rollup <render|list|diff|new|show|set|doctor|markdown|delete|duplicate> [<note|base>] [--md|--html] [--github] [--style <css-file>] [--summary "<text>"|--no-summary] [--against <path>] [--type <t[,t2]>] [--title "<t>"] [--group a,b] [--bucket <value[=Label]>]... [--clear-buckets] [--filter <k=v|k!=v|k~=v|k^=v|k?|k!?>]... [--columns a,b] [--clear-filters] [--links github|relative] [--out <path>] [--open] [--force] [--vault <path>]'
        ...
        if (sub === 'render') {
          ...
          const NEW_USAGE =
            'Usage: duo rollup new --type <t[,t2]> [--title "<t>"] [--group a,b] [--bucket \'value[=Label]\']... [--filter \'k=v\' | \'k!=v\' | \'k~=v\' | \'k^=v\' | \'k?\' | \'k!?\']... [--columns a,b] [--vault <path>]'
          const SET_USAGE =
          ...
          const parseFilterFlag = (s: string): vault.BuilderFilter => {
            ...
            die(`duo rollup ${sub}: unrecognized --filter ${JSON.stringify(s)} (use k=v, k!=v, k~=v, k^=v, k? or k!?)`)
