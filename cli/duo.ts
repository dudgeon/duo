...
const VERBS: VerbSpec[] = [
  ...
  },
  {
    name: 'goto',
    group: 'Files & navigator',
    args: '<home|vault|rollups>',
    summary:
      'ENH-257 — switch a window to its synthesized Home/Vault/Rollups top-level tab (same activation a sidebar/tab-strip click does). Distinct from "view"/"edit", which don\'t recognize these tabs\' duo:// sentinel paths. "vault"/"rollups" error (not a silent no-op) when no default vault is set, since that tab doesn\'t exist yet.'
  },
  {
    name: 'image',
    group: 'Files & navigator',
...
async function main(): Promise<void> {
  ...
  try {
    switch (cmd) {
      ...
      case 'goto': {
        // ENH-257 — `duo goto <home|vault|rollups>`: switch a window's
        // top-level tab (view/edit don't recognize these tabs' duo://
        // sentinel paths — see NavBridge.openWorkspaceTab).
        const kind = rest[0]
        if (kind !== 'home' && kind !== 'vault' && kind !== 'rollups') {
          die('Usage: duo goto <home|vault|rollups>')
        }
        const r = (await send('goto', { kind })) as { ok?: boolean; error?: string }
        if (r && r.ok === false) die(r.error ?? 'duo goto failed')
        out(r)
        break
      }
      case 'image': {
      ...
      case 'vault': {
        ...
        if (sub === 'init') {
          ...
          if (subRest.includes('--clear') && subRest.includes('--init')) {
            // Mutually exclusive (the CLI-COVERAGE grammar shows `… | --clear`);
            // reject rather than let --clear silently win (review fix).
            die('cannot use --clear and --init together')
          }
          ...
          // without writing.
          //
          // ENH-266 `--frontmatter` — a SEPARATE, explicit, opt-in migration
          // (never reachable from the auto-relink-on-vault-open hook):
          // rewrites frontmatter wikilink/bare-path entity-reference values to
          // the quoted markdown-link form, converts leftover prose-BODY
          // wikilinks, and backfills title aliases. OKF-mode only (throws
          // otherwise — Obsidian frontmatter is unaffected).
          const root = vault.resolveVaultOrDefault(process.cwd(), vaultFlag)
          if (subRest.includes('--frontmatter')) {
            out(vault.migrateFrontmatterLinks(root, { dryRun: subRest.includes('--dry-run') }))
          } else {
          ...
          die(
            'Usage: duo vault <init|list|schema|capture|stub|search|default|mv|relink|publish|promote> [args]\n' +
              ...
              '  default [<path> [--init]|--clear]  read / set the default vault (--init = create-on-choose)\n' +
              ...
              '  relink --frontmatter [--dry-run]   ENH-266 opt-in migration: frontmatter wikilinks/bare-paths → quoted md-links, body wikilinks, alias backfill (OKF-only; never auto-run)\n' +
      ...
      case 'rollup': {
        ...
        const vaultFlag = flagValue(subRest, '--vault')
        const USAGE =
          'Usage: duo rollup <render|list|diff|new|show|set|doctor|markdown|delete|duplicate> [<note|base>] [--md|--html] [--github] [--style <css-file>] [--summary "<text>"|--no-summary] [--against <path>] [--type <t[,t2]>] [--title "<t>"] [--group a,b] [--bucket <value[=Label]>]... [--clear-buckets] [--filter <k=v|k!=v|k~=v|k^=v|@=v|k?|k!?>]... [--columns a,b] [--clear-filters] [--links github|relative] [--out <path>] [--open] [--force] [--vault <path>]'
        ...
        if (sub === 'render') {
          ...
          const NEW_USAGE =
            'Usage: duo rollup new --type <t[,t2]> [--title "<t>"] [--group a,b] [--bucket \'value[=Label]\']... [--filter \'k=v\' | \'k!=v\' | \'k~=v\' | \'k^=v\' | \'@=v\' | \'k?\' | \'k!?\']... [--columns a,b] [--vault <path>]'
          const SET_USAGE =
            'Usage: duo rollup set <note> [--title "<t>"] [--type <t[,t2]>] [--group a,b] [--bucket \'value[=Label]\']... [--clear-buckets] [--filter …]... [--columns a,b] [--clear-filters] [--links github|relative] [--vault <path>]'
          const root = vault.resolveVaultOrDefault(process.cwd(), vaultFlag)
          const list = (flag: string): string[] | null => {
            const v = flagValue(subRest, flag)
            return v == null ? null : v.split(',').map((s) => s.trim()).filter(Boolean)
          }
          // Every --filter / --bucket occurrence (flagValue reads only the first).
          const filterArgs: string[] = []
          const bucketArgs: string[] = []
          for (let i = 0; i < subRest.length; i++) {
            if (subRest[i] === '--filter' && subRest[i + 1] != null) filterArgs.push(subRest[++i])
            else if (subRest[i] === '--bucket' && subRest[i + 1] != null) bucketArgs.push(subRest[++i])
          }
          const parseFilterFlag = (s: string): vault.BuilderFilter => {
            let m = s.match(/^([\w-]+)!=(.+)$/)
            if (m) return { property: m[1], op: 'ne', value: m[2] }
            // ENH-262 — @=v: "links to" — the note links entity v through ANY
            // property (or prose); the union primitive when populations reach
            // one entity via different fields (owned: parent → track node;
            // monitored: tracks[] → track node).
            m = s.match(/^@=(.+)$/)
            if (m) return { property: vault.LINKS_TO_PROP, op: 'linksto', value: m[1] }
            // ENH-259 — k^=v: transitive "is under" (any_parent) — v matches
            // an entity anywhere up property k's link chain (state above a
            // neighborhood's city). Matched before ~= / = so the caret wins.
            m = s.match(/^([\w-]+)\^=(.+)$/)
            if (m) return { property: m[1], op: 'ancestor', value: m[2] }
            // ENH-255 — k~=v: multi-valued membership (list contains v, Link
            // elements matched by note identity).
            m = s.match(/^([\w-]+)~=(.+)$/)
            if (m) return { property: m[1], op: 'contains', value: m[2] }
            m = s.match(/^([\w-]+)=(.+)$/)
            ...
            die(`duo rollup ${sub}: unrecognized --filter ${JSON.stringify(s)} (use k=v, k!=v, k~=v, k^=v, @=v, k? or k!?)`)
          ...
          if (sub === 'new') {
            ...
            const model: vault.RollupBuilderModel = {
              ...
              types,
              groupBy: list('--group') ?? [],
              buckets: bucketArgs.map(parseBucketFlag),
              filters: filterArgs.map(parseFilterFlag),
              columns: list('--columns') ?? [],
            }
            if (model.buckets.length > 0 && model.groupBy.length === 0)
              die('duo rollup new: --bucket needs --group (buckets declare groups for level 1)')
            const created = vault.createRollupNote(root, model)
            out({ root, note: created.noteRel, absPath: created.absPath, model })
          } else {
            const target = positionalArgs(subRest, ['--vault', '--title', '--type', '--types', '--group', '--filter', '--bucket', '--columns', '--links'])[0]
            ...
            const modelFlagsUsed =
              ...
              subRest.includes('--clear-buckets')
            if (!modelFlagsUsed) {
            ...
            if (model.buckets.length > 0 && model.groupBy.length === 0)
              die('duo rollup set: --bucket needs a group level (set --group too, or on the note already)')
            // BUG-256 — a --title change that shifts the slug renames
            // rollups/<slug>.md on disk (rejecting, not clobbering, when the
            // target name is already taken).
            const saved = vault.saveRollupNoteWithRename(root, data.noteAbs, model)
            if (!saved.ok) die(`duo rollup set: ${saved.error}`)
            out({
              root,
              note: saved.noteRel,
              renamed: saved.renamed,
              model,
              ...(linksFlag != null ? { links: linksFlag } : {}),
            })
          }
        } else if (sub === 'show') {
          ...
          out({
            ...
            // ENH-255 — declared buckets (label + matched group key, null =
            // empty) and filter eval-warnings, so an agent reading `show`
            // sees a broken filter instead of a plausible zero-row rollup.
